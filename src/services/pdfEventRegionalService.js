...
export const toTitleCase = (str) => {
  ...
  return str
    ...
    .join(" "); // Diminui tudo e aumenta a primeira letra de cada palavra
};
...
export const pdfEventRegionalService = {
  ...
  generateAtaRegional: async (
    ...
    doc.setFontSize(14);
    doc.text("Relatório do Serviço de Ensaio Regional", pageWidth / 2, 15, {
      align: "center",
    });
    doc.setFontSize(11);
    ...
    doc.setFontSize(10);
    doc.text(`${sedeNomeFormatado} - ${cidadeNome}`, pageWidth / 2, 27, {
      align: "center",
    });
    doc.setFont("times", "bold");
    ...
    const families = [
      ...
      },
      {
        ...
        total: stats.madeiras,
      },
      {
        name: "SAXOFONES",
        keys: ["saxsoprano", "saxalto", "saxtenor", "saxbaritono"],
        total: stats.saxofones,
      },
      {
        name: "METAIS",
        keys: [
    ...
    Object.keys(counts || {}).forEach((key) => {
      if (key.startsWith("extra_")) {
        ...
        const famIndex = families.findIndex((f) => f.name === famName);
    ...
    families.forEach((fam) => {
      const instruments = fam.keys
        .map((k) => ({ key: k, data: counts?.[k] }))
        .filter((i) => i.data);
      instruments.forEach((inst, idx) => {
        ...
        row.push(
          (inst.data?.name || inst.key.replace(/_/g, " ")).toUpperCase(),
        );
        row.push(Number(inst.data?.total) || 0);
        if (idx === 0) {
          row.push({
            content: fam.total.toString(),
            rowSpan: instruments.length,
            styles: {
              halign: "center",
              valign: "middle",
    ...
    // Adiciona a linha de totalização da orquestra
    orqBody.push([
      {
      ...
      {
        content: stats.musicos.toString(),
        styles: {
          fontStyle: "bold",
          valign: "middle",
          halign: "center",
    ...
    doc.setFontSize(8);
    doc.text(
      `Atendimento: ${toTitleCase(ataData?.atendimentoNome) || "---"} (${ataData?.atendimentoMin || "---"})`,
      rightColX,
      rightY + 5,
    );
    doc.text(
      `Hino de Abertura: ${ataData?.hinoAbertura || "---"}`,
      rightColX,
      rightY + 9,
    );
    const pal = ataData?.palavra || {};
    doc.text(
      `Palavra: ${pal.livro || "---"}, ${pal.capitulo || "0"} : ${pal.verso || "0"}`,
      rightColX,
      rightY + 13,
    );
    const assuntoTxt = doc.splitTextToSize(
      `Assunto: ${pal.assunto || "---"}`,
      90,
    );
    ...
    let pY = rightY + 24;
    doc.setFont("times", "bold");
    doc.text("PARTE MUSICAL", rightColX, pY);
    doc.setFont("times", "normal");
    let totH = 0;
    (ataData?.partes || []).forEach((p, i) => {
      ...
      const txt = doc.splitTextToSize(
        `${p.label || i + 1 + "ª PARTE"}: ${toTitleCase(condutor)} | Hinos: ${hinos.join(", ")}`,
        90,
      );
    ...
    doc.setFont("times", "bold");
    doc.text(`TOTAL GERAL DE HINOS: ${totH}`, rightColX, pY);

    // 6. OCORRÊNCIAS
    if (ataData?.ocorrencias?.length > 0) {
      pY += 6;
      doc.setFont("times", "bold");
      doc.text("OCORRÊNCIAS DO ENSAIO", rightColX, pY);
    ...
    const totalMadeirasESax = Math.max(1, stats.madeiras + stats.saxofones);
    const renderBar = (label, val, base, y, color, isSub = false) => {
      ...
      doc.text(
        `${label} [${val}/${base}]`,
        isSub ? rightColX + 4 : rightColX,
        y,
      );
      doc.setFont("times", "normal");
      doc.text(`${percReal.toFixed(0)}%`, pageWidth - margin, y, {
        align: "right",
      });
      doc.setFillColor(245, 245, 245);
      doc.rect(rightColX, y + 1.5, 90, 2, "F");
    ...
    doc.setFillColor(241, 245, 249); // Faixa na cor prata/cinza claro
    doc.rect(margin, totY, pageWidth - 20, 18, "F"); // Aumenta a altura para acomodar o novo texto
    doc.setFont("times", "bold");
    doc.setFontSize(14);
    doc.setTextColor(30, 30, 30);
    const mainTxt = `MÚSICOS: ${stats.musicos}  |  ORGANISTAS: ${stats.organistas}  |  CORAL: ~ ${stats.irmandade}  |  TOTAL GERAL: ${stats.geral}`;
    const mainTxtWidth =
      (doc.getStringUnitWidth(mainTxt) * doc.internal.getFontSize()) /
      doc.internal.scaleFactor;
    const startX = (pageWidth - mainTxtWidth) / 2;

    doc.text(
      mainTxt,
      pageWidth / 2,
      totY + 8, // Posição do texto principal
      { align: "center" },
    );

    // Adiciona o totalizador da orquestra abaixo
    doc.setFontSize(8);
    doc.setTextColor(100, 116, 139);
    doc.text(
      `ORQUESTRA: ${stats.orquestra}`,
      startX,
      totY + 13, // Posição abaixo do texto principal
      { align: "left" },
    );
    ...
    doc.setFontSize(8);
    doc.setFont("times", "bold");
    const totalMinisterio = stats.ministerio_total + stats.encRegional;
    doc.text(
      `RESUMO DA PRESENÇA MINISTERIAL (${totalMinisterio})`,
      margin,
      resY,
    );
    doc.setFont("times", "normal");
    doc.text(
      `ANCIÃES: ${stats.ancianosCasa + stats.ancianosVisitas}`,
      margin,
      resY + 5,
    );
    doc.text(
      `DIÁCONOS: ${stats.diaconosCasa + stats.diaconosVisitas}`,
      margin,
      resY + 9,
    );
    doc.text(
      `COOP. OFÍCIO: ${stats.coopOficioCasa + stats.coopOficioVisitas}`,
      margin,
      resY + 13,
    );
    doc.text(
      `COOP. RJM: ${stats.coopJovensCasa + stats.coopJovensVisitas}`,
      margin,
      resY + 17,
    );

    doc.text(
      `ENC. REGIONAIS: ${stats.encRegionalCasa + stats.encRegionalVisitas}`,
      rightColX,
      resY + 5,
    );
    doc.text(
      `EXAMINADORAS: ${stats.examinadorasCasa + stats.examinadorasVisitas}`,
      rightColX,
      resY + 9,
    );
    doc.text(
      `ENC. LOCAIS: ${stats.encLocalCasa + stats.encLocalVisitas}`,
      rightColX,
      resY + 13,
    );
    ...
    autoTable(doc, {
      ...
      didDrawPage: (data) => {
        ...
        doc.setFont("times", "normal");
        doc.text(
          `${rua}, ${num} - ${bairroEnd} • ${cidadeNome}`,
          margin + 17,
          footerY + 1,
        );
        doc.text(
          `Culto: ${sedeFullData?.diasSelecao?.map((d) => translateDay(d)).join(" e ") || "---"} às ${sedeFullData?.horaCulto || "---"} | Ensaio Local: ${sedeFullData?.ensaioLocal || "---"} às ${sedeFullData?.horaEnsaio || "---"}`,
          margin + 17,
          footerY + 4,
        );
        doc.setFontSize(6.5);
        doc.text(
          `Secretaria de Música ${toTitleCase(regionalNome)} • Sistema de Gestão Digital`,
          pageWidth / 2,
          pageHeight - 5,
          { align: "center" },
        );
    ...
    // FINALIZAÇÃO
    doc.save(
      `${dateParts[0]}-${dateParts[1]}-${dateParts[2]} - ATA REGIONAL ${sedeNomePura.toUpperCase()}.pdf`,
    );
  },
};
