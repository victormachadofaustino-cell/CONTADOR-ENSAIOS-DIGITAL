...
export const authService = { // Explicação: Inicia o motor de serviços de usuário.
  ...
  login: async (email, password) => { // Explicação: Função para entrar no sistema.
    try { // Explicação: Inicia a tentativa de login protegido.
      ...
      if (!cred.user.emailVerified) { // Explicação: Se o usuário não clicou no link do e-mail ainda...
        ...
        throw new Error("ACESSO BLOQUEADO: E-mail não verificado. Verifique sua caixa de entrada e SPAM."); // Explicação: Trava a execução e avisa a tela.
    ...
    } catch (error) { // Explicação: Captura falhas de digitação ou erros do Google Auth.
      if (error.code === 'auth/user-not-found' || error.code === 'auth/wrong-password' || error.code === 'auth/invalid-credential') { // Explicação: Se der erro de credencial inválida:
  ...
  register: async ({ email, password, name, role, comum, comumId, cidadeId, regionalId }) => { // Explicação: Função que cria novos músicos.
    ...
    if (!cidadeId || !regionalId || !comumId) { // Explicação: Se faltar algum dado de localização, nem tenta criar.
      throw new Error("Falha de jurisdição: Dados geográficos obrigatórios ausentes."); // Explicação: Barra o cadastro se o usuário tentar burlar a localidade.
    ...
    try { // Explicação: Inicia a tentativa de cadastro em duas etapas.
      ...
      try { // Explicação: Inicia a tentativa de gravar a ficha cadastral associada.
        await setDoc(doc(db, 'users', cred.user.uid), { // Explicação: Grava os dados na pasta /users/ usando o ID único do Google.
        ...
        if (cred.user) { // Explicação: Verifica se a sessão do usuário recém-criado está ativa no navegador.
      ...
      } catch (firestoreError) { // Explicação: Bloco acionado se o Firestore barrar a gravação da ficha (ex: erro de regras).
        // ROLLBACK: Se falhar no banco, deleta o login para liberar o e-mail para nova tentativa
        if (cred.user) { // Explicação: Se a conta de e-mail chegou a ser criada no Google Auth:
        ...
        throw new Error("ERRO DE PERMISSÃO: Falha ao registrar perfil no banco. Contate o administrador."); // Explicação: Retorna um aviso limpo para a tela do usuário.
      } // Explicação: Fim do bloco catch do Firestore.

    } catch (authError) { // Explicação: Captura falhas geradas na primeira etapa (criação da conta no Google).
      if (authError.code === 'auth/email-already-in-use') { // Explicação: Se o Google avisar que o e-mail já possui cadastro:
        throw new Error("Este e-mail já está em uso. Tente fazer login ou recupere sua senha."); // Explicação: Avisa o usuário de forma amigável.
  ...
  logout: async () => { // Explicação: Função para sair do sistema com segurança.
    try { // Explicação: Inicia tentativa de limpeza profunda dos dados locais.
    ...
    } catch (error) { // Explicação: Caso aconteça alguma falha mecânica no encerramento:
