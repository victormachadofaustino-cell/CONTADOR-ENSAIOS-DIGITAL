...
let updateBuffers = {}; // [Funcionamento]: Armazém temporário na memória RAM para reret os valores intermediários digitados no input.
...
export const eventService = { 
  ...
  getUsersRegional: async (regionalId) => { // [Funcionamento]: Método assíncrono para buscar todos os usuários aprovados de uma regional específica.
    ...
    try { // [Funcionamento]: Inicia a tentativa de consulta ao banco de dados.
      const q = query( // [Funcionamento]: Constrói a estrutura da busca com filtros integrados.
  ...
  subscribeToEvents: (user, callback) => { // [Funcionamento]: Abre uma escuta reativa em tempo real para listar os eventos do usuário conectado.
    ...
    else if (user.activeRegionalId || user.regionalId) { // [Funcionamento]: Caso seja administrador regional and possua uma regional focada.
    ...
    if (user.accessLevel === ROLES.GEM || user.accessLevel === 'gem_local' || user.accessLevel === 'gem' || user.accessLevel === 'GEM') { // [Funcionamento]: Se o operador for estritamente um secretário local GEM.
      constraints = [or( // [Funcionamento]: Aplica uma condition lógica OU especial de portaria do banco.
  ...
  createEvent: async (comumId, eventData, userData = null) => { // [Funcionamento]: Método encarregado de criar e estruturar um novo ensaio no banco de dados.
    if (!comumId && !eventData?.comumId && !userData?.comumId && !userData?.activeComumId) throw new Error("ID da Localidade ausente."); // [Funcionamento]: Regra de segurança: impede a criação de ensaios sem uma igreja amarrada.
    ...
    const finalComumId = comumId || eventData?.comumId || userData?.activeComumId || userData?.comumId || ''; // [Funcionamento]: Dissolve de forma definitiva as strings nulas ou vazias que quebravam as chamadas nominais da linha 235.
    ...
    const finalCidadeId = cidadeId || eventData?.cidadeId || userData?.cidadeId || userData?.activeCityId || ''; // [Funcionamento]: Extrai e garante o preenchimento legítimo do ID territorial da cidade para travar a portaria do Firebase rules.
    const finalRegionalId = regionalId || eventData?.regionalId || userData?.regionalId || userData?.activeRegionalId || ''; // [Funcionamento]: Extrai e garante o preenchimento legítimo do ID territorial da regional.
    ...
    try { // [Funcionamento]: Inicia o bloco de transação com o banco de dados.
        ...
        if (id === 'coral' || id === 'orgao') { // [Funcionamento]: Tratamento especial de inicialização para as seções complexas de Teclas/Irmandade.
          if (finalScope === 'regional') { // [Funcionamento]: Se for um ensaio amplo de escopo regional.
            ...
            initialCounts[id] = { // [Funcionamento]: Inicia a fiação simplificada local de Teclas e Coral.
              total: 0, irmaos: 0, irmas: 0, // [Funcionamento]: Inicializa todos os sub-totalizadores zerados in linha.
      ...
      const payload = { // [Funcionamento]: Monta o documento mestre unificado final que será gravado no Firestore.
        ...
        responsavel: responsavel || 'Pendente', // [Funcionamento]: Salva o nome do Ancião ou encarregado condutor del ensaio.
      ...
      if (finalScope !== 'regional') { // [Funcionamento]: Se o ensaio for local comum (não regional), monta as chamadas de presença nominais da casa.
        ...
        ministerioSnap.docs.forEach(minDoc => { // [Funcionamento]: Varre obreiro por obreiro cadastrado na igreja comuns.
          ...
          const llamadaMinRef = doc(collection(db, 'events_global', docNovoCriado.id, 'chamada_ministerio'), minDoc.id); // [Funcionamento]: Prepara a rota da ficha de presença ministerial neste ensaio.
          batchChamada.set(llamadaMinRef, { // [Funcionamento]: Prepara a gravação da presença ministerial zerada no lote.
  ...
  reopenAta: async (eventId) => { // [Funcionamento]: Método encarregado de reabrir uma ata lacrada para correções tardias de secretaria.
    ...
    const eventRef = doc(db, 'events_global', eventId); // [Funcionamento]: Aponta para a rota física do ensaio dentro della coleção global.
  ...
  updateInstrumentCount: async (comumId, eventId, { instId, field, value, userData, section, customName }) => { 
    ...
    // 🚀 REDIRECIONAMENTO DE CANOS REGIONAL: Se o clique vier das pílulas soltas de irmas/irmaos do Regional, força a rota mestre 'coral' para unificar os dados!
    let targetId = (rawId === 'irmas' || rawId === 'irmaos') ? 'coral' : (INSTRUMENT_ID_MAP[rawId] || rawId); // [Funcionamento]: Sanitiza e centraliza a árvore do JSON no nó pai correto em minúsculo.
    ...
    // 🚀 ALINHAMENTO DE SUB-CHAVES: Se o ID de clique original for irmas ou irmaos, repassa a sub-chave correta para o campo a ser atualizado dentro do Coral.
    const fieldToUpdate = (rawId === 'irmas' || rawId === 'irmaos') ? rawId : (field === 'total_simples' ? 'total' : field); // [Funcionamento]: Garante o mapeamento cirúrgico de propriedades internas.
    ...
    if (userData && userData.uid) { // [Funcionamento]: Se o crachá do secretário ativo contiver um código de login legítimo.
      ...
      if (rawId === 'irmas' || rawId === 'irmaos') {
        ...
        updateBuffers[timerKey][`responsibleName_${rawId}`] = userData.name || userData.responsavel || 'Secretaria'; // [Funcionamento]: Assina o Nome na vaga de gênero do buffer.
      } else {
        ...
        updateBuffers[timerKey]['responsibleName'] = userData.name || userData.responsavel || 'Secretaria'; // [Funcionamento]: Carimba o Nome por extenso do responsável na gaveta da RAM mestre.
      }
    ...
    debounceTimers[timerKey] = setTimeout(async () => { // [Funcionamento]: Abre uma contagem regressiva de 400 milissegundos na memória RAM. Quando você parar de digitar, ele executa o bloco abaixo.
      ...
      try { // [Funcionamento]: Inicia a tentativa de gravação definitiva em lote na nuvem do Firestore.
        ...
        if (targetId === 'coral') { // [Funcionamento]: Se o identificador de destino for o Coral, executa a mesclagem atômica lendo o documento físico para impedir pulverizações.
          ...
          const dadosAtuaisBanco = docSnapSnapshot.exists() ? docSnapSnapshot.data()?.counts?.coral : {}; // [Funcionamento]: Extrai com segurança a gaveta atual do Coral salva na nuvem.
          ...
          const finalIrmas = typeof bufferCopy['irmas'] !== 'undefined' ? bufferCopy['irmas'] : bancoIrmas; // 🚀 SOLUÇÃO DO UNDEFINED: Se o clique atual não trouxe as irmãs, herda o valor real do banco em vez de zerar!
          const finalIrmaos = typeof bufferCopy['irmaos'] !== 'undefined' ? bufferCopy['irmaos'] : bancoIrmaos; // 🚀 SOLUÇÃO DO UNDEFINED: Se o clique atual não trouxe os irmãos, herda o valor real do banco em vez de zerar!
          const somaTotalAbsoluta = finalIrmas + finalIrmaos; // [Funcionamento]: Realiza a soma matemática legítima das duas metades da irmandade.
  ...
  addMusicoComum: async (comumId, musicoPayload) => { // [Funcionamento]: Cadastra a ficha cadastral de um novo músico na lista nominal fixa da igreja comum.
    ...
    try { // [Funcionamento]: Tenta efetuará o cadastro.
      ...
      const novaFichaRef = doc(collection(db, 'comuns', comumId, 'musicos_lista')); // [Funcionamento]: Prepara a nova rota com ID aleatório na subcoleção fixa da igreja comuns.
    ...
    } catch (e) { // [Funcionamento]: Cadastra a ficha cadastral de um novo músico na lista nominal fixa da igreja comum.
