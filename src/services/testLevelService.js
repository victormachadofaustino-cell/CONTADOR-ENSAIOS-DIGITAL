...
import { db, collection, doc, setDoc, getDocs, query, orderBy, deleteDoc, updateDoc, onSnapshot } from '../config/firebase';
...
export const testLevelService = {
  ...
  initializeDefaultLevels: async () => {
    try {
      ...
      const defaultLevels = [
        { id: 'aluno', name: 'Aluno', description: 'Candidato em fase de aprendizado inicial', active: true, order: 1 },
        { id: 'aprendiz', name: 'Aprendiz', description: 'Apto a tocar nos cultos de jovens e menores', active: true, order: 2 },
        { id: 'oficializado', name: 'Oficializado', description: 'Apto a tocar em todos os cultos e serviços', active: true, order: 3 },
        { id: 'rjm', name: 'RJM', description: 'Reunião de Jovens e Menores', active: true, order: 4 }
      ];
  ...
  createLevel: async (name, description, orderIndex = 99) => {
    ...
    const idSaneado = name.toLowerCase().trim()
      .replace(/\s+/g, '_')
      .replace(/[^\w]/g, '');
