...
export const PermissionProvider = ({ children }) => {
  ...
  const permissions = useMemo(() => {
    ...
    const canCreateEvent = (eventCityId) => {
      // Explicação: Função que avalia se o usuário tem o direito de abrir uma nova agenda de ensaio.
      ...
      if (level === "regional_cidade") {
        // Explicação: Se for gestor de cidade/comarca.
    ...
    const canEditCounter = (
      eventComumId,
      sectionMetaOwnerId = null,
      eventRegionalId = null,
      scope = "local",
    ) => {
      // Explicação: Alinhado v2.0 - Avalia se o usuário pode somar, subtrair ou assumir a caneta de uma seção baseado no escopo e na regional carimbada.
      if (isMaster || isComissao) return true; // Explicação: Autoridades máximas editam e assinam qualquer naipe em qualquer tempo de forma universal.

      const minhaComumLegitima = userData?.comumId || userData?.activeComumId; // Explicação: Puxa a igreja mãe de cadastro fixa do obreiro conectado.
      const ehEnsaioDaMinhaCasa = minhaComumLegitima === eventComumId; // Explicação: Compara se o ensaio aberto pertence à igreja nativa dele de cadastro.

      if (
        (level === "gem_local" || level === "basico") &&
        ehEnsaioDaMinhaCasa
      ) {
        // Explicação: Se for da mesma igreja comum e o ensaio for puramente local.
        return true; // Explicação: Libera o controle total instantaneamente para os cliques de contagem do secretário da casa.
      } // Explicação: Fim da liberação local.

      if (
        level === "gem_local" &&
        scope === "regional" &&
        eventRegionalId === userData?.regionalId
      ) {
        // Explicação: Injeção de Regra de Regionalidade: Libera o GEM Local para assumir se for um evento regional na mesma comarca dele.
        return sectionMetaOwnerId === userData?.uid || !sectionMetaOwnerId; // Explicação: Devolve verdadeiro se ele for o dono do naipe ou se a vaga de caneta estiver livre no momento.
      } // Explicação: Fim da regra de liberação regional para GEM Local.

      if (level === "regional_cidade") {
        // Explicação: Se o operador for Encarregado de Cidade.
        return sectionMetaOwnerId === userData?.uid; // Explicação: Só altera os instrumentos se ele tiver assumido explicitamente a responsabilidade da seção.
      } // Explicação: Fim da trava de cidade.
      return sectionMetaOwnerId === userData?.uid; // Explicação: Fallback padrão de segurança restrita baseado no ID do responsável logado.
    }; // Explicação: Encerra a função canEditCounter.

    // REGRA 3: QUEM PODE FECHAR OU REABRIR O LIVRO DE ATAS
    const canManageAtaStatus = (eventCityId) => {
      // Explicação: Determina quem tem o poder eclesiástico de lacrar ou reabrir uma ata de ensaio.
      if (isMaster || isComissao) return true; // Explicação: Comissão Regional e Master lacram ou reabrem relatórios de forma universal.
      if (level === "regional_cidade") {
        // Explicação: Se o operador for Encarregado de Cidade.
        return eventCityId === userData?.cidadeId; // Explicação: Libera o lacre se o evento estiver acontecendo dentro da comarca municipal dele.
      } // Explicação: Fim da validação municipal.
      return false; // Explicação: GEM Local ou músicos básicos nunca lacram atas de nível regional ou de comarca.
    ...
    const canWriteChurchSettings = (churchComumId, churchCityId) => {
      // Explicação: Controla a caneta de edição das configurações e fichas de orquestras locais.
      if (isMaster || isComissao) return true; // Explicação: Master e comissão gerenciam o alistamento de qualquer casa sem restrições.
      if (level === "regional_cidade") {
        // Explicação: Se o operador for Encarregado de Cidade.
        return churchCityId === userData?.cidadeId; // Explicação: Permite alterar a ficha se a igreja comum pertencer ao seu município de atuação.
      } // Explicação: Fim da validação de comarca.
      if (level === "gem_local") {
        // Explicação: Se o operador for secretário local.
        return churchComumId === userData?.comumId; // Explicação: Só permite alterar se for estritamente a ficha de sua própria igreja de direito.
      } // Explicação: Fim do bloco do GEM Local.
      return false; // Explicação: Músicos comuns nunca editam ou alteram alistamentos de orquestras.
    ...
    isGemLocal,
    isBasico,
  ]); // Explicação: Só re-processa a matriz inteira se a conta física do usuário mudar no Firebase Auth.
