...
import { db } from "../api/firebase"; // Caminho confirmado conforme sua instrução
import {
  collection,
  addDoc,
  query,
      ...
      {showFeedback &&
        createPortal(
          <div
            ...
            className="fixed inset-0 z-[2000] bg-slate-950/20 backdrop-blur-[2px] flex items-end justify-center p-4 pb-24"
          >
            <motion.div
            ...
            >
              ...
              <div className="flex justify-between items-center mb-6">
                <div className="flex bg-slate-100 p-1 rounded-xl">
                  <button
                    onClick={() => setAbaFeedback("novo")}
                    className={`px-4 py-1.5 rounded-lg text-[9px] font-black uppercase ${abaFeedback === "novo" ? "bg-white text-slate-950 shadow-sm" : "text-slate-400"}`}
                  >
                    Novo
                  </button>
                  <button
                    onClick={() => setAbaFeedback("historico")}
                    className={`px-4 py-1.5 rounded-lg text-[9px] font-black uppercase ${abaFeedback === "historico" ? "bg-white text-slate-950 shadow-sm" : "text-slate-400"}`}
                  >
                    Histórico
                  </button>
                </div>
                <button
                  onClick={() => setShowFeedback(false)}
                  className="p-2 bg-slate-50 text-slate-300 rounded-full active:scale-90 transition-all"
                >
                  <X size={16} />
                </button>
              </div>
              ...
              {abaFeedback === "novo" ? (
                <form
                  ...
                  className="space-y-4 animate-premium"
                >
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <TypeButton
                      active={tipoFeedback === "bug"}
                      onClick={() => setTipoFeedback("bug")}
                      icon={<Bug size={12} />}
                      label="Bug"
                    />
                    <TypeButton
                      active={tipoFeedback === "sugestao"}
                      onClick={() => setTipoFeedback("sugestao")}
                      icon={<MessageSquare size={12} />}
                      label="Idéia"
                    />
                    <TypeButton
                      active={tipoFeedback === "elogio"}
                      onClick={() => setTipoFeedback("elogio")}
                      icon={<Star size={12} />}
                      label="Elogio"
                    />
                  </div>
                ...
                <div className="space-y-3 max-h-80 overflow-y-auto no-scrollbar pb-2 animate-premium">
                  {meusFeedbacks.length === 0 ? (
                    <div className="py-12 text-center text-slate-300 text-[10px] font-black uppercase italic">
                    ...
                    meusFeedbacks.map((f) => {
                      ...
                      return (
                        <div
                          key={f.id}
                          className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-left relative overflow-hidden transition-all"
                        >
                          {/* Borda lateral colorida idêntica ao modelo regional  */}
                          <div
                            className={`absolute top-0 left-0 w-1 h-full ${style.border.replace("border-", "bg-")}`}
                          />
                          ...
                          <div className="flex justify-between items-start mb-2">
                            ...
                            <span
                              className={`text-[7px] font-black uppercase px-2 py-0.5 rounded-full ${style.bg} ${style.color}`}
                            >
                          ...
                          {f.respostaMaster && (
                            <div className="mt-3 pt-3 border-t border-slate-200 flex gap-2 animate-in fade-in">
                              ...
                              />
                              <p className="text-[9px] font-black text-slate-900 uppercase leading-tight">
                                Retorno Master:{" "}
                                <span className="font-bold text-slate-500 normal-case italic">
                                  {f.respostaMaster}
                                </span>
                              </p>
                            </div>
