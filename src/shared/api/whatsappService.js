...
const formatarData = (dateStr) => {
  // [Funcionamento]: Converte strings técnicas de datas "2026-05-16" para formato visual "16/05/2026".
...
const toTitleCase = (str) => {
  // [Funcionamento]: Função auxiliar que padroniza nomes próprios e textos litúrgicos com iniciais maiúsculas.
  if (!str) return ""; // [Funcionamento]: Aborta se a string for nula ou inválida.
  return str
    .toLowerCase()
    .split(" ")
    .map((w) => w.charAt(0).toUpperCase() + w.slice(1))
    .join(" "); // [Funcionamento]: Diminui o texto e joga a primeira letra em caixa alta.
}; // [Funcionamento]: Encerra a função utilitária de capitalização.

const getChoirCount = (c) => {
  const choir = c?.coral || c?.Coral || c?.irmandade || {};
  return (parseInt(choir.irmaos) || 0) + (parseInt(choir.irmas) || 0);
};

export const whatsappService = {
  // [Funcionamento]: Exporta a classe mestre de formatação e templates para o WhatsApp do app.

  /**
   * MODELO 1: ALIMENTAÇÃO (COZINHA / COPA) - REFINADO EM NÍVEL PREMIUM CCB
   */
  obterTextoAlimentacao: (event, stats = null) => {
    // [Funcionamento]: Constrói a mensagem otimizada para o controle lanche da Copa.
    ...
    if (stats) {
      // [Funcionamento]: Se os cálculos da tela estiverem ativos na RAM, aproveita as chaves calculadas de custo zero.
      ...
      // [Funcionamento]: Fallback reativo caso o disparo ocorra direto de um snap de dados brutos.
      totalOrganistas = parseInt(counts.orgao?.total) || 0;
      totalIrmandade = getChoirCount(counts);
      totalMusicos = Object.keys(counts) // [Funcionamento]: Varre as chaves remanescentes para descobrir os sopros e arcos.
        .filter(
          (key) =>
            !["irmandade", "Coral", "coral", "orgao"].includes(key) &&
            !key.startsWith("meta_"),
        ) // [Funcionamento]: Ignora congregação e órgãos.
    ...
    // AJUSTE: Contabiliza ministério que não toca para o total de lanches.
    const allMinisterio = [
      ...(event?.presencaLocalFull || []),
      ...(event?.visitantes || []),
    ];
    const ministerioNaoTocando = allMinisterio.filter(
      (p) => p.tocando !== true,
    ).length;

    const totalGeral = totalOrquestra + totalIrmandade + ministerioNaoTocando; // [Funcionamento]: Soma final absoluta de lanches requeridos pela portaria.
  ...
  obterTextoEstatistico: (event, stats = null) => {
    // [Funcionamento]: Compila o relatório formal completo do Ensaio Local ou Regional.
    ...
    // 📊 EXTRAÇÃO MATRICIAL DAS ESTEIRAS DE CÁLCULO DA RAM (Captura as chaves locais e regionais sem conflitos)
    const totalMusicos = stats
      ? stats.musicos
      : Object.keys(counts)
          .filter(
            (key) =>
    ...
    const totalOrganistas = stats
      ? stats.organistas
      : parseInt(counts.orgao?.total) || 0;

    const totalIrmandadeCoral = stats ? stats.irmandade : getChoirCount(counts);

    // AJUSTE: Contabiliza ministério que não toca para o total geral, caso 'stats' não seja fornecido.
    const allMinisterio = [
      ...(event?.presencaLocalFull || []),
      ...(event?.visitantes || []),
    ];
    ...
    const totalGeral = stats
      ...
      : totalMusicos +
        totalOrganistas +
        totalIrmandadeCoral +
        ministerioNaoTocando;
    const totalHinos = stats
      ? stats.hinos || event?.hinosChamados || 0
      : event?.hinosChamados || 0;

    // 🔧 PONTES DE MAPEAMENTO CRUZADO DE ACORDO COM O ESCOPO ATIVO (Evita o bug do zero absoluto)
    const totalEncReg = stats
      ? typeof stats.encRegional === "number"
        ? stats.encRegional
