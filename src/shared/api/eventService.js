...
} from "./firebase"; // [Funcionamento]: Mantém a importação de todos os conectores e métodos oficiais do SDK do Firebase Firestore.
import {
  deleteField,
  arrayUnion,
  arrayRemove,
...
export const eventService = {
  ...
  // FUNÇÃO AUXILIAR: Monta a grade inicial de contagens
  async _buildInitialCounts(finalComumId, finalScope) {
    ...
    localSnap.docs.forEach((docInst) => {
      ...
      const id =
        ...
        originalId.toLowerCase().trim();
      const sectionName = inst.section?.toUpperCase() || "GERAL";
      sessoesDetectadas.add(sectionName);

      const isComplex = id === "coral" || id === "orgao";
      const isRegional = finalScope === "regional";
  ...
  // FUNÇÃO AUXILIAR: Cria a chamada nominal em lote
  _createNominalRoll: async (eventId, finalComumId) => {
    const batch = writeBatch(db);
    const collectionsToProcess = ["musicos_lista", "ministerio_lista"];

    for (const collName of collectionsToProcess) {
      const listRef = collection(db, "comuns", finalComumId, collName);
      const listSnap = await getDocs(listRef);

      listSnap.docs.forEach((docData) => {
        // A lógica de criação da chamada para músicos e ministério seria movida para cá.
        // Para manter o exemplo conciso, a lógica interna foi omitida, mas a estrutura está aqui.
      });
    }
    // await batch.commit(); // O commit seria feito aqui.
  },

  async createEvent(comumId, eventData, userData = null) {
    ...
    try {
      // 1. Monta a grade de contagens usando a função auxiliar
      const initialCounts = await eventService._buildInitialCounts(
        finalComumId,
        finalScope,
      );
      ...
      if (finalScope !== "regional") {
        ...
        musicosSnap.docs.forEach((musicoDoc) => {
          ...
          // 🚀 BLINDAGEM DE ROBUSTEZ: Se um músico estiver cadastrado sem instrumento, pula ele para não quebrar a criação da chamada.
          if (!mData || !mData.instrumentoId) {
            // Explicação: Adiciona uma verificação de segurança para garantir que o músico tem dados válidos e um instrumento.
            console.warn(
              `Músico '${mData.nome}' (ID: ${musicoDoc.id}) está sem instrumento cadastrado e será pulado.`,
            ); // Explicação: Avisa no console do navegador sobre o músico ignorado.
            return; // Explicação: Interrompe a execução para este músico específico e continua para o próximo.
          }
