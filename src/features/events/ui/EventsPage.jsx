...
import {
  ...
  or,
  documentId,
} from "../../../shared/api/firebase"; // [Funcionamento]: Conecta ao banco de dados Firebase.
import { eventService } from "../../../shared/api/eventService"; // [Funcionamento]: Importa o motor de busca e criação de eventos.
...
import { hasPermission } from "../../../shared/config/permissions"; // [Funcionamento]: Importa a Regra de Ouro do sistema.

// AUXILIAR DE DATA LOCAL
const getLocalDate = () => {
  // [Funcionamento]: Função utilitária que calcula e *-devolve o fuso horário correto no formato YYYY-MM-DD.
...
const EventsPage = ({
  ...
  // [Funcionamento]: Inicia a estrutura da página de gestão de eventos.
  ...
  const isRegionalCidade =
    isMaster || isComissao || level === "regional_cidade"; // [Funcionamento]: Define se o usuário administra regiões ou comarcas inteiras.
  ...
  const activeRegionalId =
    userData?.activeRegionalId || userData?.regionalId || ""; // [Funcionamento]: Consolida o ID da regional ativa do cabeçalho móvel.

  const permitidasIds = useMemo(() => {
    // [Funcionamento]: Processa em memória RAM a lista de igrejas onde este obreiro possui direito de controle.
  ...
  const temPermissaoAqui = useMemo(() => {
    // [Funcionamento]: Lógica de portaria visual que decide se o botão de "Novo Ensaio" deve aparecer na tela.
    ...
    if (level === "regional_cidade") {
      // [Funcionamento]: Se for gestor de cidade.
    ...
    comuns,
    permitidasIds,
  ]); // [Funcionamento]: Avalia novamente as travas se mudar o foco territorial.

  // 1. MONITOR DE EVENTOS v10.13 - FIX: DEPENDÊNCIA ESTÁVEL
  useEffect(() => {
    // [Funcionamento]: Conecta o ouvinte reativo de banco que escuta a lista de ensaios da comum selecionada.
    ...
    const unsubEvents = eventService.subscribeToEvents(userData, (data) => {
      // [Funcionamento]: Chama o serviço passando o crachá do usuário logado.
  ...
  useEffect(() => {
    // [Funcionamento]: Monitor que baixa e popula o menu seletor de Cidades baseado na Regional ativa.
    ...
    ); // Cria a busca de comarcas na nuvem.
    const unsubCid = onSnapshot(qCid, (sCids) => {
      // Abre o canal de escuta em tempo real.
      ...
      const todasCidades = sCids.docs.map((d) => ({
        id: d.id,
        nome: d.data().nome,
      })); // Transforma os dados em objetos levíveis.
      if (isComissao || isMaster) {
        // [Funcionamento]: Se for supervisor alto.
        const sorted = todasCidades.sort((a, b) =>
          a.nome.localeCompare(b.nome),
        ); // Organiza a lista de cidades por ordem alfabética de A a Z.
        setCidades(sorted); // Despeja no menu.
      } else {
        // [Funcionamento]: Se for gestor local ou de cidade comum.
        const filtradas = todasCidades.filter(
          (c) => c.id === userData?.cidadeId,
        ); // Tranca a visualização para exibir apenas a comarca dele.
  ...
  useEffect(() => {
    // [Funcionamento]: Monitor que puxa a lista de igrejas comuns baseando-se na cidade selecionada no primeiro menu.
    let isMounted = true; // Flag protetora de vazamento de RAM.

    // 🚀 FIX ACESSO GEM/LOCAL: A lógica de consulta foi reestruturada para respeitar as permissões.
    // Para 'gem_local', a busca agora é feita pelos IDs de igreja permitidos, evitando erros de permissão do Firestore.
    if (!isRegionalCidade && (!permitidasIds || permitidasIds.length === 0)) {
      setComuns([]);
      return;
    }
    if (isRegionalCidade && !selectedCityId) {
      setComuns([]);
      return;
    }
    ...
    if (isRegionalCidade) {
      ...
      q = query(
        collection(db, "comuns"),
        where(documentId(), "in", permitidasIds),
      );
    }
    ...
    const unsub = onSnapshot(
      ...
      (snapshot) => {
        ...
        let churches = snapshot.docs.map((d) => ({
          id: d.id,
          nome: d.data().comum || "Sem Nome",
          cidadeId: d.data().cidadeId,
          regionalNome: d.data().regionalNome,
          cidadeNome: d.data().cidadeNome,
        }));

        if (!isRegionalCidade) {
          churches = churches.filter((c) => c.cidadeId === selectedCityId);
        }

        const sortedList = churches.sort((a, b) =>
          a.nome.localeCompare(b.nome),
        );
        setComuns(sortedList);

        if (sortedList.length === 0) {
          // [Funcionamento]: Se a cidade selecionada não possuir nenhuma igreja comum autorizada para o operador.
          if (selectedChurchId !== "") {
            // Se havia alguma comum marcada por resíduo de cache.
          ...
          // [Funcionamento]: Caso existam igrejas comuns válidas na lista retornada do servidor.
          const comumValida = sortedList.some((c) => c.id === selectedChurchId); // Confere se a igreja comum que estava marcada ainda é válida nesta nova lista.
          if (!comumValida) {
            // [Funcionamento]: Se a igreja comum anterior sumiu porque mudou a comarca da cidade.
            const primeira = sortedList[0]; // Isola a primeira igreja comum legítima della nova lista.
    ...
    return () => {
      isMounted = false;
      unsub();
    }; // Executa a limpeza física de hooks ao desmontar.
  }, [selectedCityId, isRegionalCidade, permitidasIds, selectedChurchId]); // [Funcionamento]: Vigia as trocas de cidades e permissões hierárquicas. Otimizado para evitar re-execuções desnecessárias.

  // 4. LÓGICA DE AGRUPAMENTO POR ANO
  const groupedEvents = useMemo(() => {
    // [Funcionamento]: Re-organiza a sacola de ensaios transformando-a em blocos separados por ano em memória RAM (Custo Zero).
    ...
    return combined.reduce((acc, event) => {
      // Agrupa os registros passando de ensaio em ensaio.
  ...
  useEffect(() => {
    // [Funcionamento]: Gatilho inicializado no boot que força o ano corrente (Ex: 2026) a iniciar com a sanfona aberta.
  ...
  const handleCreate = async (extendedData = {}) => {
    // [Funcionamento]: Disparada quando o obreiro preenche o formulário e clica em "Confirmar Criação".
    ...
    const finalCidadeId =
      selectedCityId || comumSelecionada?.cidadeId || userData?.cidadeId || ""; // [Funcionamento]: Saneia preventivamente o ID da comarca municipal.
    ...
    const nomeCidadeLimpo =
      comumSelecionada?.cidadeNome ||
      cidadeSelecionada?.nome ||
      userData?.cidadeNome ||
      "CIDADE"; // [Funcionamento]: Garante o nome por extenso correto da cidade alvo do ensaio.

    const finalEventData = {
      // Prepara o documento mestre higienizado e denormalizado (Payload).
      type:
        extendedData.scope === "regional" ? "Ensaio Regional" : "Ensaio Local", // Define o título baseado no rádio box.
      ...
      regionalNome: (
        comumSelecionada?.regionalNome ||
        userData?.regional ||
        userData?.activeRegionalName ||
        "REGIONAL"
      ).toUpperCase(), // [Funcionamento]: Captura o nome da regional por extenso dos metadados da comum ou do crachá do usuário e força caixa alta.
      comumNome: (
        comumSelecionada?.nome ||
        comumSelecionada?.comum ||
        userData?.comum ||
        "LOCALIDADE"
      ).toUpperCase(), // Força caixa alta no nome textual da igreja comuns para economia de cota de leitura.
    ...
    try {
      // Tenta enviar a instrução para a nuvem através do service central.
      ...
      // Captura e reporta falhas ocorridas na transação atômica.
      setShowModal(false); // Fecha a janela do formulário.
      if (error.message === "CONFIG_REQUIRED")
        setShowConfigError(true); // [Funcionamento]: Se o banco disparar o aviso de erro de grade, abre a tela de reset padrão para a secretaria configurar a orquestra da casa.
  ...
  const confirmDelete = async () => {
    // [Funcionamento]: Executa o comando definitivo de remoção física e deleção da agenda da coleção global.
    ...
    if (targetEvent?.ata?.status === "closed") {
      // [Funcionamento]: Trava Eclesiástica de Produto: Impede sumariamente a deleção de Atas encerradas e lacradas.
    ...
    try {
      ...
      // Captura falha de rede ou barreira de portaria do Firebase Rules.
  ...
  return (
    // [Funcionamento]: Desenha a interface na tela.
    <div className="min-h-screen bg-[#F1F5F9] p-4 pb-32 font-sans animate-premium text-left">
      {" "}
      {/* [Funcionamento]: Janela principal com rolagem vertical livre, fundo cinza-claro e animação premium de entrada. */}
      {/* SELETORES HIERÁRQUICOS (GPS) */}
      <div className="mb-6 max-w-md mx-auto flex items-center gap-2 px-1 text-left">
        {" "}
        {/* [Funcionamento]: Alinha as duas pílulas de rolagem horizontal centralizadas para formato smartphone. */}
        {/* PÍLULA 1: MENU DE CIDADES (FUNDO BRANCO TRANSLÚCIDO) */}
        <div
          className={`flex-[1_1_0px] flex items-center gap-2 bg-white/50 backdrop-blur-sm p-2.5 rounded-2xl border border-slate-200 shadow-sm transition-all text-left ${!isRegionalCidade || cidades.length === 0 ? "opacity-50 pointer-events-none" : ""}`}
        >
          {" "}
          {/* [Funcionamento]: Estilização responsiva: se for GEM Local comum, congela em fosco bloqueando a troca de cidade por segurança territorial. */}
          <MapPin size={10} className="text-blue-600 shrink-0" />{" "}
          {/* Desenha o ícone de alfinete de GPS azul real. */}
          <select
            disabled={!isRegionalCidade || cidades.length === 0}
            value={selectedCityId}
            onChange={(e) => setContext("city", e.target.value)}
            className="bg-transparent text-[9px] font-black uppercase outline-none w-full italic text-slate-950 appearance-none cursor-pointer text-left"
          >
            {" "}
            {/* [Funcionamento]: Menu de seleção nativo stylized em letras maiúsculas micro e formato itálico prêmio. */}
            <option value="">
              {cidades.length === 0
                ? "NENHUMA CIDADE"
                : isComissao || isMaster
            ...
            {/* Linha base padrão informativa. */}
            {cidades.map((c) => (
              <option key={c.id} value={c.id}>
                {c.nome}
              </option>
            ))}{" "}
            {/* Mapeia as cidades disponíveis baixadas do banco de dados no menu. */}
          </select>{" "}
          {/* Fecha a tag do seletor. */}
        </div>{" "}
        {/* Fecha a pílula de cidade. */}
        {/* PÍLULA 2: MENU DE IGREJAS COMUNS (PRETO HIGHDENSITY PREMIUM) */}
        <div
          className={`flex-[1_1_0px] flex items-center gap-2 bg-slate-950 p-2.5 rounded-2xl shadow-xl border border-white/10 transition-all text-left ${!isRegionalCidade || comuns.length === 0 ? "opacity-50 pointer-events-none" : ""}`}
        >
          {" "}
          {/* [Funcionamento]: Estilização sofisticada preta em alta densidade com bordas brilhantes sutis. */}
          <Home size={10} className="text-blue-600 shrink-0" />{" "}
          {/* Desenha o ícone eclesiástico de casa/igreja comum. */}
          <select
            ...
            className="bg-transparent text-[9px] font-black uppercase outline-none w-full italic text-white appearance-none cursor-pointer text-left"
          >
            {" "}
            {/* [Funcionamento]: Modifica a rota de dados do GPS reativo ao selecionar a igreja comune alvo. */}
            ...
            {/* Opção informativa de portaria. */}
            {comuns.map((c) => (
              <option key={c.id} value={c.id} className="text-slate-900">
                {c.nome}
              </option>
            ))}{" "}
            {/* Mapeia as igrejas comuns autorizadas do naipe na listagem do menu. */}
          </select>{" "}
          {/* Fecha o select. */}
        </div>{" "}
        {/* Fecha a pílula de comuns. */}
      </div>{" "}
      {/* Fecha o bloco de cabeçalho GPS. */}
      {/* Botão de Criação de Ensaio Novo */}
      {temPermissaoAqui &&
        selectedChurchId &&
        comuns.length > 0 && ( // [Funcionamento]: Validação Tripla de Segurança: Só renderiza o botão se o crachá tiver direito territorial, houver comarca marcada e a lista de comuns não estiver deserta.
          <div className="mb-8 max-w-md mx-auto text-center">
            {" "}
            {/* Centraliza o botão na largura mestre do celular. */}
            <button
              onClick={() => setShowModal(true)}
              className="w-full bg-slate-950 text-white py-5 rounded-[2.2rem] font-[900] uppercase italic tracking-[0.2em] shadow-2xl flex justify-center items-center gap-3 active:scale-95 transition-all cursor-pointer"
            >
              {" "}
              {/* [Funcionamento]: Botão largo High-Density com efeito de clique mola elástico (active:scale-95). */}
              <Plus size={20} strokeWidth={3} /> Novo Ensaio{" "}
              {/* Desenha o sinal de mais grosso destacado ao lado do texto. */}
            </button>{" "}
            {/* Encerra o botão. */}
          </div> // Encerra o bloco.
        )}
      ...
      <EventModals // [Funcionamento]: Invoca as janelas modais injetando os estados de formulários e os salvamentos.
        showModal={showModal}
        setShowModal={setShowModal} // Passa o controle de abertura do formulário de novo ensaio.
        newEventDate={newEventDate}
        setNewEventDate={setNewEventDate} // Passa o calendário reativo.
        responsavel={responsavel}
        setResponsavel={setResponsavel} // Passa o input do Ancião de atendimento.
        handleCreate={handleCreate} // Entrega a subrotina que salva o ensaio na nuvem.
        showConfigError={showConfigError}
        setShowConfigError={setShowConfigError} // Passa o controle da janela de erro de grade instrumental.
        isGemLocal={isGemLocal}
        onNavigateToSettings={onNavigateToSettings} // Repassa os atalhos de portaria.
        eventToDelete={eventToDelete}
        setEventToDelete={setEventToDelete} // Passa o controle da lixeira de confirmação.
