...
import {
  ...
  Clock,
  Map,
} from "lucide-react"; // [Funcionamento]: Importa o pacote de vetores visuais modernos do Lucide.
import {
  db,
  collection,
  query,
  where,
  onSnapshot,
} from "../../../../shared/api/firebase"; // [Funcionamento]: Importa os conectores de escuta em tempo real do banco Firebase.
...
const EventModals = ({
  ...
  const optionsScope = useMemo(() => {
    ...
    if (isGemLocal || isRegionalCidade) {
      // [Funcionamento]: GEM Local ou administradores de comarca podem abrir relatórios locais.
      options.push({ id: "local", name: "Ensaio Local (Comum)" }); // [Funcionamento]: Injeta a opção de ensaio de igreja comum.
    }
    if (isRegionalCidade) {
      // [Funcionamento]: Somente autoridade de nível de cidade para cima abre pílula regional.
      options.push({
        id: "regional",
        name: "Ensaio Regional (Cidade/Regional)",
      }); // [Funcionamento]: Injeta a opção regional abrangente.
    }
  ...
  useEffect(() => {
    if (showModal && userData?.uid) {
      // [Funcionamento]: Se a janela estiver aberta e o login do usuário for válido.
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      // [Funcionamento]: Abre o canal de escuta contínuo e reativo na nuvem.
  ...
  const onConfirmAction = () => {
    // [Funcionamento]: Função disparada ao clicar no botão de confirmar agendamento de ensaio.
    if (invitedUsers.length > 0) {
      // [Funcionamento]: Se o lote de convites regionais possuir registros selecionados.
      ...
      localStorage.setItem(
        `recent_guests_${userData?.uid}`,
        JSON.stringify(updatedRecents),
      ); // [Funcionamento]: Salva na memória do chip do celular.
    }

    const invitedIds =
      scope === "regional" ? invitedUsers.map((u) => u.uid) : []; // [Funcionamento]: Converte a malha de objetos em array simples de strings para as Security Rules lerem.
    ...
    handleCreate({
      // [Funcionamento]: Dispara o salvamento delegando a montagem dos metadados geográficos para a página orquestradora.
        ...
        {showModal && (
          ...
          <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-md z-[100] flex items-start justify-center p-4 overflow-y-auto no-scrollbar text-left pt-12">
            ...
            >
              ...
              <button
                onClick={closeModal}
                className="absolute top-6 right-6 p-2 bg-slate-100 rounded-full text-slate-400 active:scale-90 transition-all cursor-pointer outline-none"
              >
                <X size={18} />
              </button>

              <h3 className="text-xl font-[900] uppercase italic text-slate-950 mb-5 shrink-0 leading-none tracking-tighter select-none">
                Novo Registro
              </h3>
              ...
              <div className="space-y-4 overflow-y-auto no-scrollbar pr-1 flex-1 min-h-0">
                ...
                <div className="space-y-1.5">
                  <label className="text-[8px] font-black text-slate-400 uppercase ml-2 tracking-widest italic select-none">
                    Data Agendada
                  </label>
                  <input
                    type="date"
                  ...
                  {scope === "regional" && (
                    ...
                    >
                      <div className="flex justify-between items-center ml-2 select-none">
                        ...
                        <span className="text-[7px] font-bold text-blue-500 bg-blue-50 px-2 py-0.5 rounded-full uppercase">
                          Filtro Amplo
                        </span>
                      </div>
                      ...
                      <div className="space-y-2 max-h-36 overflow-y-auto no-scrollbar">
                            ...
                            {recentUsers.map((u) => (
                              ...
                              >
                                <div className="text-left leading-none">
                                  ...
                                  <p className="text-[6px] font-bold uppercase mt-1 opacity-60">
                                    {u.comum}
                                  </p>
                                </div>
                                {invitedUsers.find((i) => i.uid === u.uid) && (
                        ...
                        {filteredUsers.length === 0 && searchTerm !== "" ? (
                          <p className="text-[9px] font-bold text-slate-300 uppercase italic p-4 text-center select-none">
                            ...
                            >
                              <div className="text-left leading-none">
                                ...
                                <div className="flex items-center gap-1 mt-1">
                                  ...
                                  />
                                  <p
                                    className={`text-[7px] font-bold uppercase ${invitedUsers.find((i) => i.uid === user.uid) ? "text-blue-400" : "text-slate-300"}`}
                                  >
                                    {user.cidade} • {user.comum}
                                  </p>
                                </div>
                              </div>
                              {invitedUsers.find((i) => i.uid === user.uid) && (
                                <Check size={14} className="text-amber-500" />
                              )}
                            </button>
                          ))
        ...
        {showConfigError && (
          ...
          <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-[200] flex items-start justify-center p-4 overflow-y-auto no-scrollbar text-center pt-20">
            ...
            >
              ...
              <h3 className="text-xl font-[900] text-slate-950 uppercase italic mb-3 tracking-tighter leading-none select-none">
                Orquestra Ausente
              </h3>
              <p className="text-[11px] font-bold text-slate-400 uppercase leading-relaxed mb-6 select-none">
                Esta localidade ainda não possui uma orquestra configurada. É
                necessário definir os instrumentos antes de agendar ensaios.
              </p>
              ...
              <div className="space-y-3 shrink-0">
                ...
                <button
                  ...
                  className="w-full py-2.5 text-slate-300 font-black uppercase text-[9px] tracking-widest cursor-pointer outline-none"
                >
                  Entendido
                </button>
              </div>
        ...
        {eventToDelete && (
          ...
          <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-[110] flex items-start justify-center p-4 overflow-y-auto no-scrollbar text-center pt-24">
            ...
            >
              ...
              <h3 className="text-lg font-[900] uppercase italic text-slate-950 tracking-tighter leading-tight select-none">
                Remover Agenda?
              </h3>
              <p className="text-[10px] font-bold text-slate-400 uppercase mt-3 mb-5 leading-relaxed select-none">
                Todos os dados e contagens deste ensaio serão permanentemente
                excluídos da jurisdição.
              </p>
