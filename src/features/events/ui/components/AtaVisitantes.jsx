...
import {
  ...
  Pencil,
  Music,
} from "lucide-react"; // Importa os ícones visuais incluindo o novo ícone de Lápis
import { toTitleCase } from "../../../../shared/api/pdfEventService"; // Reaproveitando lógica de formatação
...
const AtaVisitantes = ({
  ...
  return (
    <div className="space-y-4">
      {" "}
      {/* Define o espaçamento vertical entre os elementos */}
      ...
      <div className="space-y-3">
        {" "}
        {/* Espaçamento entre os cartões de visitantes */}
        {(visitantes || []).length === 0 ? ( // Verifica se a lista está vazia
          ...
          visitantes.map(
            (
              v,
              idx, // Percorre a lista de visitantes cadastrados
            ) => (
              <div
                key={v.id || idx}
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      ...
                      {v.dataEnsaio && (
                        <span className="text-[7px] font-black bg-blue-50 px-2 py-1 rounded-lg text-blue-400 flex items-center gap-1 uppercase border border-blue-100/50">
                          ...
                          {v.dataEnsaio} {/* Exibe data do ensaio */}
                          {v.hora && (
                            <>
