...
const GuestManager = ({ eventId, invitedUsers, userData, isClosed }) => {
  ...
  const guestsInfo = useMemo(() => {
    ...
    return invitedUsers.map((guest) => {
      ...
      if (found)
        return { uid: found.uid, name: found.name, comum: found.comum };
  ...
  const checkAlreadyInvited = (userId) => {
    ...
    return invitedUsers.some(
      (i) => (typeof i === "object" ? i.uid : i) === userId,
    );
  };
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="leading-tight">
                  <h4 className="text-[10px] font-black text-slate-950 uppercase italic leading-none">
                    {guest.name}
                  </h4>
                  <p className="text-[7px] font-bold text-blue-600 uppercase tracking-widest mt-1">
                    {guest.comum}
                  </p>
                </div>
              </div>
              {!isClosed && (
                <button
                  ...
                  className="p-2 text-red-400 hover:bg-red-50 rounded-lg transition-colors active:scale-90"
                >
                  <UserMinus size={16} />
                </button>
              )}
            ...
            >
              <div className="flex justify-between items-center mb-6">
                ...
                <button
                  ...
                  className="p-2 bg-white rounded-full text-slate-300 shadow-sm active:scale-90 transition-all"
                >
                  <X size={20} />
                </button>
              </div>
                  ...
                  <div className="mb-4">
                    ...
                    {recentUsers.map((u) => {
                      ...
                      return (
                        <div
                          key={`recent-${u.uid}`}
                          className="bg-blue-50/50 p-4 rounded-2xl border border-blue-100 flex justify-between items-center shadow-sm mb-2"
                        >
                          <div className="flex items-center gap-2">
                            <Clock size={12} className="text-blue-400" />
                            <div className="text-left leading-none">
                              <h4 className="text-[10px] font-black text-slate-950 uppercase leading-none">
                                {u.name}
                              </h4>
                              <p className="text-[7px] font-bold text-slate-400 uppercase italic mt-1">
                                {u.comum}
                              </p>
                            </div>
                          </div>
                    ...
                    return (
                      ...
                      >
                        <div className="text-left leading-none">
                          ...
                          <p className="text-[7px] font-bold text-slate-400 uppercase italic mt-1 tracking-tighter">
                            {u.comum}
                          </p>
                        </div>
                        <button
