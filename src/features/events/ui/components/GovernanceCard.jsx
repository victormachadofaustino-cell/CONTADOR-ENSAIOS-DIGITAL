...
/**
 * GovernanceCard v1.3 - PADRONIZAÇÃO FINAL DE UI
 * v1.3 - Ajuste fino de preenchimento para paridade total com InstrumentCard 7XL.
 */
const GovernanceCard = ({ type, val, onChange, userData, isMyTurn, isOtherTurn, responsibleName, onToggle, disabled }) => {
  ...
  return (
    <motion.div 
      ...
      className={`p-5 rounded-[2.5rem] border transition-all relative overflow-hidden mb-3 ${
        canEdit ? 'bg-white border-slate-200 shadow-xl scale-[1.01]' : 'bg-slate-50 border-slate-100 opacity-80'
      }`}
    >
      {/* BOTÃO DE POSSE INDIVIDUAL - Padrão Unificado */}
      {!disabled && (
      ...
      {/* CONTROLES DE QUANTIDADE - AJUSTE DE PREENCHIMENTO 7XL */}
      <div className={`h-32 flex items-center justify-between bg-slate-50 border border-slate-100 rounded-[2rem] relative shadow-inner ${!canEdit ? 'grayscale pointer-events-none opacity-50' : ''}`}>
        <button 
          ...
          className="w-14 h-full flex items-center justify-center text-slate-300 active:text-red-500 transition-colors"
        >
          <Minus size={24} strokeWidth={4}/>
        </button>
        
        <div className="flex-1 flex flex-col items-center justify-center pointer-events-none h-full">
          <span className="absolute top-2.5 text-[7px] font-black text-slate-400 uppercase tracking-[0.2em] italic leading-none">Total Presente</span>
          <span className="font-[900] text-slate-950 italic text-7xl tracking-tighter leading-none select-none mt-2">
            {val}
          </span>
        </div>
        ...
        <button 
          ...
          className="w-14 h-full flex items-center justify-center text-slate-950 active:text-blue-600 transition-colors"
        >
