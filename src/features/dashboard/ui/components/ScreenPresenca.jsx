...
const ScreenPresenca = ({ stats }) => {
  // [Funcionamento]: Recebe as estatísticas unificadas e reativas do Maestro.

  // 🧮 COMPILADOR DE BI DE PRESENÇA (PREPARA, SEPARA E ORDENA A HIERARQUIA EM MEMÓRIA LOCAL)
  const estruturaHierarquica = useMemo(() => {
    // [Funcionamento]: Monta a árvore visual dividida em Naipes, Instrumentos e Músicos ativos.
    // 💡 1. Definição das Gavetas das Famílias Oficiais (Estrutura Mestre Fixa)
    const familias = {
      // [Funcionamento]: Cria os compartimentos vazios para as 6 categorias litúrgicas oficiais.
    ...
    if (
      ...
      !Array.isArray(stats.musicosPresentesLista)
    ) {
      // [Funcionamento]: Trava protetora caso a lista venha nula ou vazia.
      return []; // [Funcionamento]: Retorna uma lista vazia se não houver dados.
    } // [Funcionamento]: Fecha a barreira de segurança.

    // 🔍 2. ALGORITMO DE TRIAGEM INTELIGENTE: Prioriza a 'section' e usa a lógica antiga como fallback.
    stats.musicosPresentesLista.forEach((membro) => {
      // [Funcionamento]: Passa uma varredura em cada um dos músicos listados no ensaio.
      ...
      const instNome = (membro.instrumentoNome || "SOLISTA")
        .toUpperCase()
        .trim(); // [Funcionamento]: Captura o nome próprio do instrumento em letras maiúsculas.
      let familiaAlvo = ""; // [Funcionamento]: Inicia a string vazia para validação rigorosa.

      // 🚀 CORREÇÃO DE AGRUPAMENTO: Prioriza a família ('section') vinda dos dados do evento.
      // A lógica antiga de "adivinhação" é mantida como um fallback de segurança para dados mais antigos.
      if (membro.section && familias[membro.section.toUpperCase()]) {
        familiaAlvo = membro.section.toUpperCase();
      } else {
        // 🎯 DICIONÁRIO DE PRECEDÊNCIA ESTÁTICA (FALLBACK)
        ...
        // 🎺 Grupo 4: Metais
          ...
          instNome.includes("FLUGEL") ||
          instNome.includes("CORNET")
        ) {
          familiaAlvo = "METAIS";
        }
        // 🥖 Grupo 5: Madeiras
          ...
          familiaAlvo = "MADEIRAS";
        }
        // 🎻 Grupo 6: Cordas (Fallback)
        else {
          // Fallback de contingência: Se for um instrumento exótico não mapeado, joga em Cordas para não sumir do mapa
          familiaAlvo = "CORDAS";
        }
      }
      ...
      if (!familias[familiaAlvo].instrumentos[instNome]) {
        // [Funcionamento]: Checa se o instrumento já possui um array próprio.
    ...
    Object.keys(familias).forEach((chaveFamilia) => {
      // [Funcionamento]: Varre as 6 famílias estruturadas na RAM.
      ...
      if (listaInstrumentos.length > 0) {
        // [Funcionamento]: Valida se o naipe possui integrantes presentes.
        ...
        listaInstrumentos.forEach((nomeInstrumento) => {
          // [Funcionamento]: Varre cada instrumento pertencente à família instrumental.
          ...
          musicosDesteInst.sort((a, b) => {
            // [Funcionamento]: Inicia o desempate alfabético dos nomes.
          ...
          instrumentosOrdenados.push({
            // [Funcionamento]: Salva os músicos organizados sob o rótulo do seu respectivo instrumento.
            ...
            ordemHierarquica:
              musicosDesteInst[0]?.instrumentoOrdem ??
              musicosDesteInst[0]?.ordem ??
              musicosDesteInst[0]?.peso ??
              999,
        ...
        instrumentosOrdenados.sort((a, b) => {
          // [Funcionamento]: Método de ordenação inteligente por peso de BI.
          if (a.ordemHierarquica !== b.ordemHierarquica) {
            // [Funcionamento]: Se os pesos configurados no painel de ajustes forem diferentes.
        ...
        resultadoFormatado.push({
          // [Funcionamento]: Consolida o naipe completo estruturado na árvore final.
  ...
  const totalPresentes =
    stats.musicosPresentesLista?.filter((m) => m.presente === true).length || 0; // [Funcionamento]: Filtra os booleanos verdadeiros computando o volume total de cabeças confirmadas.

  return (
    // [Funcionamento]: Palco visual mobile-first do componente.
    <div className="space-y-4 animate-fadeIn text-left w-full">
      ...
      {estruturaHierarquica.length > 0 ? (
        estruturaHierarquica.map((familia) => (
          <div
            key={familia.id}
            className="space-y-3 bg-slate-100/40 p-3 rounded-[2rem] border border-slate-200/40 animate-fadeIn"
          >
            {/* 🏷️ CAMADA 1: Título de Família (Naipe Geral) */}
            <div className="flex items-center gap-2 px-1 select-none">
              <span className="text-[10px] font-black uppercase text-slate-900 tracking-wider italic">
                {familia.titulo}
              </span>
                  ...
                  <div className="space-y-1">
                    {instrumento.musicos.map((membro, idxMusico) => (
                      ...
                      >
                        ...
                        <CheckCircle2
                          size={14}
                          className="text-emerald-500 shrink-0 select-none ml-2"
                        />
                      </div>
                    ))}
                  </div>
        ...
        <div className="bg-white p-8 rounded-[2rem] border border-slate-200/70 text-center flex flex-col items-center justify-center min-h-[140px]">
          <span className="text-slate-400 font-bold text-xs normal-case max-w-[240px] block leading-relaxed">
            Nenhum músico local foi confirmed ou marcado as presente nesta ata
            até o momento.
          </span>
        </div>
      )}
