...
export const useDashAnalytics = (
  events,
  filterType,
  selectedYear,
  subFilter,
) => {
  // [Funcionamento]: Inicia o gancho customizado de matemática do painel recebendo os eventos e filtros da tela.
  ...
  const mesesRef = [
    ...
    "Nov",
    "Dez",
  ]; // [Funcionamento]: Cria uma lista de etiquetas abreviadas dos meses para alinhar as barras do gráfico.

  const processedData = useMemo(() => {
    // [Funcionamento]: Ativa a trava de memória useMemo para que o processamento só rode quando uma option de filtro mudar na tela.
    const legacyMap = {
      // [Funcionamento]: Cria uma tabela de tradução para converter siglas antigas de instrumentos em nomes oficiais limpos.
      vln: "violino",
      vla: "viola",
      vcl: "violoncelo", // [Funcionamento]: Traduz as abreviações de cordas do passado para o singular minúsculo.
      flt: "flauta",
      clt: "clarinete",
      acd: "acordeon",
      org: "orgao", // [Funcionamento]: Traduz as abreviações de sopros e teclas do passado para o padrão atual.
    };

    const filtered = events.filter((ev) => {
      // [Funcionamento]: Inicia a varredura para reter na memória apenas os ensaios que batem com o tempo selecionado.
      ...
      if (filterType === "semester")
        return subFilter === "0" ? mIdx < 6 : mIdx >= 6; // [Funcionamento]: Filtro Semestral: Separa os ensaios entre os primeiros 6 meses ou os últimos 6 meses.
      if (filterType === "quarter")
        return Math.floor(mIdx / 3) === parseInt(subFilter); // [Funcionamento]: Filtro Trimestral: Divide os meses em blocos de 3 para agrupar as estações do ano.
    ...
    filtered.forEach((ev) => {
      // [Funcionamento]: Abre um laço de repetição para processar cada ensaio que passou pelo filtro temporal.
      // 🚀 INJEÇÃO DE CÁLCULO ISOLADO POR EVENTO: Cria totalizadores locais para aplicar as regras de negócio antes de somar nos globais.
      let musicosLocaisDoEvento = 0;
      let organistasTotaisDoEvento = 0;
      let irmandadeTotalDoEvento = 0;
      ...
      if (!groups[key]) {
        // [Funcionamento]: If o mês ainda não foi preparado na tabela de acúmulo, inicializa a ficha dele zerada.
        groups[key] = {
          // [Funcionamento]: Define a estrutura interna de propriedades que os gráficos do Recharts precisam ler.
      ...
      Object.entries(ev.counts || {}).forEach(([id, data]) => {
        // [Funcionamento]: Abre o mapa fatiado de contagens de instrumentos gravado dentro do documento do Firestore.
        ...
        if (
          ...
          // [Funcionamento]: Identifica if a linha processada pertence ao grupo coral da irmandade sentada.
          ...
          irmandadeTotalDoEvento += irmaosCount + irmasCount;
        } else if (
          sec === "ORGANISTAS" ||
          saneId === "orgao" ||
          saneId === "org"
        ) {
          // [Funcionamento]: Identifica if a linha processada pertence à categoria das organistas da bancada.
          ...
          organistasTotaisDoEvento += tot;
        } else {
          // [Funcionamento]: Entra na ramificação da Orquestra de sopros, cordas e foles solistas.
          ...
          musicosLocaisDoEvento += tot; // CORREÇÃO: O card "Músicos" agora considera o total (comum + visitas).

          if (
            sec === "CORDAS" ||
            ["violino", "viola", "violoncelo", "contrabaixo"].includes(saneId)
          ) {
            // [Funcionamento]: Classifica o instrumento dentro do naipe de cordas da orquestra.
          ...
          // 🚀 ALTERAÇÃO SOLICITADA (RADAR ISOLADO DE SAXOFONES): Separa os Saxofones de forma independente antes que caiam no bolo de madeiras!
            ...
            sec.includes("SAX") ||
            ["saxalto", "saxtenor", "saxsoprano", "saxbaritono"].includes(
              saneId,
            ) ||
            saneId.startsWith("sax")
          ) {
            // [Funcionamento]: Identifica se é do naipe exclusivo de palhetas cônicas de metal.
            ...
            // [Funcionamento]: Classifica o instrumento dentro do naipe de madeiras de sopro tradicionais.
            ...
            // [Funcionamento]: Classifica o instrumento dentro do naipe de metais pesados de bocal.
            ...
            // [Funcionamento]: Classifica os teclistas e acordeonistas dentro da sua seção correspondente.
      ...
      if (ev.ata?.deduzirOrganistas) {
        irmandadeTotalDoEvento = Math.max(
          0,
          irmandadeTotalDoEvento - organistasTotaisDoEvento,
        );
      }

      // Consolida os totais do evento nos totalizadores globais
      tM += musicosLocaisDoEvento;
      tO += organistasTotaisDoEvento;
      tI += irmandadeTotalDoEvento;

      ev.ata?.partes?.forEach((p, idx) => {
        // [Funcionamento]: Varre o array de blocos e partes liturgicas preenchidas na ata da secretaria.
        p.hinos?.forEach((h) => {
          // [Funcionamento]: Passa pente fino na sub-lista de hinos coletados de cada parte.
          if (h && h.trim() !== "") {
            // [Funcionamento]: Verifica if o slot do hino possui um número preenchido e limpa espaços vazios.
            ...
            if (idx === 0) g.h1++;
            else if (idx === 1) g.h2++; // [Funcionamento]: Classifica if o hino foi entoado na abertura (1ª parte) ou no encerramento (2ª parte).
          }
        });
      });

      ev.ata?.visitantes?.forEach((v) => {
        // [Funcionamento]: Abre o array de registros nominais assinados no livro de visitantes da ata.
        ...
        cityMap[(v.cidadeUf || "N/I").toUpperCase().trim()] =
          (cityMap[(v.cidadeUf || "N/I").toUpperCase().trim()] || 0) + 1; // [Funcionamento]: Contabiliza a cidade e o estado de origem do visitante na tabela geográfica.
        bairroMap[(v.bairro || "N/I").toUpperCase().trim()] =
          (bairroMap[(v.bairro || "N/I").toUpperCase().trim()] || 0) + 1; // [Funcionamento]: Contabiliza o bairro de origem do visitante para o carrossel de mapas.

        // AJUSTE 2: DEDUÇÃO DO MINISTÉRIO QUE TOCA
        if (cargo.includes("ENCARREGADO") && v.tocando !== true) {
          tEnc++;
          g.condutoresVisitas++; // [Funcionamento]: Alimenta especificamente a gaveta de condutores visitantes do mês corrente.
        }

        nominal.push({
          // [Funcionamento]: Enfileira o objeto do visitante estruturado com a data para alimentar o histórico de listagem.
      ...
      ev.ata?.presencaLocalFull?.forEach((p) => {
        // [Funcionamento]: Abre a sub-lista de presenças fixas salvas da irmandade do corpo local.
        // AJUSTE 2: DEDUÇÃO DO MINISTÉRIO QUE TOCA
        if (
          (p.role || "").toUpperCase().includes("ENCARREGADO") &&
          p.tocando !== true
        ) {
          tEnc++;
    ...
    const chartArray = Object.values(groups) // [Funcionamento]: Transforma o mapa mensal indexado em um vetor (array) linear legível pelo Recharts.
      ...
      .map((g) => {
        ...
        const totalOrquestraDoMes =
          g.comumTotalAcumulado + g.visitaTotalAcumulada;
        ...
        const publicoGeralEstimado =
          totalOrquestraDoMes + totalCoralDoMes + totalOrganistasDoMes;
        ...
        return {
          ...
          público:
            publicoGeralEstimado > 0
              ? publicoGeralEstimado
              : Math.round(totalOrquestraDoMes * 1.8), // [Funcionamento]: Injeta o gomo de público estimado ou aplica proporção estatística segura.
        };
      });

    const uniqueCities = [
      ...new Set(nominal.map((v) => (v.cidadeUf || "").toUpperCase().trim())),
    ].sort(); // [Funcionamento]: Extrai as opções de cidades únicas eliminando duplicados para alimentar os menus de busca.
    const uniqueMins = [
      ...new Set(nominal.map((v) => (v.min || "").toUpperCase().trim())),
    ].sort(); // [Funcionamento]: Extrai as opções de cargos únicos sem repetição para os seletores de portaria.

    return {
      // [Funcionamento]: Retorna o pacote consolidado de resultados calculados para fechar o useMemo.
      ...
      topHinos: Object.entries(hinosMap)
        .map(([num, count]) => ({ num, count }))
        .sort((a, b) => b.count - a.count), // [Funcionamento]: Transforma o mapa de hinos em lista ordenada do mais tocado para o menos tocado.
      origemVisitas: {
        // [Funcionamento]: Entrega as fatias prontas para os gráficos de pizza e rankings de geolocalização.
        cargos: Object.entries(minMap)
          .map(([name, value]) => ({ name, value }))
          .sort((a, b) => b.value - a.value), // [Funcionamento]: Lista ranqueada de ministérios visitantes.
        cidades: Object.entries(cityMap)
          .map(([name, value]) => ({ name, value }))
          .sort((a, b) => b.value - a.value), // [Funcionamento]: Lista ranqueada de cidades com maior índice de visitas.
        bairros: Object.entries(bairroMap)
          .map(([name, value]) => ({ name, value }))
          .sort((a, b) => b.value - a.value), // [Funcionamento]: Lista ranqueada de bairros com maior índice de visitas.
      },
      nominalFinal: nominal.sort((a, b) =>
        b.eventDate.localeCompare(a.eventDate),
      ), // [Funcionamento]: Lista nominal completa ordenada dos ensaios mais recentes para os mais antigos.
