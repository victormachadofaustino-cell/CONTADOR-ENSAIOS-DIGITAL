...
import {
  ...
  deleteDoc,
  updateDoc,
} from "../../../shared/api/firebase"; // Explicação: CORREÇÃO: Ajustado caminho para voltar 2 níveis e achar o Firebase de forma limpa.
import {
  Shield,
  Lock,
  Plus,
  ...
  Music,
  ChevronDown,
} from "lucide-react"; // Explicação: Importa o pacote de ícones modernos e visuais para ilustrar os botões e menus do aplicativo.
import { useAuth } from "../../../app/providers/AuthContext"; // Explicação: CORREÇÃO: Ajustado caminho para voltar 2 níveis e ler o Crachá Eletrônico.
...
const ModuleOrchestraBody = ({ comumId, instrumentsData = [] }) => {
  // Explicação: Inicia o componente do Corpo Orquestral recebendo a ID da igreja e os instrumentos que ela ativou.
  ...
  const [newMusico, setNewMin] = useState({
    nome: "",
    instrumentoId: "",
    situacao: "",
  }); // Explicação: Memória temporária para o formulário de cadastro de um novo músico.
  ...
  const temPoderEdicao = useMemo(() => {
    // Explicação: Valida de forma soberana se o usuário tem o direito legal de alterar os músicos desta comum.
  ...
  const ordemSessoesDefinidas = [
    ...
    "IRMANDADE",
    "GERAL",
  ]; // Explicação: Vetor fixo de referência para ordenar as divisórias de naipes no menu suspenso.

  // Mapeia e organiza os instrumentos ativos por NAIPES no Dropdown removendo o CORAL
  const instrumentosAgrupados = useMemo(() => {
    // Explicação: Filtra os instrumentos ativos, expulsa o Coral e monta um dicionário dividido por famílias ordenadas.
    const filtrados = [...instrumentsData].filter(
      (i) => i.active && i.name?.toUpperCase() !== "CORAL",
    );
  ...
  const instrumentosDisponiveisPlano = useMemo(() => {
    // Explicação: Cria uma lista reta para agilizar buscas rápidas de IDs na hora de salvar o formulário.
  ...
  useEffect(() => {
    // Explicação: Escuta em tempo real os músicos desta comum apenas quando este modal está aberto na tela.
    if (!comumId) return;
    setLoading(true);
    ...
    const unsubMusicos = onSnapshot(
      subColecaoRef,
      (snapshot) => {
        // Explicação: Sincroniza adições, edições e remoções de forma instantânea.
  ...
  const handleInsertMusico = async () => {
    // Explicação: Injeta um novo músico na base de dados da igreja atual.
    if (
      !newMusico.nome.trim() ||
      !newMusico.instrumentoId ||
      !newMusico.situacao
    ) {
      return toast.error(
        "Preencha o nome completo, selecione o instrumento e o nível de teste!",
      );
    }
    if (!temPoderEdicao)
      return toast.error("Seu crachá não autoriza gravações nesta localidade.");

    // Descobre o nome do instrumento para carimbagem (Denormalização Preventiva contra consultas externas)
    const instAlvo = instrumentosDisponiveisPlano.find(
      (i) => i.id === newMusico.instrumentoId,
    );
  ...
  const handleSaveEdit = async () => {
    // Explicação: Salva as alterações feitas no registro do músico.
    if (
      !editingMusico?.nome.trim() ||
      !editingMusico?.instrumentoId ||
      !editingMusico?.situacao
    ) {
      return toast.error("Preencha todos os campos da ficha!");
    }
    const instAlvo = instrumentosDisponiveisPlano.find(
      (i) => i.id === editingMusico.instrumentoId,
    );
  ...
  const handleConfirmDelete = async () => {
    // Explicação: Deleta permanentemente o músico da lista nominal.
    if (!deletingMusico || !comumId) return;
    try {
      await deleteDoc(
        doc(db, "comuns", comumId, "musicos_lista", deletingMusico.id),
      );
    ...
    <div className="space-y-4 text-left pb-10 font-sans relative">
      ...
      >
        ...
        <div className="leading-tight text-left">
          ...
          >
            {temPoderEdicao
              ? "Modo de Escrita Liberado"
              : "Acesso Limitado: Apenas Consulta"}
          </p>
          <p className="text-[7px] font-bold text-slate-400 uppercase">
            Estes nomes aparecerão na lista de chamada do ensaio
          </p>
        </div>
      </div>
      ...
      <div className="space-y-2">
        <div className="px-1.5 flex flex-col text-left">
          <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest italic leading-none">
            Corpo Orquestral Alistado ({musicos.length})
          </span>
        </div>
        ...
        {musicos.length === 0 ? (
          <div className="p-8 text-center bg-white rounded-3xl border border-dashed border-slate-200">
            <p className="text-[9px] font-black text-slate-300 uppercase italic tracking-widest">
              Nenhum músico alistado nesta igreja comum.
            </p>
          </div>
        ) : (
          <div className="space-y-2">
              ...
              >
                <div className="leading-tight text-left flex-1 min-w-0 pr-1 whitespace-normal break-words">
                  <div className="flex flex-wrap items-center gap-1.5">
                    <span className="text-[7.5px] font-black text-indigo-600 bg-indigo-50 border border-indigo-100 px-1.5 py-0.5 rounded-md uppercase italic flex items-center gap-1">
                      <Music size={8} /> {m.instrumentoNome || "GERAL"}
                    </span>
                    <span className="text-[7.5px] font-black text-slate-500 bg-slate-50 border border-slate-200 px-1.5 py-0.5 rounded-md uppercase italic">
                      {m.situacao || "OFF"}
                    </span>
                  </div>
                  <p className="text-[11.5px] font-black text-slate-900 uppercase italic mt-1.5 leading-snug whitespace-normal break-words">
              ...
              <div className="space-y-3.5 text-left">
                <div className="flex flex-col gap-1 text-left">
                  <label className="text-[7.5px] font-black text-slate-400 uppercase tracking-wider pl-1">
                    Nome Completo do Músico
                  </label>
                  <input
                    type="text"
                    placeholder="NOME DO IRMÃO POR EXTENSO"
                    ...
                    value={newMusico.nome}
                    onChange={(e) =>
                      setNewMin({ ...newMusico, nome: e.target.value })
                    }
                  />
                </div>
                <div className="flex flex-col gap-1 text-left">
                ...
                <div className="flex flex-col gap-1 text-left">
                  <label className="text-[7.5px] font-black text-slate-400 uppercase tracking-wider pl-1">
                    Nível de Teste Atual
                  </label>
                  <div className="relative">
                    <select
                      className="w-full bg-slate-50 p-3.5 rounded-xl font-black text-slate-950 text-[10px] border border-slate-100 uppercase italic outline-none focus:border-indigo-600 min-h-[44px] appearance-none cursor-pointer"
                      value={newMusico.situacao}
                      onChange={(e) =>
                        setNewMin({ ...newMusico, situacao: e.target.value })
                      }
                    >
                ...
                <div className="flex flex-col gap-1 text-left">
                  ...
                  <div className="relative">
                    <select
                      ...
                      value={editingMusico.instrumentoId}
                      onChange={(e) =>
                        setEditingMusico({
                          ...editingMusico,
                          instrumentoId: e.target.value,
        ...
        {deletingMusico && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => setDeletingMusico(null)}
            ...
            <motion.div
              ...
              className="relative w-full max-w-xs bg-white rounded-[2.2rem] p-8 shadow-2xl text-center border border-slate-100"
            >
              <div className="w-14 h-14 bg-red-50 text-red-500 rounded-full flex items-center justify-center mx-auto mb-4">
                <Trash2 size={22} strokeWidth={2.5} />
              </div>
              <h3 className="text-xs font-black text-slate-950 uppercase italic leading-tight mb-2">
                Remover do Corpo Orquestral?
              </h3>
              <p className="text-[9.5px] font-bold text-slate-400 uppercase leading-relaxed mb-6 whitespace-normal px-1">
                ...
                da lista de chamada oficial desta igreja comuns. Esta operação
                limpará o alistamento dele.
              </p>
              <div className="space-y-2">
                <button
