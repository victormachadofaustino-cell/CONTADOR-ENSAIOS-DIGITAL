...
const ModuleGlobal = ({ cargos, ministerios, onConfirmDelete }) => {
  ...
  const isComissao =
    userData?.accessLevel === "master" || userData?.accessLevel === "comissao";
  ...
  const addItemToList = async (tipo, val, set) => {
    ...
    try {
      ...
      set("");
      toast.success(
        `${tipo === "cargo" ? "Cargo" : "Ministério"} adicionado à base`,
      );
    } catch (e) {
  ...
  const handleDelete = (id, nome) => {
    ...
    if (onConfirmDelete) {
      ...
      if (
        window.confirm(
          "Remover da base oficial? Esta ação impactará filtros de todo o sistema.",
        )
      ) {
    ...
    return (
      <div className="p-10 text-center space-y-4">
        ...
        <p className="text-[10px] font-black uppercase text-slate-400 italic">
          Acesso Restrito ao Administrador da Regional
        </p>
      </div>
    );
    ...
    <div className="p-2 space-y-10 text-left animate-in fade-in duration-500">
      ...
      <div className="space-y-4">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() =>
              addItemToList("cargo", newCargoInput, setNewCargoInput)
            }
      ...
      <div className="bg-slate-950 p-5 rounded-[2.2rem] border border-white/5 flex gap-4 items-start shadow-2xl">
        ...
        <div className="text-left">
          <p className="text-[9px] font-black text-blue-400 uppercase tracking-widest italic mb-1">
            Privilégio Administrativo
          </p>
          <p className="text-[8px] font-bold text-slate-400 uppercase leading-relaxed tracking-tighter">
            Os cargos e ministérios editados aqui são referências globais para a
            sua Regional. Alterações refletirão em todos os formulários e
            filtros do sistema.
          </p>
        </div>
      </div>
