import React, { useState, useEffect, useMemo, useRef } from "react"; // Explicação: Importa as ferramentas essenciais do React, incluindo o 'useRef' para criar um escudo temporal.
import { db, doc, onSnapshot, updateDoc } from "../../../shared/api/firebase"; // Explicação: Conecta com os motores de documentos específicos e atualizações atômicas do Firebase.
...
import { useAuth } from "../../../app/providers/AuthContext"; // Explicação: Importa o cérebro de identidade para ler as permissões territoriais do usuário logado.

const ModuleChurch = ({ localData, onUpdate }) => {
  // Explicação: Inicia o componente de Cadastro da Comum recebendo os dados do GPS superior.
  ...
  const temPoderEdicao = useMemo(() => {
    // Explicação: Calcula em tempo real se o usuário logado tem autoridade territorial para mexer nos dados desta igreja.
  ...
  const lastUpdateRef = useRef(0); // Explicação: Escudo temporal que guarda o momento do último clique ou digitação do usuário.
  ...
  useEffect(() => {
    // Explicação: Monitor que escuta qualquer mudança física feita nesta igreja diretamente no banco de dados.
    ...
    const unsubDoc = onSnapshot(doc(db, "comuns", localData.id), (docSnap) => {
      // Explicação: Abre o ouvinte em tempo real no documento desta igreja específica.
      // 🚀 ESCUDO ANTI-SOBRESCRITA: Se o usuário digitou algo nos últimos 3 segundos, o ouvinte não sobrescreve os dados para evitar apagar a digitação.
      const isRecentUpdate = Date.now() - lastUpdateRef.current < 3000;
  ...
  const handleFieldChange = (field, value, isAddress = false) => {
    ...
    lastUpdateRef.current = Date.now(); // Explicação: Ativa o escudo temporal, marcando que o usuário está editando.
    ...
    if (isAddress) {
      ...
      })); // Explicação: Grava na gaveta de endereços.
    } else {
  ...
  const saveToDatabase = async () => {
    // Explicação: Motor de persistência que salva as alterações assim que o usuário tira o foco do campo (onBlur).
    ...
    lastUpdateRef.current = Date.now(); // Explicação: Renova o escudo para garantir a gravação segura.
    try {
      const docRef = doc(db, "comuns", localData.id); // Explicação: Mira cirurgicamente no documento desta comum.
      const updatePayload = {
        // Explicação: Monta o pacote enxuto de dados purificados, livre de qualquer menção a obreiros ou ministério.
  ...
  const toggleDia = async (idx) => {
    // Explicação: Liga ou desliga um dia de culto semanal ao clicar nas pílulas de dias.
    if (!temPoderEdicao) return; // Explicação: Proteção de crachá eletrônico.
    lastUpdateRef.current = Date.now(); // Explicação: Ativa o escudo ao clicar nos dias da semana.
  ...
  if (!localData)
    return (
      <div className="py-10 text-center opacity-30 text-[8px] font-black uppercase tracking-widest italic">
        Selecione uma localidade ativa no topo.
      </div>
    );

  return (
    // Explicação: Inicia o desenho da interface limpa de mercado.
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      ...
      <div
        className={`p-4 rounded-2xl flex items-center gap-3 border ${temPoderEdicao ? "bg-blue-50 border-blue-100" : "bg-amber-50 border-amber-100"}`}
      >
        {temPoderEdicao ? (
        ...
        <div className="leading-tight text-left">
          ...
          >
            {temPoderEdicao
              ? "Zeladoria de Cadastro Liberada"
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2 px-1">
          ...
          <p className="text-[10px] font-black text-slate-950 uppercase italic tracking-widest leading-none">
            Endereço e Localização da Comum
          </p>
        </div>
        <div className="space-y-2">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[7.5px] font-black text-slate-400 uppercase tracking-wider pl-1">
              Rua / Avenida / Logradouro por Extenso
            </label>
            <input
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="col-span-1 flex flex-col gap-1">
              ...
              <input
                disabled={!temPoderEdicao}
                placeholder="Nº"
                className="w-full bg-white p-4 rounded-2xl font-black text-slate-950 text-xs border border-slate-200 uppercase italic outline-none text-center focus:border-indigo-600"
                value={formData.endereco?.numero || ""}
                onChange={(e) =>
                  handleFieldChange("numero", e.target.value, true)
                }
                onBlur={saveToDatabase}
              />
            </div>
            <div className="col-span-2 flex flex-col gap-1">
              <label className="text-[7.5px] font-black text-slate-400 uppercase tracking-wider pl-1">
                Bairro Oficial por Extenso
              </label>
              <input
                disabled={!temPoderEdicao}
                placeholder="BAIRRO"
                className="w-full bg-white p-4 rounded-2xl font-black text-slate-950 text-xs border border-slate-200 uppercase italic outline-none focus:border-indigo-600"
                value={formData.endereco?.bairro || ""}
                onChange={(e) =>
                  handleFieldChange("bairro", e.target.value, true)
                }
                onBlur={saveToDatabase}
              />
            </div>
          ...
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              className="w-full bg-white p-4 rounded-2xl font-black text-slate-950 text-xs border border-slate-200 uppercase italic outline-none focus:border-indigo-600 text-center tracking-widest"
      ...
      <div className="space-y-3.5 pt-1">
        <div className="flex items-center gap-2 px-1">
          ...
          <p className="text-[10px] font-black text-slate-950 uppercase italic tracking-widest leading-none">
            Dias de Reuniões e Cultos Semanais
          </p>
        </div>
        ...
        <div className="grid grid-cols-4 sm:grid-cols-7 gap-1.5">
          {diasSemana.map((d, i) => (
            <button
              ...
              disabled={!temPoderEdicao}
              onClick={() => toggleDia(i)}
              className={`h-9 rounded-xl font-black text-[9px] transition-all tracking-tighter outline-none ${formData.diasSelecao?.includes(i) ? "bg-emerald-600 text-white shadow-xs" : "bg-white text-slate-300 border border-slate-200"}`}
            >
              {d}
            </button>
          ))}
        ...
        <div className="space-y-2">
          <div className="bg-white p-4 rounded-2xl border border-slate-200/60 flex items-center justify-between">
            <div className="flex items-center gap-3 min-w-0 flex-1">
              ...
              <span className="text-[9px] font-black text-slate-500 uppercase italic tracking-tight whitespace-normal">
                Horário Padrão dos Cultos Semanais
              </span>
            </div>
            <input
              disabled={!temPoderEdicao}
              type="time"
              className="bg-transparent font-black text-slate-950 text-xs outline-none text-right shrink-0 ml-2"
          ...
          {formData.diasSelecao?.includes(0) && ( // Explicação: Condicional Inteligente: Se houver culto no Domingo (índice 0), abre o campo exclusivo de horário dominical.
            <div className="bg-emerald-50/50 p-4 rounded-2xl border border-emerald-100 flex items-center justify-between animate-in fade-in zoom-in-95 duration-200">
              <div className="flex items-center gap-3 min-w-0 flex-1">
                ...
                <span className="text-[9px] font-black text-emerald-700 uppercase italic tracking-tight whitespace-normal">
                  Horário Exclusivo do Culto de Domingo
                </span>
              </div>
              <input
                disabled={!temPoderEdicao}
                type="time"
                className="bg-transparent font-black text-emerald-800 text-xs outline-none text-right shrink-0 ml-2"
                value={formData.horaCultoDomingo || "18:30"}
                onChange={(e) =>
                  handleFieldChange("horaCultoDomingo", e.target.value)
                }
                onBlur={saveToDatabase}
              />
            </div>
      ...
      <div className="space-y-3 pt-1">
        <div className="flex items-center gap-2 px-1">
          ...
          <p className="text-[10px] font-black text-slate-950 uppercase italic tracking-widest leading-none">
            Cronograma do Ensaio Local Técnico
          </p>
        </div>
        <div className="grid grid-cols-2 gap-2">
          <div className="flex flex-col gap-1">
            ...
            <input
              disabled={!temPoderEdicao}
              placeholder="EX: 4º SÁBADO"
              className="w-full bg-white p-4 rounded-2xl font-black text-slate-950 text-xs border border-slate-200 uppercase italic outline-none focus:border-indigo-600 text-center"
          ...
          <div className="flex flex-col gap-1">
            ...
            <input
              disabled={!temPoderEdicao}
              type="time"
              className="w-full bg-white p-4 rounded-2xl font-black text-slate-950 text-xs border border-slate-200 outline-none focus:border-indigo-600 text-center"
