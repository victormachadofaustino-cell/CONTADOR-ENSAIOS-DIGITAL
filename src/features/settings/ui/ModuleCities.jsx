...
const ModuleCities = ({ regionalId, onConfirmDelete }) => {
  ...
  const handleUpdate = async (id) => {
    ...
    try {
      await updateDoc(doc(db, "config_cidades", id), {
        nome: editValue.toUpperCase().trim(),
      });
      setEditingId(null);
          ...
          >
            {editingId === c.id ? (
              <div className="flex items-center gap-2 w-full">
                <input
                  autoFocus
                  className="flex-1 bg-slate-50 p-2 rounded-lg text-[10px] font-black outline-none uppercase"
                  value={editValue}
