...
import {
  ...
  query,
  where,
} from "../../../shared/api/firebase"; // Explicação: Conecta com os motores de consulta, lote e documentos específicos do Firebase.
...
import {
  ...
  GripVertical,
  ChevronUp,
} from "lucide-react"; // 🚀 PRESERVAÇÃO & EXPANSÃO: Mantém as importações e adiciona os ícones GripVertical (hambúrguer de arrastar) e ChevronUp.
...
const ModuleOrchestra = ({ comumId, instrumentsData = [] }) => {
  // Explicação: Inicia o componente da Orquestra recebendo os instrumentos ativos injetados pelo pai.
  ...
  const [formData, setFormData] = useState({
    ...
    id: "",
    ordem: 0,
  }); // Explicação: Memória temporária para armazenar os campos digitados no formulário.
  ...
  useEffect(() => {
    // Explicação: Monitor reativo que vigia se existe uma contagem em andamento para congelar a edição.
    ...
    ); // Explicação: Procura se há ensaios com status "open" rodando na nuvem agora.
    const unsub = onSnapshot(q, (snapshot) => {
      // Explicação: Escuta reativa em tempo real para capturar fechamento ou aberturas rápidas de atas.
  ...
  const ordemSessoes = [
    ...
    "IRMANDADE",
    "GERAL",
  ]; // Explicação: Ordem padrão oficial e por extenso exigida para a separação estética das famílias musicais.

  const sectionsFound = useMemo(() => {
    // Explicação: Processa e agrupa quais naipes existem na lista de instrumentos atual fornecida pela nuvem.
    const secoes = [
      ...new Set(instrumentsData.map((i) => i.section?.toUpperCase())),
    ].filter(Boolean); // Explicação: Varre o cadastro de instrumentos coletando as famílias e limpando chaves duplicadas ou nulas.
    return secoes.sort(
      (a, b) =>
        (ordemSessoes.indexOf(a) > -1 ? ordemSessoes.indexOf(a) : 99) -
        (ordemSessoes.indexOf(b) > -1 ? ordemSessoes.indexOf(b) : 99),
    ); // Explicação: Organiza os blocos da tela seguindo rigidamente o peso do vetor oficial.
  ...
  const podeGerenciar = useMemo(() => {
    // Explicação: Mapeamento de Privilégios: Cruza os dados do crachá eletrônico para liberar ou congelar os switches de cliques.
    ...
    if (level === "regional_cidade")
      return userData?.cidadeId === userData?.activeCityId; // Explicação: Administradores de cidade validam se a comarca coincide com a deles.
    if (level === "gem_local") {
      // Explicação: Regra de ouro da comum: Libera os botões de ligar/desligar se for a igreja do chip dele.
  ...
  const handleToggleActive = async (instrument) => {
    // Explicação: Aciona a inversão da chave magnética liga/desliga de ativação do instrumento na comum.
    if (!podeGerenciar)
      return toast.error(
        "Seu crachá não possui privilégios de gestão para esta localidade.",
      ); // Explicação: Trava visual defensiva.
    if (isEnsaioAberto)
      return toast.error(
        "Ação Bloqueada: Existe um ensaio ativo computando dados no momento.",
      ); // Explicação: Trava de integridade de dados.
    ...
    try {
      const docRef = doc(
        ...
        "instrumentos_config",
        instrument.id,
      ); // Explicação: Localiza a vaga física do instrumento na pasta privada da igreja comuns.
      await setDoc(
        docRef,
        {
          id: instrument.id,
        ...
        { merge: true },
      ); // Explicação: Mescla as chaves no banco de dados sem apagar propriedades nativas vizinhas.
  ...
  const handleInsertPattern = async () => {
    // Explicação: Restaura em lote toda a grade oficial de instrumentos da Matriz Nacional CCB.
    if (!comumId || !podeGerenciar)
      return toast.error("Operação não autorizada para o seu nível de acesso."); // Explicação: Trava defensiva de portaria.
    if (isEnsaioAberto)
      return toast.error(
        "Bloqueado: Feche o ensaio em andamento antes de redefinir a grade.",
      ); // Explicação: Trava de barreira operacional.

    if (
      !window.confirm(
        "Deseja aplicar o RESET DE FÁBRICA? Isso substituirá a lista atual pelo Padrão CCB oficial por extenso.",
      )
    )
      return; // Explicação: Caixa de auditoria manual de segurança del navegador.

    const loadingToast = toast.loading(
      "Sincronizando instrumentos com a Matriz Nacional...",
    ); // Explicação: Inicia indicador de carregamento fixo na tela.
    ...
    try {
      ...
      ); // Explicação: Vai até a nuvem capturar o catálogo oficial nacional de fábrica da CCB.
      if (nationalSnap.empty) {
        // Explicação: Proteção se a coleção mestre nacional sumir por acidente do servidor.
      ...
      localSnap.docs.forEach((d) =>
        batch.delete(doc(db, "comuns", comumId, "instrumentos_config", d.id)),
      ); // Explicação: Insere comandos de exclusão de todas as linhas locais na esteira do lote.

      nationalSnap.docs.forEach((docInst, index) => {
        // Explicação: Passa instrumento por instrumento inserindo comandos de escrita limpa na esteira de gravação.
        ...
        batch.set(docRef, {
          // Monta o payload de fábrica ligado por padrão.
          id: docInst.id,
      ...
      toast.success("Grade Padrão CCB Instalada por Extenso!", {
        id: loadingToast,
      }); // Alerta o sucesso.
    } catch (e) {
  ...
  const handleSaveInstrument = async () => {
    // Explicação: Cria um instrumento local customizado ou grava as alterações de nomenclatura feitas pelo gestor.
    if (!formData.name.trim() || !formData.section)
      return toast.error(
        "Preencha o nome por extenso e a família instrumental!",
      ); // Trava de barreira.
    const instrumentId =
      formData.id || formData.name.trim().toLowerCase().replace(/\s+/g, ""); // Explicação: Gera uma ID compacta sem espaços vazios se for item inédito.
    ...
    try {
      ...
      const pesoOrdemCalculado = formData.id
        ? parseInt(formData.ordem) || 0
        : instrumentsData.length * 10 + 10;

      await setDoc(
        docRef,
        {
          // Registra ou updates a ficha cadastral do instrumento na comum.
          id: instrumentId,
  ...
  const handleDeleteCustom = async (id) => {
    // Explicação: Remove permanentemente um instrumento customizado adicionado pelo secretário local.
    if (isEnsaioAberto)
      return toast.error("Bloqueado: Existe uma ata de ensaio aberta."); // Trava de integridade de contagem.
    if (
      !window.confirm("Deseja realmente remover este instrumento da sua comum?")
    )
  ...
  const handleReorderInstrument = async (
    sectionName,
    currentIndex,
    direction,
  ) => {
    ...
    if (isEnsaioAberto)
      return toast.error(
        "Ação Congelada: Existe um ensaio ativo computando dados.",
      ); // Trava sanitária.

    const instDoNaipe = instrumentsData.filter(
      (i) => i.section?.toUpperCase() === sectionName.toUpperCase(),
    ); // Explicação: Separa apenas os irmãos desse grupo.
    const targetIndex =
      direction === "up" ? currentIndex - 1 : currentIndex + 1; // Explicação: Calcula o índice alvo da troca de cadeiras.
    ...
    novaListaEstágio.forEach((inst, index) => {
      // Explicação: Varre o arranjo recalculando as novas cadeiras limpas de 10 em 10.
    ...
    <div className="space-y-4 text-left font-sans">
      ...
      {isEnsaioAberto && (
        <div className="bg-rose-50 border border-rose-100 text-rose-900 p-4 rounded-2xl flex gap-3 items-start animate-premium">
          ...
          <p className="text-[10px] font-bold leading-relaxed uppercase whitespace-normal text-left">
            <span className="font-black">Grade Temporariamente Congelada:</span>{" "}
            Existe uma ata de ensaio aberta recebendo coletas numéricas neste
            momento. Encerre o ensaio para liberar as chaves magnéticas.
          </p>
        </div>
      )}
      ...
      <div className="space-y-2">
        {sectionsFound.map((sectionName) => {
          ...
          const instDoNaipe = instrumentsData.filter(
            (i) => i.section?.toUpperCase() === sectionName,
          );

          return (
            <div
              key={sectionName}
              className="bg-white rounded-2xl border border-slate-200/60 shadow-2xs overflow-hidden"
            >
              <button
                type="button"
                onClick={() =>
                  setOpenSection(isSectionOpen ? null : sectionName)
                }
              ...
              >
                <div className="flex items-center gap-3 text-left min-w-0 flex-1">
                  ...
                  <h4 className="text-[11px] font-black text-slate-800 uppercase tracking-wider italic whitespace-normal break-words leading-none text-left">
                    {sectionName}
                  </h4>
                  <span className="bg-slate-100 text-slate-600 text-[9px] font-black px-2 py-0.5 rounded-full shrink-0">
                    {instDoNaipe.length}
                  </span>
                </div>
                <ChevronDown
                  size={14}
                  className={`text-slate-400 transition-transform duration-300 shrink-0 ml-2 ${isSectionOpen ? "rotate-180" : ""}`}
                />
              </button>
                  ...
                  >
                    <div className="p-3 space-y-2">
                      {instDoNaipe.map(
                        (
                          inst,
                          idx, // Explicação: Adicionado o index 'idx' para alimentar o rastreamento mecânico das setas invisíveis do hambúrguer.
                        ) => (
                          <div
                            key={inst.id}
                            className="bg-white p-3.5 rounded-xl border border-slate-200/50 flex items-center justify-between shadow-3xs gap-3"
                          >
                            ...
                            {podeGerenciar && !isEnsaioAberto && (
                              <div className="flex flex-col items-center justify-center pr-1 border-r border-slate-100/80 gap-1 shrink-0">
                                <button
                                  ...
                                  className={`p-0.5 transition-opacity ${idx === 0 ? "opacity-10 pointer-events-none" : "text-slate-300 active:text-indigo-600"}`}
                                >
                                  <ChevronUp size={12} strokeWidth={4} />
                                </button>
                                <GripVertical
                                  size={14}
                                  className="text-slate-400/70 cursor-grab active:cursor-grabbing select-none"
                                />{" "}
                                {/* 🚀 O ÍCONE DOS TRES RISCOS HORIZONTAIS PEDIDO PELO OPERADOR */}
                                <button
                                  type="button"
                                  disabled={idx === instDoNaipe.length - 1}
                                  onClick={() =>
                                    handleReorderInstrument(
                                      sectionName,
                                      idx,
                                  ...
                                  className={`p-0.5 transition-opacity ${idx === instDoNaipe.length - 1 ? "opacity-10 pointer-events-none" : "text-slate-300 active:text-indigo-600"}`}
                                >
                            ...
                            <div className="min-w-0 flex-1 text-left leading-tight whitespace-normal break-words">
                              <p className="text-[11.5px] font-black text-slate-900 uppercase italic leading-snug whitespace-normal break-words text-left">
                                {inst.name}
                              </p>
                              <p className="text-[8px] font-bold text-slate-400 uppercase tracking-tight mt-1 text-left">
                                Ficha Técnica Exame: {inst.evalType || "Sem"}
                              </p>
                            </div>
                            ...
                            <div className="flex items-center gap-2.5 shrink-0">
                              ...
                              >
                                <div
                                  className={`bg-white w-5 h-5 rounded-full shadow-xs transform transition-transform duration-300 ${inst.active ? "translate-x-5" : "translate-x-0"}`}
                                />
                              </button>
                            </div>
                          </div>
            ...
            >
              <div className="flex justify-between items-center mb-5 border-b border-slate-100 pb-2.5 text-left">
                <h3 className="text-[10px] font-black text-slate-900 uppercase tracking-wider italic text-left">
                  {showModal === "add"
                    ? "Adicionar Instrumento Novo"
                    : "Ajustar Cadastro Ficha"}
                </h3>
                <button
                  onClick={() => setShowModal(null)}
                  className="w-8 h-8 bg-slate-50 rounded-full text-slate-400 hover:text-slate-600 flex items-center justify-center outline-none cursor-pointer"
                >
              ...
              <div className="space-y-4 text-left">
                <div className="flex flex-col gap-1 text-left">
                  <label className="text-[7.5px] font-black text-indigo-600 uppercase tracking-widest pl-0.5 text-left">
                    Nome Instrumental por Extenso
                  </label>
                  <input
                    type="text"
                ...
                <div className="flex flex-col gap-1 text-left">
                  <label className="text-[7.5px] font-black text-indigo-600 uppercase tracking-widest pl-0.5 text-left">
                    Metodologia de Avaliação Coletada
                  </label>
                  <div className="relative">
                    <select
                      className="w-full bg-slate-50 border border-slate-100 p-3.5 rounded-xl text-xs font-black text-slate-900 outline-none min-h-[44px] appearance-none cursor-pointer uppercase italic"
                      ...
                      onChange={(e) =>
                        setFormData((prev) => ({
                          ...prev,
                          evalType: e.target.value,
                        }))
                      }
                    >
                      <option value="Sem">
                        SEM EXAME COLETADO (PRESENÇA NOMINAL)
                      </option>
                      <option value="Encarregado">
                        ENCARREGADO MUSICAL LOCAL
                      </option>
                      <option value="Examinadora">
                        EXAMINADORA DE ÓRGÃO DO GRUPO
                      </option>
                    </select>
                    <ChevronDown
                      size={14}
