...
function App() {
  ...
  // CORREÇÃO MESTRA: O ID efetivo prioriza a seleção ativa, mas faz fallback para o cadastro do usuário.
  // Isso garante que o Regional nunca veja uma página em branco ao entrar.
      ...
      if (u) {
        unsubSnap = onSnapshot(doc(db, 'users', u.uid), (docSnap) => {
          if (docSnap.exists()) {
            ...
            // Inicialização de Contexto para Master e Regional
            if (data.isMaster || data.escopoRegional || data.role === 'Encarregado Regional') {
              ...
              if (savedId) {
                ...
                // Fallback inicial: usa os próprios dados se não houver seleção prévia
                setActiveComumId(data.comumId);
                setActiveComumName(data.comum);
              }
              setActiveRegionalId(localStorage.getItem('activeRegionalId') || data.regionalId);
              setActiveRegionalName(localStorage.getItem('activeRegionalName') || data.regional);
            } else {
              setActiveComumId(data.comumId);
  ...
  if (view === 'loading') return <div className="h-dvh flex items-center justify-center bg-[#F1F5F9] font-black text-slate-950 animate-pulse uppercase">Sincronizando...</div>;
  ...
  return (
    <div className="h-dvh bg-[#F1F5F9] flex flex-col font-sans overflow-hidden relative text-left">
      ...
      {view === 'login' ? (
        ...
        <>
          <Header userData={{ ...userData, comumId: comumIdEfetivo, comum: activeComumName, activeRegionalId, activeRegionalName, isComissao, isRegional }} 
            ...
            <AnimatePresence mode="popLayout" custom={direcao}>
              <motion.div key={lobbyTab} custom={direcao} initial={{ opacity: 0, x: direcao * 100 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: direcao * -100 }} transition={{ type: "spring", stiffness: 300, damping: 30 }} className="w-full h-full overflow-y-auto no-scrollbar pt-4 pb-32 px-4">
                <div className="max-w-md mx-auto">
                  {lobbyTab === 'ensaios' && <EventsPage userData={{ ...userData, comumId: comumIdEfetivo, comum: activeComumName, activeRegionalId, activeRegionalName, regionalId: activeRegionalId }} isAdmin={isAdmin} onSelectEvent={(id) => { setCurrentEventId(id); setView('app'); }} />}
                  ...
                  {lobbyTab === 'config' && <SettingsPage userEmail={user?.email} isMaster={isMaster} userData={{ ...userData, activeRegionalId, activeRegionalName, regionalId: activeRegionalId, isComissao, comumId: comumIdEfetivo, isRegional }} />}
                </div>
              </motion.div>
            </AnimatePresence>
          </main>
        ...
        <CounterPage currentEventId={currentEventId} counts={counts} onBack={() => setView('lobby')} isAdmin={isAdmin} isMaster={isMaster} userData={{ ...userData, comumId: comumIdEfetivo, comum: activeComumName, activeRegionalId, activeRegionalName, regionalId: activeRegionalId, isBasico, isAdmin, isRegional }} allEvents={events} />
      )}
    </div>
  );
}
