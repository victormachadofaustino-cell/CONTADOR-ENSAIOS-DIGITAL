...
const CARGOS_ADMIN = [
  ...
  "Secretario da Música",
]; // Explicação: Lista estática de cargos com competência administrativa de portaria.

function App() {
  // Explicação: Início da construção do componente mestre do aplicativo inteiro.
  const {
    userData: authContextData,
    loading: authContextLoading,
    setContext,
  } = useAuth(); // Explicação: Conecta com o Cérebro Central para extrair o Crachá Eletrônico e o seletor territorial de GPS.
  ...
  const [lobbyTab, setLobbyTab] = useState(
    localStorage.getItem("lastTab") || "ensaios",
  ); // Explicação: Guarda na memória do celular qual aba inferior o usuário parou navegando.
  const [currentEventId, setCurrentEventId] = useState(
    localStorage.getItem("lastEventId") || null,
  ); // Explicação: Guarda a identification do ensaio que está recebendo contagem no momento.
  ...
  const [activeComumId, setActiveComumId] = useState(
    localStorage.getItem("activeComumId") || null,
  ); // Explicação: Memória de backup local para a ID da comum selecionada.
  const [activeComumName, setActiveComumName] = useState(
    localStorage.getItem("activeComumName") || "",
  ); // Explicação: Memória de backup local para o nome da comum selecionada.
  const [activeRegionalId, setActiveRegionalId] = useState(
    localStorage.getItem("activeRegionalId") || null,
  ); // Explicação: Memória de backup local para a ID da regional selecionada.
  const [activeRegionalName, setActiveRegionalName] = useState(
    localStorage.getItem("activeRegionalName") || "Navegar...",
  ); // Explicação: Texto de exibição da regional no topo.
  ...
  const comumIdEfetivo =
    authContextData?.activeComumId || activeComumId || authContextData?.comumId; // Explicação: Descobre a ID da igreja alvo cruzando o GPS com o crachá do usuário.

  const currentEventData = useMemo(() => {
    // Explicação: Memoriza e localiza as informações do ensaio focado para evitar re-renderização.
  ...
  const ORDEM_TABS = useMemo(() => {
    // Explicação: Desenha a barra de abas inferior ocultando ou exibindo o menu de Ajustes baseado no poder do crachá.
    const temAcessoAjustes = isGemLocal; // Explicação: Define a regra lógica de corte de portaria.
    return temAcessoAjustes
      ? ["ensaios", "dash", "config"]
      : ["ensaios", "dash"]; // Explicação: Adiciona ou remove a aba de ajustes no vetor.
  }, [isGemLocal]); // Explicação: Recalcula o menu se mudar o nível de permissão.

  useEffect(() => {
    // Explicação: Salva no navegador qual aba o usuário parou para manter a experiência na reabertura.
  ...
  useEffect(() => {
    // Explicação: Salva a ID do ensaio aberto para blindar a contagem contra fechamentos acidentais do app.
    if (currentEventId) {
      // Explicação: Se houver ensaio em andamento na tela.
      localStorage.setItem("lastEventId", currentEventId); // Explicação: Registra a chave do ensaio no armazenamento local.
    } else {
      // Explicação: Se o ensaio foi fechado com sucesso pelo botão voltar.
  ...
  useEffect(() => {
    // Explicação: Trava de Segurança: Se o nível do usuário mudar e ele perder o acesso, expulsa ele para a aba de ensaios.
    if (authContextData && !ORDEM_TABS.includes(lobbyTab)) {
      // Explicação: Se a aba atual sumiu da nova matriz de permissões.
  ...
  const mudarTab = (novaTab) => {
    // Explicação: Gerencia a troca de abas calculando a direção do deslize della tela para a esquerda ou direita.
  ...
  useEffect(() => {
    // Explicação: Monitor de Sincronização: Amarra os backups locais de estados às variáveis vivas do Cérebro Central.
    if (authContextData?.activeComumId) {
      // Explicação: Se houver uma igreja comum ativa no contexto global.
    ...
    if (authContextData?.activeRegionalId) {
      // Explicação: Se houver uma regional selecionada no contexto global.
  ...
  useEffect(() => {
    // Explicação: Escudo de Rota e Identidade: Ouve as mudanças de autenticação do Firebase de forma ultra enxuta.
    const unsubAuth = onAuthStateChanged(auth, (u) => {
      // Explicação: Abre um canal nativo para receber o token de login do dispositivo.
      if (u && !u.emailVerified) {
        // Explicação: Trava de segurança: Se o e-mail não foi verificado, barra o acesso imediatamente.
      ...
      if (u) {
        // Explicação: Se o usuário estiver legitimamente logado.
        if (!authContextLoading && authContextData) {
          // Explicação: Se as credenciais do crachá terminaram de baixar do contexto.
          const savedView = localStorage.getItem("lastEventId")
            ? "app"
            : "lobby"; // Explicação: Se caiu a conexão no meio de um ensaio, volta direto pro contador.
          setView(
            authContextData.approved || authContextData.accessLevel === "master"
              ? savedView
              : "waiting-approval",
          ); // Explicação: Desvia usuários não aprovados para a tela de espera.
        } // Explicação: Encerra a conferência de dados.
      } else {
        // Explicação: Caso não haja nenhuma conta conectada no aparelho.
  ...
  useEffect(() => {
    // Explicação: Disparado na largada de boot para casar os estados visuais.
    if (!authContextLoading && user && authContextData) {
      // Explicação: Se o crachá voltou e o login está firme.
      const savedView = localStorage.getItem("lastEventId") ? "app" : "lobby"; // Explicação: Verifica se deve reabrir direto no contador.
      setView(
        authContextData.approved || authContextData.accessLevel === "master"
          ? savedView
          : "waiting-approval",
      ); // Explicação: Executa o roteamento.
    } else if (!authContextLoading && !user) {
      // Explicação: Se o contexto terminou sem usuário ativo.
  ...
  useEffect(() => {
    // Explicação: Ouve em tempo real as mudanças geográficas e territoriais do GPS do cabeçalho de forma unificada.
    ...
    const unsubEvents = eventService.subscribeToEvents(
      authContextData,
      (dadosCarregados) => {
  ...
  if (view === "loading" || authContextLoading) {
    // Explicação: Renderiza o visual de carregamento premium de jurisdição.
    return (
      // Explicação: Desenha o spinner centralizado.
      <div className="h-dvh flex flex-col items-center justify-center bg-[#F1F5F9] p-4 text-center space-y-4">
        <div className="w-8 h-8 border-4 border-slate-200 border-t-indigo-600 rounded-full animate-spin" />{" "}
        {/* Explicação: Círculo animado com rotação contínua Tailwind CSS. */}
        <p className="font-black text-slate-950 uppercase text-[10px] tracking-widest italic">
          Sincronizando Jurisdição...
        </p>{" "}
        {/* Explicação: Texto de rodapé de carregamento em caixa alta micro. */}
      </div>
    ); // Explicação: Fim della renderização do carregamento.
  } // Explicação: Fim della trava de bootstrap de rede.

  return (
    // Explicação: Inicia a renderização estrutural do layout do aplicativo.
    <div className="h-dvh bg-[#F1F5F9] flex flex-col font-sans overflow-hidden relative text-left">
      {" "}
      {/* Explicação: Container raiz do aplicativo travado na altura do dispositivo móvel. */}
      <Toaster position="top-center" />{" "}
      {/* Explicação: Posiciona o contêiner centralizador de alertas e toas flutuantes. */}
      <AnimatePresence>
        {showSplash && <CapaEntrada aoEntrar={() => setShowSplash(false)} />}
      </AnimatePresence>{" "}
      {/* Explicação: Controla a exibição e saída animada da cortina de Splash Screen. */}
      {view === "login" || view === "waiting-approval" ? ( // Explicação: Se o usuário não estiver logado ou aprovado, barra e exibe a tela de credenciais.
        <LoginPage
          ...
          userData={authContextData}
        /> // Explicação: Renderiza o portal eclesiástico de entrada.
        ...
        <>
          ...
          <main className="flex-1 relative overflow-hidden">
            {" "}
            {/* Explicação: Palco central onde as telas das abas inferiores entram e saem. */}
            <AnimatePresence mode="popLayout" custom={direcao}>
              {" "}
              {/* Explicação: Gerenciador de animações sincronizadas de entrada e saída de abas. */}
              <motion.div
                key={lobbyTab}
                custom={direcao}
                ...
                className="w-full h-full overflow-y-auto no-scrollbar pt-4 pb-36 px-4"
              >
                {" "}
                {/* Explicação: Bloco móvel animado com rolagem vertical livre suave. */}
                <div className="max-w-md mx-auto">
                  {" "}
                  {/* Explicação: Limitador ergonômico central de largura para smartphones comuns. */}
                  {lobbyTab === "ensaios" && ( // Explicação: Renderiza a lista de ensaios repassando as permissões unificadas por crachá.
                    <EventsPage // Explicação: Invoca a grade de ordens e cultos.
                      ...
                      onSelectEvent={(id) => {
                        // Explicação: Executado quando o secretário seleciona um ensaio da lista.
                  ...
                  {lobbyTab === "dash" && (
                    <DashPage userData={authContextData} />
                  )}{" "}
                  {/* Explicação: Abre o painel estatístico consolidado histórico della comum. */}
                  {lobbyTab === "config" && <SettingsPage />}{" "}
                  {/* Explicação: Abre o painel de gerenciamento de alistamentos e grade della comum. */}
                </div>
              </motion.div>
            </AnimatePresence>
          </main>
          {!showSplash && <Tickets moduloAtual={lobbyTab} />}{" "}
          {/* Explicação: Exibe o botão de suporte técnico após a saída della splash screen. */}
          <Footer
        ...
        <>
          <CounterPage // Explicação: Invoca a interface reativa mestre do contador digital.
            ...
            onBack={() => {
              // Explicação: Executado quando o usuário clica na seta superior de voltar.
          ...
          {/* Explicação: Carrega o suporte no contador. */}
        </>
      )}
    </div>
  );
