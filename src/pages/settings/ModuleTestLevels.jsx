...
import { testLevelService } from '../../services/testLevelService'; // Explicação: Importa o motor de dados e conexões com o Firestore para níveis de teste.
...
const ModuleTestLevels = ({ regionalId }) => { // Explicação: Inicia a construção da tela visual de gerenciamento de níveis de teste.
  ...
  useEffect(() => { // Explicação: Monitor que abre o canal reativo com o banco assim que a tela abre.
  ...
  const handleCreate = async (e) => { // Explicação: Função acionada ao clicar no botão de salvar o novo nível.
    ...
    try {
      ...
      const proximaOrdem = levels.length > 0 ? Math.max(...levels.map(l => l.order || 0)) + 1 : 1;
      
      const sucesso = await testLevelService.createLevel(newName, newDesc, proximaOrdem); // Explicação: Envia os dados para gravação na coleção do Firestore.
      if (sucesso) {
      ...
      console.error(err);
    } finally { // 🚀 CORREÇÃO CIRÚRGICA: Consertada a palavra com erro de digitação para destravar o compilador do Vite!
      setIsSubmitting(false); // Explicação: Libera o botão para novos cadastros.
    }
  };

  const handleDelete = (levelId, levelNome) => { // Explicação: Acionada ao clicar no botão de lixeira de um card.
      ...
      <div className="flex flex-col gap-4 p-1 min-w-[260px] text-left">
        <div className="flex flex-col gap-1 text-left">
          <div className="flex items-center gap-2 text-red-600">
            ...
            <p className="text-[11px] font-black uppercase tracking-wider">Remover Opção</p>
          </div>
          <p className="text-[10px] text-slate-500 font-semibold leading-relaxed">
            Deseja remover o nível <span className="font-bold text-slate-900 italic">"{levelNome}"</span> da lista suspensa da orquestra?
          </p>
        </div>
        <div className="flex gap-2 justify-end pt-1">
          <button onClick={() => toast.dismiss(t.id)} className="px-3 py-1.5 text-[9px] font-black uppercase tracking-widest text-slate-400">Cancelar</button>
          <button 
            onClick={async () => {
              toast.dismiss(t.id);
    ...
    ), { position: 'top-center', style: { borderRadius: '2rem', padding: '16px' } });
  };

  return ( // Explicação: Desenha a interface visual dentro da cortina flutuante.
    <div className="space-y-5 text-left font-sans">
      
      {/* SEÇÃO 1: FORMULÁRIO DE CADASTRO (VERTICAL STACKING ANTI-ESMAGAMENTO) */}
      <form onSubmit={handleCreate} className="bg-white p-4 rounded-[2rem] border border-slate-200/60 shadow-3xs space-y-3 text-left">
        <div className="pl-1 flex flex-col text-left">
          <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest italic">Novo Nível de Teste</span>
        </div>
      ...
      <div className="space-y-2 text-left">
        <div className="pl-1 flex flex-col text-left">
          <span className="text-[8px] font-black text-indigo-600 uppercase tracking-widest italic">Opções Ativas no Banco de Dados</span>
        </div>

        {levels.length === 0 ? (
          <div className="p-8 text-center bg-white border border-dashed border-slate-200 rounded-[2rem]">
            <p className="text-[9px] font-black text-slate-300 uppercase italic tracking-wider">Nenhum nível cadastrado. Aguardando semente...</p>
          </div>
        ) : (
          levels.map((lvl) => (
            <div 
