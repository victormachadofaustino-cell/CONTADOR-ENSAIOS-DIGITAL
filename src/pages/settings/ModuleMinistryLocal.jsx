...
const ModuleMinistryLocal = ({ comumId }) => { // Explicação: Inicia a construção do submódulo de Ministério Local recebendo a ID da igreja ativa.
  ...
  useEffect(() => { // Explicação: Escuta em tempo real os obreiros desta igreja comuns apenas quando o modal está aberto.
    ...
    const unsubMin = onSnapshot(subColecaoRef, (snapshot) => { // Explicação: Sincroniza em tempo real as adições, edições ou exclusões.
      ...
      lista.sort((a, b) => { // Explicação: Compara os cargos de dois obreiros na fila.
    ...
    }, (error) => { // Explicação: Captura erros da conexão em tempo real de forma segura.
  ...
  const handleInsertMinisterio = async () => { // Explicação: Função responsável por injetar um novo membro no Corpo Ministerial.
    if (!newMin.nome.trim() || !newMin.cargo) return toast.error("Preencha o nome completo e o cargo!"); // Explicação: Verifica se os campos obrigatórios estão preenchidos.
    if (!temPoderEdicao) return toast.error("Seu crachá não autoriza escritas nesta comum."); // Explicação: Barra a gravação se o usuário não possuir a caneta de escrita.

    try { // Explicação: Inicia o bloco de tentativa de conexão com o banco de dados na nuvem.
      const subColecaoRef = collection(db, 'comuns', comumId, 'ministerio_lista'); // Explicação: Localiza a rota física da subcoleção.
      await addDoc(subColecaoRef, { // Explicação: Envia o comando assíncrono para incluir o documento na nuvem.
    ...
    } catch (err) { // Explicação: Captura falhas de conexão de rede ou interrupções do banco.
  ...
  const handleSaveEdit = async () => { // Explicação: Grava as alterações feitas no obreiro que estava em edição.
    if (!editingMin?.nome.trim() || !editingMin?.cargo) return toast.error("Preencha todos os campos!"); // Explicação: Trava defensiva sanitária.
    try { // Explicação: Tenta atualizar os dados na nuvem.
      const docRef = doc(db, 'comuns', comumId, 'ministerio_lista', editingMin.id); // Explicação: Mira no documento exclusivo daquele obreiro.
      await updateDoc(docRef, { // Explicação: Grava os campos atualizados no Firestore.
    ...
    } catch (err) { // Explicação: Pega erros de internet.
  ...
  const handleConfirmDelete = async () => { // Explicação: Remove em definitivo o obreiro do banco de dados após a confirmação.
    ...
    try { // Explicação: Tenta remover o registro do banco de dados.
      await deleteDoc(doc(db, 'comuns', comumId, 'ministerio_lista', deletingMin.id)); // Explicação: Deleta permanentemente o documento físico do Firestore.
    ...
    } catch (err) { // Explicação: Pega falhas.
      toast.error("Erro ao remover obreiro."); // Explicação: Emite aviso de erro.
    } // Fim do catch.
  }; // Explicação: Encerra a função handleConfirmDelete.

  const handleEditClick = (m) => { // Explicação: Função auxiliar que injeta o obreiro selecionado no estado de edição and abre o modal correspondente.
  ...
  if (loading) return ( // Explicação: Renderiza um esqueleto de carregamento compacto e limpo.
    <div className="py-10 flex flex-col items-center justify-center gap-2"> {/* Explicação: Caixa centralizada com margens confortáveis. */}
      <div className="w-6 h-6 border-3 border-indigo-600 border-t-transparent rounded-full animate-spin"></div> {/* Explicação: Anel azul giratório de alta performance. */}
      <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest italic">Sincronizando Ministério...</p> {/* Explicação: Rótulo explicativo do boot. */}
    </div> // Explicação: Fecha a caixinha de loading.
  ); // Explicação: Fim da barreira de loading.

  return ( // Explicação: Inicia a renderização estrutural da interface visual.
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.93, opacity: 0, y: 10 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.93, opacity: 0, y: 10 }} className="relative w-full max-w-xs bg-white rounded-[2.2rem] p-6 shadow-2xl border border-slate-100 text-left">
              <div className="flex justify-between items-center mb-5 border-b border-slate-50 pb-2.5">
                <h3 className="text-[10px] font-black uppercase italic text-slate-950 tracking-wider">Adicionar Novo Obreiro</h3>
              ...
              <div className="space-y-3.5 text-left">
                <div className="flex flex-col gap-1 text-left">
                  ...
                  <input type="text" placeholder="NOME DO MINISTÉRIO POR EXTENSO" className="w-full bg-slate-50 p-3.5 rounded-xl font-black text-slate-950 text-xs border border-slate-100 uppercase italic outline-none focus:border-indigo-600 min-h-[44px]" value={newMin.nome} onChange={e => setNewMin({...newMin, nome: e.target.value})} />
                </div>
                <div className="flex flex-col gap-1 text-left">
                  ...
                  <div className="relative">
                    <select className="w-full bg-slate-50 p-3.5 rounded-xl font-black text-slate-950 text-[10px] border border-slate-100 uppercase italic outline-none focus:border-indigo-600 min-h-[44px] appearance-none cursor-pointer" value={newMin.cargo} onChange={e => setNewMin({...newMin, cargo: e.target.value})}>
                ...
                <button onClick={handleInsertMinisterio} className="w-full h-11 bg-indigo-600 text-white rounded-xl font-black uppercase italic text-[10px] shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2 pt-0.5 mt-2">
