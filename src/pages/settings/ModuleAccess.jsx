...
const ModuleAccess = ({ comumId, cargos }) => { // Explicação: Inicia o módulo de controle de acesso.
  ...
  useEffect(() => { // Explicação: Vigia a lista de usuários respeitando o limite geográfico.
    ...
    const unsubUsers = onSnapshot(qUsers, (s) => { // Explicação: Escuta em tempo real as atualizações dessa lista no banco de dados.
      ...
      if (!jaNaLista && userData) { // Explicação: Se o próprio gestor não estiver na lista:
    ...
    }, (err) => { // Explicação: Captura falhas ou desautorizações da portaria do banco.
    ...
    return () => { isMounted = false; unsubUsers(); }; // Explicação: Limpa o monitor e desativa o canal quando a tela é fechada.
  ...
  const usersGrouped = useMemo(() => { // Explicação: Organiza os usuários por nome da igreja na lista.
    ...
    return filteredUsers.reduce((acc, user) => { // Explicação: Agrupa a lista criando gavetas para cada nome de igreja.
  ...
  const podeEditarEstePerfil = (u) => { // Explicação: Decide se você tem poder para abrir a edição deste perfil.
    ...
    if (isGemLocal && u.comumId === userData.comumId) { // Explicação: GEM edita quem for da comum dele.
      return u.accessLevel !== 'master' && u.accessLevel !== 'comissao' && u.accessLevel !== 'regional_cidade'; // Explicação: Impede o GEM de alterar gestores municipais ou superiores.
    }
  ...
  const handleUpdate = async (userId, data) => { // Explicação: Envia a aprovação ou troca de nível para o banco.
    try { // Explicação: Inicia bloco de tentativa de gravação.
    ...
    } catch (e) { // Explicação: Captura qualquer barreira da portaria física do Firestore.
  ...
  if (authLoading) return <div className="p-10 text-center animate-pulse text-[10px] font-black uppercase text-slate-400 tracking-widest">Sincronizando Portaria...</div>; // Explicação: Carregamento preventivo.

  return ( // Explicação: Inicia a renderização do layout visual HTML/React.
    <div className="space-y-4 text-left font-sans animate-in fade-in duration-500">
      <div className="space-y-3">
        {sortedGroups.length === 0 ? ( // Explicação: Avalia se existem igrejas válidas mapeadas para exibição.
        ...
        ) : sortedGroups.map(groupName => { // Explicação: Inicia o mapeamento vertical percorrendo cada igreja comum.
          ...
          return ( // Explicação: Retorna o layout de cartões organizados de cada congregação.
                ...
                {isOpen && ( // Explicação: Abre as fichas internas caso a gaveta da igreja esteja ativa.
                  <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden">
                    <div className={`${isForcedOpen ? 'space-y-2' : 'p-4 pt-0 space-y-2'}`}>
                      {groupUsers
                        .sort((a, b) => (a.approved === b.approved) ? 0 : a.approved ? 1 : -1) // Explicação: Ordena colocando os pendentes de aprovação no topo da lista.
                        .map(u => ( // Explicação: Mapeia cada usuário transformando-o em um cartão vertical de toque de 44px.
                ...
                <div className="space-y-2">
                   ...
                   <div className="grid grid-cols-1 gap-2">
                     ...
                     <LevelButton 
                       ...
                       canEdit={podeEditarEstePerfil(selectedUser) && hasPermission(userData, 'change_role', { currentRole: selectedUser.accessLevel, targetRole: ROLES.BASICO })} 
                     />
                     <LevelButton 
                       ...
                       canEdit={podeEditarEstePerfil(selectedUser) && hasPermission(userData, 'change_role', { currentRole: selectedUser.accessLevel, targetRole: ROLES.GEM })} 
                     />
                     <LevelButton 
                       ...
                       canEdit={podeEditarEstePerfil(selectedUser) && hasPermission(userData, 'change_role', { currentRole: selectedUser.accessLevel, targetRole: ROLES.CIDADE })} 
                     />
                     <LevelButton 
                       ...
                       canEdit={podeEditarEstePerfil(selectedUser) && hasPermission(userData, 'change_role', { currentRole: selectedUser.accessLevel, targetRole: ROLES.COMISSAO })} 
                     />
                   </div>
                </div>
