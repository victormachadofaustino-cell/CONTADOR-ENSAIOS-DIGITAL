...
import {
  ...
  where,
  getDoc,
} from "../../config/firebase"; // Explicação: CONEXÃO COM O FIREBASE: Mantém a importação estável de todos os motores de dados.
...
const CounterPage = ({ currentEventId, counts, onBack, allEvents }) => {
  ...
  useEffect(() => {
    // Explicação: Monitor inteligente que carrega a subcoleção de chamada nominal do instrumento focado sem tocar na raiz da comum.
    if (!currentEventId || !instrumentoFocadoChecklist) {
      // Explicação: Se o código do ensaio ou o instrumento selecionado estiverem ausentes.
    ...
    const idExtensoReal =
      idCru.length <= 4 ? MAPA_TRADUTOR_EXTENSO[idCru] : idCru; // Explicação: Traduz a sigla para o nome por extenso de salvamento.

    const valorAtualBanco = localCounts?.[idExtensoReal || idCru]?.comum || 0; // Explicação: Puxa os presentes antigos salvos no mapa mestre.
    setValorNumeroDireto(valorAtualBanco > 0 ? String(valorAtualBanco) : ""); // Explicação: Alimenta a memória temporária do input local para digitação fluida.
    ...
    const llamadaRef = collection(
      ...
      currentEventId,
      "chamada_musicos",
    ); // Explicação: Prepara a rota da ficha de presença de portaria.
    ...
    ); // Explicação: Saneia e expulsa ponteiros em branco para não crashar a consulta.

    const q = query(
      llamadaRef,
      where("instrumentoId", "in", arrayFiltroValido),
    ); // Explicação: Une as chaves filtradas e seguras na mesma consulta do Firestore.

    const unsubChamada = onSnapshot(q, (snap) => {
      // Explicação: Ouve em tempo real as presenças daquele naipe específico.
    ...
    localCounts,
    MAPA_TRADUTOR_EXTENSO,
  ]); // Explicação: Reinicia se mudar o instrumento focado ou o ensaio.
  ...
  const handleTogglePresencaMusico = async (musico) => {
    ...
    try {
      ...
      ); // Explicação: Clona e updates a portaria na RAM.
      let totalPresentesEfetivo = listaAtualizadaEspelho.filter(
        (m) => m.presente,
      ).length; // Explicação: Conta as cabeças marcadas em verdadeiro.

      const idCru = instrumentoFocadoChecklist.id; // Explicação: Resgata a ID de foco.
      const targetId =
        idCru.length <= 4 ? MAPA_TRADUTOR_EXTENSO[idCru] || idCru : idCru; // Explicação: Traduz para a chave por extenso tolerante para checar o teto físico real do painel.

      // 🚀 BARREIRA ANTI-ZERO PARA TOQUE NOMINAL: Garante a mesma tolerância de limite para chamadas por nome se o total das setas vier zerado por falha.
      const totalMaximoPermitido =
        parseInt(localCounts?.[targetId]?.total) ||
        parseInt(localCounts?.[idCru]?.total) ||
        999; // Explicação: Resgata o limite absoluto ou libera folga segura.

      if (totalPresentesEfetivo > totalMaximoPermitido) {
        // Explicação: Se a somatória de nomes marcados passar do teto físico.
        totalPresentesEfetivo = totalMaximoPermitido; // Explicação: Reduz e trava o contador no limite teto mestre.
      } // Explicação: Encerra a checagem nominal de limite.

      setValorNumeroDireto(
        totalPresentesEfetivo > 0 ? String(totalPresentesEfetivo) : "",
      ); // Explicação: Despeja o totalizador no input numérico do modal sem travar em zero.
      ...
      setLocalCounts((prev) => ({
        ...
        }, // 🚀 ALINHAMENTO VISUAL ATÔMICO: Grava e espelha no nó extenso do Firebase.
        [idCru]: {
          ...prev[idCru],
          comum: totalPresentesEfetivo,
          modoContagem: "nominal",
        }, // 🚀 ALINHAMENTO VISUAL ATÔMICO: Updates simultaneamente a chave original que o card de trás lê, fazendo o número pular do zero na hora!
      })); // Explicação: Updates visual otimista da tela mestre.
    } catch (err) {
      // Explicação: Captura falhas de escrita.
      toast.error("Erro ao registrar presença."); // Explicação: Notifica erro de rede.
    } // Explicação: Encerra o scope da tentativa.
