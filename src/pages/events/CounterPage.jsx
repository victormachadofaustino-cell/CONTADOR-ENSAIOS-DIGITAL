...
const CounterPage = ({ currentEventId, counts, onBack, isMaster, isAdmin, userData, allEvents }) => {
  ...
  const allInstruments = useMemo(() => {
    const ordemOficial = [
      'irmandade', 'orgao', 'violino', 'viola', 'violoncelo', 
    ...
    const extras = Object.keys(localCounts)
      .filter(key => !key.startsWith('meta_') && !ordemOficial.includes(key))
      .map(key => ({
        id: key,
        name: localCounts[key].name || key.toUpperCase(),
        section: localCounts[key].section?.toUpperCase() || 'GERAL',
  ...
  const sections = useMemo(() => {
    const ordemSessoes = ['CORAL', 'ORGANISTAS', 'CORDAS', 'MADEIRAS', 'SAXOFONES', 'METAIS', 'TECLAS'];
  ...
  const confirmAddExtra = async () => {
    ...
    const idExtra = extraName.toLowerCase().trim().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/\s+/g, '_');
  ...
  const handleToggleGroup = (sec) => {
    ...
    if (responsibleId === myUID || isMaster) setActiveGroup(sec);
        ...
        <div className="max-w-md mx-auto space-y-3">
          {activeTab === 'contador' && sections.map(sec => {
            ...
            const sectionTotal = allInstruments.filter(i => i.section?.toUpperCase() === sec).reduce((acc, inst) => {
                ...
                return acc + (inst.id === 'irmandade' ? (parseInt(c?.irmaos) || 0) + (parseInt(c?.irmas) || 0) : (parseInt(c?.total) || 0));
            }, 0);
                ...
                <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden">
                  ...
                  {activeGroup === sec && (
                    <div className="p-3 pt-0 space-y-2 animate-in slide-in-from-top-2 duration-300">
                      ...
                      {!isClosed && isEditingEnabled(sec) && sec !== 'CORAL' && sec !== 'ORGANISTAS' && (
                        <button 
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-5 px-6 text-xs font-black text-slate-950 outline-none uppercase italic shadow-inner focus:bg-white focus:border-blue-200 transition-all"
                    value={extraName}
