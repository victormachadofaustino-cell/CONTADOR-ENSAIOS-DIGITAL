...
import { ChevronDown, Lock, ShieldCheck, Trash2, Globe, MapPin } from 'lucide-react'; // Explicação: Importa os ícones de setas, cadeados, escudos e alfinetes de mapas da biblioteca.

// v1.3.4: CONFIGURAÇÃO DE SEGURANÇA E PARAMETRIZAÇÃO CONTEXTUAL DE PERMISSÕES
import { hasPermission } from '../../../config/permissions'; // Explicação: Importa o gerenciador oficial da portaria para verificar se o usuário pode realizar ações.
...
const EventsList = ({ 
  ...
  const formatMonth = (dateStr) => { // Explicação: Recorta a string estável americana de data e converte o mês em sigla textual brasileira.
    if (!dateStr || typeof dateStr !== 'string' || !dateStr.includes('-')) return '---'; // Explicação: Proteção de barreira sanitária caso o formato venha inválido.
    const months = ["Jan", "Fev", "Mar", "Abr", "Mai", "Jun", "Jul", "Ago", "Set", "Out", "Nov", "Dez"]; // Vetor indexado de meses.
  ...
  return ( // Explicação: Desenha o layout do esqueleto da listagem cronológica na tela do celular.
    <div className="space-y-8 max-w-md mx-auto text-left"> {/* Explicação: Grid empilhado verticalmente limitado a larguras mobile e alinhado à esquerda. */}
      {years.map(year => ( // Explicação: Inicia a varredura dos anos disponíveis para criar os blocos sanfona.
        <div key={year} className="space-y-4 text-left">
          ...
          >
            <span className="text-3xl font-[900] italic text-slate-950 tracking-tighter text-left leading-none">{year}</span> {/* Número do ano em destaque extra negrito. */}
            <div className="h-[2px] flex-1 bg-slate-200 rounded-full opacity-50 group-hover:opacity-100 transition-opacity"></div> {/* Linha estética horizontal expansiva. */}
            <div className="bg-white p-2 rounded-xl shadow-sm border border-slate-100 shrink-0"> {/* Moldura para a seta. */}
                <ChevronDown 
                  size={14} 
          ...
          {openYears[year] && ( // Explicação: Avalia o dicionário e abre as fileiras de cartões apenas se o ano estiver verdadeiro.
            <div className="space-y-4 animate-in fade-in duration-300 text-left">
              {(groupedEvents[year] || []).map(e => { // Explicação: Começa a processar e desenhar os cartões individuais de cada ensaio cadastrado.
                ...
                const isGuest = e.comumId !== userData?.comumId && (e.invitedUsers || []).includes(userData?.uid); // Explicação: Verifica se o ID do secretário está no vetor de permissões extras de ajuda de contagem.
                ...
                const podeApagarEsteEvento = hasPermission(userData, 'delete_event', e);

                // Definição dinâmica de cores e ícones baseada no escopo
                const scopeColor = isRegional ? 'text-blue-600' : 'text-amber-500'; // Azul para regional, amarelo para local.
                const sidebarColor = isRegional ? 'bg-blue-600' : (isGuest ? 'bg-blue-400' : 'bg-slate-950'); // Injeta tom ardósia, azul ou turquesa na linha indicativa.

                return ( // Explicação: Renderiza o cartão físico interativo do ensaio.
                  <div 
                  ...
                  >
                    ...
                    <div className={`absolute left-0 top-0 h-full w-1.5 ${isClosed ? 'bg-slate-300' : sidebarColor}`} /> {/* Explicação: Fica cinza fosco se o livro estiver trancado, sinalizando somente leitura. */}
                    
                    <div className="flex items-center gap-5 text-left flex-1 min-w-0 pr-2">
                      {/* Calendário do Card (Data do Ensaio) */}
                      <div className={`flex flex-col items-center justify-center w-16 h-16 rounded-[1.8rem] border-2 transition-colors shrink-0 ${
                        ...
                        <span className="text-2xl font-[900] italic leading-none">{e.date?.split('-')[2] || '--'}</span> {/* Recorta e exibe o número do dia. */}
                        <span className="text-[8px] font-black uppercase mt-1 tracking-widest leading-none">{formatMonth(e.date)}</span> {/* Exibe a sigla do mês convertida. */}
                      </div>
                      ...
                      <div className="text-left leading-none flex-1 min-w-0">
                        <div className="flex flex-col gap-1.5 mb-1.5 text-left">
                            ...
                            <div className="flex items-center gap-1.5 text-left w-full">
                              {(isRegional || isGuest) && !isClosed && <Globe size={8} className="text-blue-600 shrink-0" />} {/* Desenha o ícone de globo terrestre para sinalizar relevância regional. */}
                              <p className={`text-[8px] font-black uppercase italic tracking-[0.2em] leading-none text-left truncate ${isClosed ? 'text-slate-300' : scopeColor}`}>
                                {e.type || (isRegional ? 'Ensaio Regional' : 'Ensaio Local')} {/* Imprime a modalidade textual do evento. */}
                              </p>
                            </div>
                        </div>
                        <h4 className={`text-[13px] font-[900] uppercase italic tracking-tighter text-left leading-tight truncate w-full ${isClosed ? 'text-slate-400' : 'text-slate-950'}`}>
                          {e.responsavel} {/* Imprime o nome completo do Ancião ou Encarregado de atendimento. */}
                        </h4>
                        {(isGuest || isRegional) && ( // Explicação: Se o ensaio envolver tráfego geográfico externo, denormaliza e mostra o nome da igreja por extenso.
                          <div className="flex items-center gap-1 mt-1.5 opacity-40 text-left w-full">
                            ...
                            <p className="text-[7px] font-bold uppercase text-left truncate flex-1">{e.comumNome || 'Local não informado'}</p>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 shrink-0" onClick={(cl) => cl.stopPropagation()}> {/* 🚀 TRAVA DE PROPAGAÇÃO DE FILHO: Bloqueia que o toque nos botões da direita acione o clique do card pai por acidente. */}
                      {isClosed ? (
                        <Lock size={16} className="text-slate-200 shrink-0" /> // Desenha o cadeado cinza fosco de livro trancado.
                      ) : (
                        <ShieldCheck size={16} className={`shrink-0 ${(isRegional || isGuest) ? 'text-blue-500' : 'text-emerald-500'} animate-pulse`} /> // Desenha o escudo pulsante de coleta ao vivo.
                      )}
                      
                      {/* v1.3.4: Lógica de exclusão reativa baseada na Portaria Null-Safety Contextual */}
                      {podeApagarEsteEvento && !isClosed && ( // Explicação: Acende e renderiza o botão vermelho da lixeira se o crachá local do GEM bater territorialmente.
                        <button 
