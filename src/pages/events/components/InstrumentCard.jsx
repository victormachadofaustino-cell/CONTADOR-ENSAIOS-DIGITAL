...
const InstrumentCard = ({
  ...
  const isIrmandade = ["irmandade", "irmas", "irmaos", "coral"].includes(
    subId || "",
  ); // Explicação: Verifica se o ID bate com as ramificações do Coral.

  // Explicação: Identifica se este cartão é de organista para aplicar tratamento especial de cargo.
  const isOrganista =
    subId.includes("organista") ||
    inst.name?.toLowerCase().includes("organ") ||
    inst.label?.toLowerCase().includes("orgao") ||
    subId.includes("orgao"); // Explicação: Procura variações textuais de Órgão ou Organista.

  // Explicação: Identifica se é um campo de liderança ou examinadora.
  const isGovernance =
    (inst.isGovernance ||
      subId.includes("enc_local") ||
      inst.evalType === "Examinadora") &&
    !isOrganista; // Explicação: Define se o cartão atua como cargo de escala de liderança.
  ...
  const targetId =
    sectionName?.toUpperCase() === "IRMANDADE"
      ? "coral"
      : sectionName?.toUpperCase() === "ORGANISTAS"
        ? "orgao"
  ...
  const podeModificarAqui = useMemo(() => {
    ...
    if (level === "gem_local" || level === "basico") {
      ...
      return (
        minhaIgreja === "hsfjhZ3KNx7SsCM8EFpu" || minhaIgreja === igrejaDoEvento // Explicação: Libera o acesso se as chaves territoriais forem idênticas.
  ...
  const displayVal =
    subId === "irmas" ? irmas : subId === "irmaos" ? irmaos : total; // Explicação: Escolhe a variável matemática de exibição correspondente da linha.
  ...
  const isModoNominalAtivo =
    data?.modoContagem === "nominal" ||
    data?.[targetId]?.modoContagem === "nominal"; // Explicação: Sinaliza se o instrumento está operando via lista nominal de presenças.

  /**
   * handleUpdate v3.16 - Lógica de Saneamento Hierárquico
   */
  const handleUpdate = (field, value) => {
    // Explicação: Subrotina que processa as setas de + e - da tela principal de trás.
    ...
    const payloadAdicional =
      field === "total" && isModoNominalAtivo
        ? { modoContagem: "numerico" }
        : {}; // Explicação: Desativa o modo nominal se reajustar setas físicas.
  ...
  const handleOpenChecklistSaneado = () => {
    ...
    }); // Explicação: Executa o gatilho modular.
  }; // Explicação: Termina o handleOpenChecklistSaneado.

  return (
    // Explicação: Desenha o cartão do instrumento com bordas arredondadas (Higiene de UI).
    <div
      className={`p-4 rounded-[2rem] border transition-all relative overflow-hidden bg-white shadow-sm w-full text-left ${
        isMyTurn
    ...
    >
      <div className="mb-3 flex justify-between items-center pr-1 text-left w-full gap-2">
        {" "}
        {/* Explicação: Container do cabeçalho alinhado no topo para centralizar itens horizontalmente. */}
        <div className="flex flex-col text-left leading-none min-w-0 flex-1">
          {" "}
          {/* Explicação: Alinha verticalmente os textos protegendo contra estouro de largura. */}
          <h5 className="font-[900] text-[11px] italic uppercase text-slate-950 tracking-tighter leading-none mb-1 flex items-center gap-1.5 flex-wrap whitespace-normal text-left">
            {" "}
            {/* Explicação: Título em destaque sem elipses para visualização inteira por extenso. */}
            {inst.label || inst.name || inst.nome || "INSTRUMENTO"}{" "}
            {/* Explicação: Busca as referências do nome do objeto mestre 'inst'. */}
            {(isGovernance || isOrganista) && (
              <ShieldCheck size={12} className="text-blue-500 shrink-0" />
            )}{" "}
            {/* Explicação: Selo de cargo oficial impedido de encolher. */}
          </h5>
          {(isOtherTurn || isMyTurn) && ( // Explicação: Exibe quem é o responsável pela contagem deste instrumento.
            <div className="flex items-center gap-1.5 mt-1 text-left">
              {" "}
              {/* Explicação: Caixa horizontal de alfinete de alinhamento do ponto luminoso e nome. */}
              <div
                className={`w-1.5 h-1.5 rounded-full shrink-0 ${isMyTurn ? "bg-blue-500 animate-pulse" : "bg-amber-400"}`}
              />{" "}
              {/* Explicação: Punto luminoso reativo de controle de dono da aba. */}
              <span
          ...
          !isClosed && ( // Explicação: Botão para assumir a posse da contagem em eventos Regionais.
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                onToggleOwnership();
              }} // Explicação: Dispara a troca de posse individualizada.
            ...
            >
              ...
              {/* Explicação: Desenha o ícone de boneco com sinal de mais de adição. */}
              <span className="text-[8px] font-black uppercase italic">
                {isMyTurn ? "LOGADO" : "ASSUMIR"}
              </span>{" "}
              {/* Explicação: Etiqueta textual em caixa alta micro. */}
            </button>
          )}
      </div>
      ...
      <div className="flex flex-col gap-2.5 w-full text-left">
        ...
        {isIrmandade && !isRegional ? ( // Explicação: Desenha caixas separadas para Irmãs e Irmãos no Local.
          <div className="flex gap-2 h-28 w-full items-stretch text-left">
            {" "}
            {/* Explicação: Garante o alinhamento de colunas lado a lado sem esmagamento interno de inputs. */}
            <CounterBox
              label="IRMÃS"
              color="slate"
              val={irmas}
          ...
          <div className="flex gap-2 h-28 w-full items-stretch">
            {" "}
            {/* Explicação: Força o esticamento vertical ocupando 100% de largura para não esmagar inputs. */}
            <CounterBox
          ...
          <>
            <div className="flex gap-2 h-28 w-full items-stretch text-left">
              {" "}
              {/* Explicação: Une e estica horizontalmente o bloco de Total, Comum e Visitas impedindo o encolhimento. */}
              <CounterBox
                ...
                color={
                  isMyTurn && isRegional
                    ...
                    : isRegional
                      ? "white"
                      : "slate"
                } // Explicação: Decide a cor de contraste da caixa mestre de totalização.
              ...
              />
              {!isRegional &&
                !isGovernance && ( // Explicação: Campos detalhados de visitas exclusivos para Ensaios Locais comuns.
                  <>
                    <div
                      onClick={() =>
                        !isSubFieldDisabled && handleOpenChecklistSaneado()
                      } // Explicação: Dispara o modal nominal convertendo a chave para carregar os músicos da garagem.
                    ...
                    >
                      <CounterBoxButton
                        ...
                        val={comuneVal} // Explicação: Número atual gravado e updated síncrono.
                    ...
                    <div
                      className={`flex-[0.5] flex flex-col items-center justify-center rounded-[1.5rem] border transition-colors min-w-[50px] ${total === 0 ? "bg-slate-50 border-slate-100" : "bg-blue-50 border-blue-100"}`}
                    >
                      {" "}
                      {/* Explicação: Aplica uma largura mínima de segurança para o bloco matemático de visitas não sumir. */}
                      <span
                        className={`text-[7px] font-black uppercase tracking-widest mb-1 italic ${total === 0 ? "text-slate-300" : "text-blue-400"}`}
                      >
                        Visitas
                      </span>{" "}
                      {/* Explicação: Etiqueta cinza ou azul. */}
                      <span
                        className={`text-2xl font-[900] italic leading-none ${total === 0 ? "text-slate-200" : "text-blue-600"}`}
                      >
                        {visitas}
                      </span>{" "}
                      {/* Explicação: Resultado automático della subtração Total - Comum. */}
                    </div>
                  </>
                )}
            </div>
            ...
            {!isRegional &&
              !isGovernance && ( // Explicação: Barra inferior de controle de Liderança (Encarregados Locais presentes).
                <div
                  className={`mt-0.5 rounded-xl p-2 flex items-center justify-between border transition-all w-full ${isSubFieldDisabled ? "bg-slate-50 border-slate-100" : "bg-slate-100/50 border-slate-200/50"}`}
                >
                  {" "}
                  {/* Explicação: Barra horizontal compacta cinza ergonômica ocupando largura total. */}
                  <div className="flex items-center gap-2 min-w-0 flex-1 text-left">
                    {" "}
                    {/* Explicação: Agrupamento do ícone protegendo contra esmagamento de texto. */}
                    <div
                      className={`p-1.5 rounded-lg text-white shrink-0 ${isSubFieldDisabled ? "bg-slate-300" : "bg-slate-950"}`}
                    >
                      {" "}
                      {/* Explicação: Ícone quadrado escuro. */}
                      <UserCheck size={10} strokeWidth={3} />{" "}
                      {/* Explicação: Ícone de verificação de obreiro. */}
                    </div>
                    <span className="text-[8px] font-black text-slate-500 uppercase italic tracking-widest leading-none truncate text-left">
                      {" "}
                      {/* Explicação: Texto descritivo truncado para segurança visual. */}
                      {labelLideranca || "Liderança"}{" "}
                      {/* Explicação: Exibe 'Encarregado' ou 'Examinadora' dinamicamente. */}
                    </span>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    {" "}
                    {/* Explicação: Agrupa os botões impedindo-os de quebrar de tamanho. */}
                    <button
                      type="button"
                      disabled={isSubFieldDisabled}
                      onClick={() => handleUpdate("enc", enc - 1)}
                      className={`p-1.5 cursor-pointer ${isSubFieldDisabled ? "opacity-0" : "text-slate-400"}`}
                    >
                      {" "}
                      {/* Explicação: Botão de diminuir encarregado. */}
                      <Minus size={14} strokeWidth={4} />{" "}
                      {/* Explicação: Desenha o traço del menos grosso. */}
                    </button>
                    <span
                      className={`text-lg font-[900] italic w-6 text-center ${isSubFieldDisabled ? "text-slate-200" : "text-slate-950"}`}
                    >
                      {enc}
                    </span>{" "}
                    {/* Explicação: Exibe o número de encarregados ativos salvos. */}
                    <button
                      ...
                      className={`p-1.5 cursor-pointer ${isSubFieldDisabled || enc >= total ? "opacity-20 pointer-events-none" : "text-slate-400"} transition-opacity`} // Explicação: Efeito de esmaecido se atingir a trava limite.
                    >
                      <Plus size={14} strokeWidth={4} />{" "}
                      {/* Explicação: Desenha o sinal de mais grosso. */}
                    </button>
                  </div>
                </div>
              )}
...
const CounterBox = ({
  ...
  return (
    <div
      className={`flex-1 rounded-[1.5rem] border transition-all relative flex flex-col items-center justify-center overflow-hidden min-w-[65px] h-full ${
        disabled
    ...
    >
      <p
        className={`absolute top-2 text-[6px] font-black uppercase tracking-[0.2em] ${color === "slate" ? "text-white/30" : "text-slate-400"}`}
      >
        {label}
      </p>
      ...
      <div className="flex items-center w-full h-full pt-3 justify-between">
        ...
        >
          <Minus
            size={isMain ? 14 : 11}
            strokeWidth={4}
            className={color === "slate" ? "text-white/20" : "text-slate-300"}
          />
        </button>
        ...
        >
          <Plus
            size={isMain ? 14 : 11}
            strokeWidth={4}
            className={color === "slate" ? "text-white/80" : "text-slate-950"}
          />
        </button>
      </div>
    </div>
...
const CounterBoxButton = ({
  ...
  <div
    className={`w-full h-full rounded-[1.5rem] border transition-all relative flex flex-col items-center justify-center overflow-hidden p-4 shadow-3xs border-dashed text-center active:scale-98 cursor-pointer ${
      disabled
  ...
  >
    <p
      className={`text-[6px] font-black uppercase tracking-[0.2em] mb-1 truncate ${disabled ? "text-slate-200" : activeMode ? "text-white/50" : color === "slate" ? "text-white/30" : "text-slate-400"}`}
    >
      {label}
    </p>
    ...
    <div className="flex items-center justify-center gap-1.5">
      <span
        className={`text-2xl font-[900] italic leading-none tracking-tighter ${disabled ? "text-slate-200" : "text-inherit"}`}
      >
        {val}
      </span>
      {activeMode && !disabled && (
    ...
    <span
      className={`text-[5.5px] font-black uppercase tracking-tight mt-1.5 block ${disabled ? "text-slate-200" : activeMode ? "text-white/70" : "text-indigo-500"}`}
    >
      {disabled
        ? "Bloqueado"
        : activeMode
