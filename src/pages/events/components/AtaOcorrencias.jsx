 ...
 * v1.1 - Adicionado Agrupamento Inteligente e Remoção do item 'Coral' do dropdown.
 */
const AtaOcorrencias = ({ ocorrencias = [], onSave, instruments = [], isClosed }) => {
  ...
  const handleAdd = () => {
    if (template === 'A') {
      ...
      if (!textoLivre) return toast.error("Digite o texto");
      if (textoLivre.length > 160) return toast.error("Limite excedido");
      onSave([...ocorrencias, { id: Date.now().toString(), tipo: 'C', texto: textoLivre }]);
    }
    resetForm();
  };
    ...
    <div className="space-y-4 text-left">
      ...
      {showAdd && (
        <div className="bg-white border-2 border-slate-950 rounded-[2rem] p-6 shadow-xl animate-in fade-in zoom-in duration-200">
          <div className="flex justify-between items-center mb-6 text-slate-950">
             <span className="text-[10px] font-black uppercase italic tracking-tighter">Novo Registro</span>
          ...
          <div className="space-y-4">
            ...
            {template === 'C' && (
              <textarea maxLength={160} rows={3} value={textoLivre} onChange={e => setTextoLivre(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-xs font-bold outline-none resize-none" placeholder='Ex: "Neste ensaio Deus preparou um instrumento..."' />
            )}

            <button onClick={handleAdd} className="w-full bg-slate-950 text-white py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl">Confirmar Registro</button>
          </div>
        </div>
      )}
      ...
      <div className="space-y-3">
        {ocorrencias.map((item) => (
          <div key={item.id} className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm flex justify-between items-center group">
            <p className="text-[10px] font-bold text-slate-900 pr-4 italic leading-relaxed">{item.texto}</p>
            {!isClosed && <button onClick={() => onSave(ocorrencias.filter(o => o.id !== item.id))} className="text-slate-200 hover:text-red-500"><Trash2 size={14} /></button>}
          </div>
        ))}
      </div>
    </div>
