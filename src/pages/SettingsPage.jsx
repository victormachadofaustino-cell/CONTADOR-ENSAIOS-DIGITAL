...
import {
  // Explicação: Importa o pacote de ícones modernos e visuais para ilustrar os botões e menus do aplicativo.
  Home,
  Music,
  Users,
  ShieldCheck,
...
import ModuleChurch from "../features/settings/ui/ModuleChurch"; // Explicação: Importa o submódulo de edição de dados cadastrais da igreja.
import ModuleOrchestra from "../features/settings/ui/ModuleOrchestra"; // Explicação: Importa o submódulo de ativação e desativação de instrumentos musicais.
import ModuleGlobal from "../features/settings/ui/ModuleGlobal"; // Explicação: Importa o submódulo administrativo de Cargos e Ministérios.
import ModuleAccess from "../features/settings/ui/ModuleAccess"; // Explicação: Importa o submódulo de controle de portaria e liberação de usuários.
import ModuleCities from "../features/settings/ui/ModuleCities"; // Explicação: Importa o submódulo de cadastro geográfico de cidades.
import ModuleChurchesManager from "../features/settings/ui/ModuleChurchesManager"; // Explicação: Importa o gerenciador geral de criação e manutenção de cookies comuns.
import ModuleMinistryLocal from "../features/settings/ui/ModuleMinistryLocal"; // Explicação: AMARRAÇÃO DE INFRAESTRUTURA: Importa o novo lar purificado do Corpo Ministerial Eclesiástico.
import ModuleOrchestraBody from "../features/settings/ui/ModuleOrchestraBody"; // Explicação: AMARRAÇÃO DE INFRAESTRUTURA: Importa a garagem nominal de músicos para a chamada de presença.
...
const SettingsPage = () => {
  ...
  const activeRegionalId =
    userData?.activeRegionalId || userData?.regionalId || null; // Explicação: Define a ID da Regional principal baseada no Crachá do usuário.
  ...
  const [sharedData, setSharedData] = useState({
    // Explicação: Cria um armário de memória para guardar dados compartilhados entre as telas.
  ...
  const confirmarExclusaoNativa = (itemNome, aoConfirmar) => {
    // Explicação: Cria uma caixa de confirmação de exclusão bonita, mobile-first e segura.
    toast(
      (
        t, // Explicação: Dispara um alerta customizado no centro da tela com botões grandes.
      ) => (
        <div className="flex flex-col gap-4 p-1 min-w-[280px]">
          {" "}
          {/* Explicação: Caixa vertical com espaçamentos confortáveis para cliques no celular. */}
          <div className="flex flex-col gap-1 text-left">
            {" "}
            {/* Explicação: Agrupa os textos de aviso de exclusão alinhados à esquerda. */}
            <div className="flex items-center gap-2 text-red-600">
              {" "}
              {/* Explicação: Linha com ícone de lixeira e título na cor vermelha de perigo. */}
              <Trash2 size={16} strokeWidth={3} />{" "}
              {/* Explicação: Ícone visual de lixeira com traço grosso and bem visível. */}
              <p className="text-[12px] font-black uppercase tracking-wider">
                Confirmar Exclusão
              </p>{" "}
              {/* Explicação: Texto de cabeçalho em letras maiúsculas e bem marcantes. */}
            </div>{" "}
            {/* Explicação: Fecha o cabeçalho do aviso. */}
            <p className="text-[11px] text-slate-500 font-medium leading-relaxed whitespace-normal break-words">
              {" "}
              {/* Explicação: Parágrafo explicativo em tom cinza com quebra de linha de texto inteira sem cortes. */}
              Deseja realmente excluir{" "}
              <span className="font-bold text-slate-950 italic">
                "{itemNome}"
              </span>
              ?{" "}
              {/* Explicação: Mostra o nome do item que será deletado em negrito e itálico completo. */}
              Esta ação é irreversível e removerá todos os dados vinculados.{" "}
              {/* Explicação: Alerta de segurança informando que não dá para desfazer a ação. */}
            </p>{" "}
            {/* Explicação: Fecha o parágrafo explicativo. */}
          </div>{" "}
          {/* Explicação: Fecha o bloco de textos. */}
          <div className="flex gap-2 justify-end pt-2">
            {" "}
            {/* Explicação: Linha horizontal que alinha os botões de ação no canto inferior direito. */}
            ...
            {/* Explicação: Fecha o botão cancelar. */}
            <button // Explicação: Botão vermelho e definitivo para confirmar a exclusão.
              onClick={() => {
                // Explicação: Executa as funções de apagar e fecha o alerta ao mesmo tempo.
            ...
            >
              Excluir Agora{" "}
              {/* Explicação: Texto do botão de confirmation definitiva. */}
            </button>{" "}
            {/* Explicação: Força o fechamento do botão de exclusão. */}
          </div>{" "}
          {/* Explicação: Fecha a linha de botões. */}
        </div>
      ),
      {
        ...
        style: {
          // Explicação: Desenha a estética física do balão (cantos arredondados, fundo branco e sombra marcante).
          borderRadius: "2.5rem",
          background: "#fff",
  ...
  useEffect(() => {
    // Explicação: Monitor que roda automaticamente para sincronizar as seleções do GPS com os menus da tela.
    if (userData?.activeCityId) {
      // Explicação: Se o GPS do app estiver focado em uma cidade específica.
      const city = sharedData.cidades.find(
        (c) => c.id === userData.activeCityId,
      ); // Explicação: Procura essa cidade dentro da nossa lista na memória.
      if (city)
        setSelectedCity(city); // Explicação: Se achou, marca ela como a cidade ativa na tela.
    ...
    if (!userData?.activeComumId) {
      // Explicação: Se o GPS não tiver nenhuma igreja selecionada.
  ...
  useEffect(() => {
    // Explicação: O Motor de Otimização de Banco de Dados que busca as locais de apoio baseadas no Crachá do Usuário.
    ...
    try {
      // Explicação: Tenta abrir as conexões com o banco de dados.
      unsubs.push(
        onSnapshot(collection(db, "referencia_cargos"), (s) => {
          // Explicação: Abre um canal em tempo real com a tabela global de Cargos e Ministérios.
          ...
          setSharedData((prev) => ({
            // Explicação: Separa e salva os dados nas suas respectivas gavetas na memória interna.
            ...prev,
            cargos: cargosData.filter((c) => c.tipo === "cargo"), // Explicação: Filtra e guarda apenas o que for Cargo de músico.
            ministeriosDropdown: cargosData.filter(
              (c) => c.tipo === "ministerio",
            ), // Explicação: Filtra e guarda apenas o que for Ministério da irmandade.
          }));
        }),
      );

      // --- CRITÉRIO DE QUOTA CIRÚRGICO: Secretário Local (gem_local) não baixa a lista de toda a regional ---
      if (level === "gem_local" && userData?.comumId) {
        // Explicação: Se o crachá do usuário disser que ele cuida apenas de uma igreja local.
        ...
        unsubs.push(
          onSnapshot(comumDocRef, (docSnap) => {
            // Explicação: Abre o canal de escuta apenas para esse documento específico de forma ultra econômica.
            if (!isMounted || !docSnap.exists()) return; // Explicação: Aborta se o documento não existir ou se a tela foi fechada.
            const comumUnica = {
              id: docSnap.id,
              ...docSnap.data(),
              comum: docSnap.data().comum || "Sem Nome",
            }; // Explicação: Monta os dados della única igreja dele.
            setSharedData((prev) => ({
              ...prev,
              comunsDaRegional: [comumUnica],
              cidades: [
                {
                  id: comumUnica.cidadeId,
                  nome: comumUnica.cidadeNome || "SUA CIDADE",
                },
              ],
            })); // Explicação: Alimenta as opções da tela direto com o dado dele.
        ...
        // Explicação: Caso o usuário seja nível Regional ou Master, ele sim precisa ver a lista expandida.
        const qComuns = query(
          collection(db, "comuns"),
          where("regionalId", "==", activeRegionalId),
        ); // Explicação: Cria uma busca filtrando apenas as igrejas que pertencem à regional dele.
        unsubs.push(
          onSnapshot(qComuns, (s) => {
            // Explicação: Abre o canal em tempo real para escutar a lista de igrejas da regional.
            if (!isMounted) return; // Explicação: Aborta se a tela foi fechada durante o processo.
            const listaBruta = s.docs.map((d) => ({
              id: d.id,
              ...d.data(),
              comum: d.data().comum || "Sem Nome",
            })); // Explicação: Transforma a lista de comuns do banco in formato legível.
            const permitidasIds = [
              userData?.comumId,
              ...(userData?.acessosPermitidos || []),
            ]; // Explicação: Mapeia quais IDs de igrejas este usuário tem direito de enxergar.
            ...
            const comunsVisiveis = isComissao // Explicação: Aplica o filtro de segurança de visualização territorial.
              ...
              : listaBruta.filter(
                  (c) =>
                    c.cidadeId === userData?.cidadeId ||
                    permitidasIds.includes(c.id),
                ); // Explicação: Se for regional de cidade, vê apenas as igrejas da sua própria cidade.

            setSharedData((prev) => ({
              ...prev,
              comunsDaRegional: comunsVisiveis,
            })); // Explicação: Salva a lista filtrada de igrejas comuns na memória da tela.

            const qCidades = query(
              collection(db, "config_cidades"),
              where("regionalId", "==", activeRegionalId),
            ); // Explicação: Cria uma busca pelas cidades configuradas dentro desta regional.
            unsubs.push(
              onSnapshot(qCidades, (sCids) => {
                // Explicação: Abre o canal de escuta em tempo real para as cidades da regional.
                if (!isMounted) return; // Explicação: Cancela se o usuário saiu da tela.
                const todasCidades = sCids.docs.map((d) => ({
                  id: d.id,
                  nome: d.data().nome,
                })); // Explicação: Transforma as cidades em objetos com ID e Nome.
                const filtradas = isComissao
                  ? todasCidades.sort((a, b) => a.nome.localeCompare(b.nome))
                  : todasCidades.filter((cid) => cid.id === userData?.cidadeId); // Explicação: Membros da comissão veem todas em ordem alfabética; gestores de cidade veem apenas a sua cidade.
      ...
      // Explicação: Captura qualquer erro inesperado de conexão.
    ...
    }; // Explicação: Função de limpeza que desliga todos os canais de banco ao sair della tela, evitando vazamento de memória.
  }, [activeRegionalId, isComissao, level]); // Explicação: Gatilhos que disparam este bloco se sofrerem alterações.

  useEffect(() => {
    // Explicação: Monitor responsável por escutar os detalhes profundos da igreja selecionada (Instrumentos e Métodos).
    ...
    const comumAindaValida = sharedData.comunsDaRegional.some(
      (c) => c.id === comumIdEfetivo,
    ); // Explicação: Checa se a igreja selecionada ainda existe e está disponível na lista ativa.
    if (!comumAindaValida && level !== "gem_local") {
      // Explicação: Se a igreja sumiu ou ficou inválida e não for um secretário fixo travado.
    ...
    const unsub = onSnapshot(docRefFocado, (docSnap) => {
      // Explicação: Abre o canal em tempo real para monitorar os dados cadastrais da igreja comum selecionada.
      if (docSnap.exists() && isMounted)
        setSelectedComum({ id: docSnap.id, ...docSnap.data() }); // Explicação: Atualiza em tempo real as informações de endereço e horários da igreja comum.
    });
    ...
    const unsubInst = onSnapshot(
      collection(db, "comuns", comumIdEfetivo, "instrumentos_config"),
      (sInst) => {
        // Explicação: Abre canal com a subcoleção interna que diz quais instrumentos estão ativos ou desativados nesta igreja.
        ...
        instrumentosOrdenadosLote.sort(
          (a, b) => (a.ordem || 0) - (b.ordem || 0),
        ); // 🚀 FIAÇÃO DO FILTRO DND: Organiza a fila da orquestra de forma crescente baseando-se nas cadeiras numéricas!

        setSharedData((prev) => ({
          ...prev,
          instruments: instrumentosOrdenadosLote,
        })); // Explicação: Despacha a grade reordenada com sucesso para a memória compartilhada do app.
      },
    );
    return () => {
      isMounted = false;
      unsub();
      unsubInst();
    }; // Explicação: Desliga os ouvintes de detalhes da igreja ao trocar de seleção ou fechar a página.
  ...
  if (loading && isRegionalCidade)
    return (
      // Explicação: Renderiza um visual de carregamento premium em formato de spinner enquanto checa os poderes territoriais.
      <div className="h-[60vh] flex flex-col items-center justify-center gap-4">
        {" "}
        {/* Explicação: Centraliza o carregamento verticalmente ocupando 60% da altura da tela. */}
        <div className="w-8 h-8 border-4 border-blue-600 border-t-transparent rounded-full animate-spin"></div>{" "}
        {/* Explicação: Círculo azul giratório com animação de spin fluida. */}
        <div className="font-black italic text-slate-400 text-[10px] uppercase tracking-widest text-center">
          Sincronizando Jurisdição...
        </div>{" "}
        {/* Explicação: Texto descritivo elegante em letras maiúsculas e espaçadas. */}
      </div>
    );

  return (
    // Explicação: Inicia a renderização do layout físico da página de Ajustes.
    <div className="space-y-5 pb-40 px-4 pt-4 max-w-md mx-auto text-left font-sans">
      {" "}
      {/* Explicação: Margens mobile confortáveis, largura máxima controlada para não esticar em tablets e alinhamento à esquerda. */}
      ...
      {isRegionalCidade && ( // Explicação: Só exibe seletores de GPS se o usuário tiver poder regional para gerenciar mais de uma localidade.
        <div
          key={`pills-container-${activeRegionalId}`}
          className="grid grid-cols-1 gap-2.5 pb-4 border-b border-slate-100"
        >
          {" "}
          {/* Explicação: Grid empilhado verticalmente com espaçamento premium de mercado no topo absoluto. */}
          {/* Pill Cidade */}
          <div
            className={`flex items-center gap-2.5 bg-white/60 backdrop-blur-sm p-3 rounded-2xl border border-slate-200 shadow-xs transition-all ${!isComissao || sharedData.cidades.length === 0 ? "opacity-50 pointer-events-none" : ""}`}
          >
            {" "}
            {/* Explicação: Card com fundo branco semi-transparente, efeito vidro, bordas arredondadas e toque confortável. Fica cinza se for Secretário Local travado. */}
            <MapPin size={13} className="text-indigo-600 shrink-0" />{" "}
            {/* Explicação: Ícone de alfinete de localização na cor azul/indigo de alta legibilidade. */}
            <select // Explicação: Menu de seleção da Cidade ativa.
              ...
              onChange={(e) => {
                // Explicação: Executa em cascata ao mudar a cidade.
                const city = sharedData.cidades.find(
                  (c) => c.id === e.target.value,
                ); // Explicação: Localiza o objeto completo da cidade escolhida.
            ...
            >
              <option value="">
                {sharedData.cidades.length === 0
                  ? "NENHUMA CIDADE CADASTRADA"
                  : isComissao
                    ? "SELECIONE A CIDADE..."
                    : userData?.cidadeNome || "SUA CIDADE ATIVA"}
              </option>{" "}
              {/* Explicação: Opção padrão inteligente por extenso adaptada ao nível do crachá do irmão. */}
              {sharedData.cidades.map((c) => (
                <option key={c.id} value={c.id}>
                  {c.nome}
                </option>
              ))}{" "}
              {/* Explicação: Varre a lista de cidades da memória e cria as linhas de option no menu. */}
            </select>{" "}
            {/* Explicação: Fecha o seletor de cidade. */}
          </div>{" "}
          {/* Explicação: Fecha a caixinha visual da cidade. */}
          {/* Pill Localidade */}
          <div
            className={`flex items-center gap-2.5 bg-slate-950 p-3 rounded-2xl shadow-md border border-white/5 ${!selectedCity && sharedData.cidades.length > 0 ? "opacity-50 pointer-events-none" : ""}`}
          >
            {" "}
            {/* Explicação: Card de alto destaque estético (fundo preto ardósia, sombra profunda, cantos arredondados). Bloqueado até que uma cidade seja escolhida. */}
            <Home size={13} className="text-indigo-400 shrink-0" />{" "}
            {/* Explicação: Ícone pequeno de igreja comum na cor azul clara de alto contraste. */}
            <select // Explicação: Menu de seleção da Igreja Comum ativa para manutenção.
              ...
              onChange={(e) => {
                // Explicação: Executa ao escolher uma igreja.
                const com = sharedData.comunsDaRegional.find(
                  (c) => c.id === e.target.value,
                ); // Explicação: Acha a igreja correspondente na memória da lista.
            ...
            >
              <option value="" className="text-slate-900">
                {" "}
                {/* Explicação: Linha padrão explicativa com texto escuro para legibilidade interna. */}
                {sharedData.cidades.length === 0
                  ? "AGUARDANDO SELEÇÃO DE CIDADE"
                  : sharedData.comunsDaRegional.filter((c) =>
                        selectedCity ? c.cidadeId === selectedCity.id : true,
                      ).length === 0
                    ? "NENHUMA IGREJA DISPONÍVEL"
                    : "SELECIONE A LOCALIDADE POR EXTENSO..."}
              </option>{" "}
              {/* Explicação: Fecha a linha padrão. */}
            ...
            {/* Explicação: Fecha o seletor de localidade. */}
            <ChevronDown
              size={13}
              className="text-white/20 ml-auto shrink-0"
            />{" "}
            {/* Explicação: Seta discreta indicando que a caixinha preta expande ao tocar. */}
          </div>{" "}
          {/* Explicação: Fecha a caixinha visual de localidade. */}
        </div>
      )}
      ...
      {isComissao && ( // Explicação: Matriz de Permissão: Renderiza os botões de administração de base apenas para quem possui crachá Master ou Comissão.
        <div className="space-y-2 animate-in fade-in duration-300">
          <div className="px-1 flex flex-col text-left">
            {" "}
            {/* Explicação: Pequeno rótulo de seção organizacional para guiar o usuário sem tutoriais. */}
            <span className="text-[8px] font-black text-indigo-600 uppercase tracking-widest italic leading-none">
              Nível: Administration Regional
            </span>
          </div>
          <div className="grid grid-cols-2 gap-2">
            {" "}
            {/* Explicação: Interface compacta de mercado dividindo as ações em 2 colunas menores e organizadas. */}
            <MenuButton
              icon={<Briefcase size={15} />}
              title="Cargos & Funções"
              onClick={() => setActiveModal("global")}
            />{" "}
            {/* Explicação: Botão compacto que aciona o modal autônomo de cargos. */}
            <MenuButton
              icon={<MapPin size={15} />}
              title="Gestão de Cidades"
              onClick={() => setActiveModal("cities")}
            />{" "}
            {/* Explicação: Botão compacto que aciona o modal autônomo de cidades. */}
          </div>
          ...
          <div className="grid grid-cols-1 pt-0.5">
            <MenuButton
              icon={<Layers size={15} />}
              title="Níveis de Teste da Lista Suspensa"
              moduleName="Regional"
              onClick={() => setActiveModal("niveis_teste")}
            />
          </div>
        </div>
      )}
      ...
      {isRegionalCidade && ( // Explicação: Renderiza o gerenciador de igrejas para quem cuida de cidades ou sub-regiões.
        <div className="space-y-2 pt-1 animate-in fade-in duration-300">
          {isComissao && <div className="h-px bg-slate-100 mx-1" />}{" "}
          {/* Explicação: Linha divisória sutil para não embolar as seções visuais no celular. */}
          <div className="px-1 flex flex-col text-left">
            <span className="text-[8px] font-black text-indigo-600 uppercase tracking-widest italic leading-none">
              Nível: Gestão de Infraestrutura
            </span>
          </div>
          <div className="grid grid-cols-1">
            {" "}
            {/* Explicação: Botão expandido em largura inteira para gerenciamento de obras de novas igrejas comuns. */}
            <MenuButton
              icon={<Building2 size={15} />}
              title="Manutenção e Criação de Comuns"
              onClick={() => setActiveModal("churches_mgr")}
            />
          </div>
        </div>
      )}
      ...
      <div className="space-y-3 pt-1">
        {comumIdEfetivo &&
        sharedData.comunsDaRegional.some((c) => c.id === comumIdEfetivo) ? ( // Explicação: Chegagem Crítica de Segurança: Só renderiza as configurações operacionais se houver uma igreja legitimamente selecionada no topo.
          <div className="space-y-2.5 animate-in fade-in slide-in-from-top-3 duration-500">
            {" "}
            {/* Explicação: Agrupador das configurações da igreja com animação premium. */}
            <div className="h-px bg-slate-100 mx-1" />{" "}
            {/* Explicação: Divisor visual estético. */}
            <div className="px-1 leading-none flex flex-col text-left">
              {" "}
              {/* Cabeçalho interno informando a manutenção ativa sem cortes de texto. */}
              <p className="text-[8px] font-black text-emerald-600 uppercase tracking-widest italic leading-none">
                Zeladoria Ativa na Comum:
              </p>
              <h3 className="text-[13px] font-black text-slate-950 uppercase italic tracking-tight leading-normal mt-1.5 whitespace-normal break-words">
                {" "}
                {/* Explicação: Nome exibido por inteiro sem reticências feias, quebrando linha se necessário. */}
            ...
            {isGemLocal && ( // Explicação: ORDEM OPERACIONAL SOLICITADA PELO USUÁRIO (Segurança ➔ Cadastro ➔ Ministério Local ➔ Corpo Orquestral ➔ Orquestra).
              <div className="grid grid-cols-1 gap-2 pt-0.5">
                {" "}
                {/* Explicação: Grade vertical compacta exibindo todas as descrições inteiras sem abreviações. */}
                {/* 1. SEGURANÇA */}
                <MenuButton
                  icon={<Users size={15} />}
                  title="Controle de Acessos & Portaria Local"
                ...
                <MenuButton
                  icon={<Home size={15} />}
                  title="Alterar Endereço e Dias de Cultos"
                  moduleName="Cadastro"
                  onClick={() => setActiveModal("church")}
                />
                {/* 3. MINISTÉRIO LOCAL */}
                <MenuButton
                  icon={<ClipboardList size={15} />}
                  title="Corpo Ministerial & Administração Eclesiástica"
                  moduleName="Ministério Local"
                  onClick={() => setActiveModal("ministerio_local")}
                />
                {/* 4. CORPO ORQUESTRAL */}
                <MenuButton
                  icon={<Contact size={15} />}
                  title="Cadastro de Músicos & Chamada de Presença"
                  moduleName="Corpo Orquestral"
                  onClick={() => setActiveModal("corpo_orquestral")}
                />
                {/* 5. ORQUESTRA */}
                <MenuButton
                  icon={<Music size={15} />}
                  title="Configurar Instrumentação & Ativação de Naipes"
                  moduleName="Orquestra"
                  onClick={() => setActiveModal("orchestra")}
                />
              </div>
            )}
          </div>
          ...
          <div className="p-8 text-center bg-white rounded-[2.5rem] border border-dashed border-slate-200 animate-in fade-in zoom-in duration-300 shadow-xs">
            {" "}
            {/* Explicação: Card branco, contorno pontilhado cinza discreto e animação suave de zoom. */}
            <div className="w-12 h-12 bg-slate-50 text-slate-300 rounded-full flex items-center justify-center mx-auto mb-3">
              {" "}
              {/* Explicação: Moldura para o ícone. */}
              <MapPin size={24} />{" "}
              {/* Explicação: Ícone grande de alfinete de mapa indicando foco de localização geográfica. */}
            </div>{" "}
            {/* Explicação: Fecha a moldura do ícone. */}
            <p className="text-slate-400 font-black uppercase italic text-[9px] tracking-widest leading-relaxed whitespace-normal px-2">
              {" "}
              {/* Explicação: Texto de instrução por extenso sem cortes. */}
              Jurisdição Territorial Indefinida <br />{" "}
              <span className="text-[8px] opacity-75 normal-case font-bold text-slate-400 block mt-1">
                Por favor, utilize os seletores de filtros localizados no topo
                da tela para selecionar uma localidade activa e liberar os
                painéis de ajustes.
              </span>{" "}
              {/* Explicação: Explica de forma interativa o que fazer. */}
            </p>{" "}
            {/* Explicação: Fecha o texto de instrução. */}
          </div> // Explicação: Fecha o card de orientação.
        )}
      </div>{" "}
      {/* Explicação: Fecha o bloco operacional de atividades. */}
            ...
            >
              ...
              <div className="p-5 border-b border-slate-100 flex items-center justify-between shrink-0 text-left">
                <div className="leading-tight text-left pr-2 flex-1 min-w-0">
                  <p className="text-[8px] font-black text-indigo-600 uppercase tracking-widest italic mb-0.5">
                    Módulo Configurações
                  </p>
                  <h3 className="text-[12px] font-black text-slate-900 uppercase italic tracking-tight whitespace-normal break-words leading-snug">
                    {" "}
                    {/* Explicação: Título do modal por extenso sem cortes in elipse. */}
                    {activeModal === "global" &&
                      "Painel de Referências: Cargos & Ministérios"}
                    {activeModal === "cities" &&
                      "Geografia Regional: Gestão de Cidades Ativas"}
                    {activeModal === "churches_mgr" &&
                      "Infraestrutura: Manutenção de Comuns"}
                    {activeModal === "users" &&
                      "Segurança Operacional: Acesso & Portaria Local"}
                    {activeModal === "church" &&
                      "Zeladoria de Dados Cadastrais da Comum"}
                    {activeModal === "ministerio_local" &&
                      "Painel da Comum: Corpo Ministerial Eclesiástico"}
                    {activeModal === "corpo_orquestral" &&
                      "Painel da Comum: Cadastro Nominal do Corpo Orquestral"}
                    {activeModal === "orchestra" &&
                      "Configuração de Orquestra & Ativação de Naipes"}
                    {activeModal === "niveis_teste" &&
                      "Painel Regional: Gestão de Níveis de Teste"}
                  </h3>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-5 bg-slate-50/40 no-scrollbar">
                {activeModal === "global" && (
                  <ModuleGlobal
                    ...
                    onConfirmDelete={confirmarExclusaoNativa}
                  />
                )}
                {activeModal === "cities" && (
                  <ModuleCities
                ...
                {activeModal === "ministerio_local" && (
                  <ModuleMinistryLocal comumId={comumIdEfetivo} />
                )}
                {activeModal === "corpo_orquestral" && (
                  <ModuleOrchestraBody
                    comumId={comumIdEfetivo}
                ...
                {activeModal === "niveis_teste" && (
                  <ModuleTestLevels regionalId={activeRegionalId} />
                )}
              </div>
            </motion.div>
          </div>
...
const MenuButton = ({ icon, title, moduleName, onClick }) => {
      ...
      <div className="flex items-center gap-3 text-left leading-none min-w-0 flex-1">
        ...
        <div className="min-w-0 flex-1 leading-tight">
          {moduleName /* Explicação: Pequena tag superior em itálico azul indicando o escopo funcional */ && (
            <p className="text-[7px] font-black text-indigo-600 uppercase mb-0.5 tracking-wider italic opacity-85 leading-none">
              {moduleName}
            </p>
          )}
          <h3 className="text-[11px] font-black text-slate-800 uppercase italic tracking-tight whitespace-normal break-words leading-snug">
            {title}
          </h3>{" "}
          {/* Explicação: Exibição textual por extenso, sem cortes in elipse para total higiene visual. */}
        </div>
      </div>
      <ChevronRight
        size={13}
        className="text-slate-300 shrink-0 ml-2 group-hover:text-slate-400 transition-colors"
      />
    </button>
  );
};
