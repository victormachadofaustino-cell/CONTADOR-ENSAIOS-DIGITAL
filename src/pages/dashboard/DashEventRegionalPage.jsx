...
import { 
  ...
  Camera, BookOpen, ListMusic, LayoutGrid, Scale // [Funcionamento]: Importa os desenhos dos ícones visuais do sistema.
} from 'lucide-react'; // [Funcionamento]: Biblioteca oficial de ícones premium vetorizados.
import { pdfEventRegionalService } from '../../services/pdfEventRegionalService'; // [Funcionamento]: Importa o serviço que gera o arquivo impresso em PDF.
import { whatsappService } from '../../services/whatsappService'; // [Funcionamento]: Importa o serviço que formata o texto para WhatsApp.
import { useAuth } from '../../context/AuthContext'; // [Funcionamento]: Puxa o contexto de login para ler o crachá do usuário logado.
import { db, doc, getDoc } from '../../config/firebase'; // [Funcionamento]: Conecta ao banco de dados do Firestore.
...
import { hasPermission } from '../../config/permissions'; // [Funcionamento]: Importa o cérebro que valida as permissões de segurança.
...
const DashEventRegionalPage = ({ counts, ataData, isAdmin, eventId }) => { // [Funcionamento]: Inicia o componente Maestro do painel regional.
  ...
  const stats = useMemo(() => { // [Funcionamento]: useMemo retém os cálculos em cache para economizar processamento e bateria.
    const totals = { // [Funcionamento]: Inicializa o objeto limpo com todos os acumuladores de dados do ensaio.
      ...
      localidadesUnicas: new Set(), visitasMusicos: 0 // [Funcionamento]: Guarda localidades únicas sem repetições.
    }; // [Funcionamento]: Fim da declaração inicial da árvore de estatísticas.

    if (counts) { // [Funcionamento]: Verifica se os dados numéricos de portaria foram carregados do banco.
      Object.entries(counts).forEach(([id, data]) => { // [Funcionamento]: Passa uma varredura em cada registro do mapa de contagem.
        ...
        const ehIrmandadeOuCoral = section.includes('CORAL') || section.includes('IRMANDADE') || saneId.includes('coral') || saneId.includes('irmandade') || saneId === 'irmas' || saneId === 'irmaos'; // [Funcionamento]: Filtra se pertence aos bancos ou ao coro.

        if (ehIrmandadeOuCoral) { // [Funcionamento]: Bloco de acúmulo exclusivo da congregação sentada.
          totals.irmaos += valIrmaos || (saneId === 'irmaos' ? effectiveTotal : 0); // [Funcionamento]: Adiciona ao totalizador de vozes masculinas.
          totals.irmas += valIrmas || (saneId === 'irmas' ? effectiveTotal : 0); // [Funcionamento]: Adiciona ao totalizador de vozes femininas.
          if (saneId === 'coral' && !counts.irmas && !counts.irmaos) { // [Funcionamento]: Tratamento de fallback para o canal legado de coral.
        ...
        } else if (section.includes('ORGANISTA') || saneId.includes('orgao') || saneId.includes('org')) { // [Funcionamento]: Triagem do banco de órgão eletrônico.
        ...
        } else { // [Funcionamento]: Caso seja músico de fileira (sopro, bocal ou arco).
          ...
          if (section === 'CORDAS' || ['vln', 'vla', 'vcl', 'violino', 'viola', 'violoncelo'].includes(saneId)) totals.cordas += effectiveTotal; // [Funcionamento]: Soma instrumentos do naipe de cordas.
          else if (section.includes('SAX')) totals.saxofones += effectiveTotal; // [Funcionamento]: Soma instrumentos do naipe de saxofones.
          else if (section.includes('MADEIRA Mum') || ['flt', 'clt', 'oboe', 'fgt'].includes(saneId)) totals.madeiras += effectiveTotal; // [Funcionamento]: Soma instrumentos do naipe de madeiras.
          else if (section.includes('METAI') || ['tpt', 'tbn', 'trp', 'euf', 'tub'].includes(saneId)) totals.metais += effectiveTotal; // [Funcionamento]: Soma instrumentos do naipe de metais.
          else if (section === 'TECLAS' || saneId === 'acordeon' || saneId === 'acd'.includes(saneId)) totals.teclas += effectiveTotal; // [Funcionamento]: Soma os acordeonistas presentes.
    ...
    const processarPessoas = (lista, isVisitante = false) => { // [Funcionamento]: Função interna que varre e classifica o oficialato nominal da ata.
      if (!lista || !Array.isArray(lista)) return; // [Funcionamento]: Aborta caso o vetor nominal venha vazio do banco.
      lista.forEach(p => { // [Funcionamento]: Percorre item por item da lista de pessoas.
        ...
        if (cargo === 'Ancião') { if (isVisitante) totals.ancianosVisitas++; else totals.ancianosCasa++; } // [Funcionamento]: Acumula os blocos de Anciães.
        else if (cargo === 'Diácono') { if (isVisitante) totals.diaconosVisitas++; else totals.diaconosCasa++; } // [Funcionamento]: Acumula os blocos de Diáconos.
        else if (cargo === 'Cooperador do Ofício' || cargo === 'Cooperador do Ofio') { if (isVisitante) totals.coopOficioVisitas++; else totals.coopOficioCasa++; } // [Funcionamento]: Acumula Cooperadores de Ofício.
        else if (cargo === 'Cooperador RJM' || cargo === 'Cooperador de Jovens e Menores') { if (isVisitante) totals.coopJovensVisitas++; else totals.coopJovensCasa++; } // [Funcionamento]: Acumula Cooperadores de Jovens.
        else if (cargo === 'Encarregado Regional') { if (isVisitante) totals.encRegionalVisitas++; else totals.encRegionalCasa++; } // [Funcionamento]: Acumula os Encarregados Regionais de música.
        else if (cargo === 'Encarregado Local') { if (isVisitante) totals.encLocalVisitas++; else totals.encLocalCasa++; } // [Funcionamento]: Acumula os Encarregados Locais de orquestra.
        else if (cargo === 'Examinadora') { if (isVisitante) totals.examinadorasVisitas++; else totals.examinadorasCasa++; } // [Funcionamento]: Acumula as Examinadoras de organistas.
        const localidade = (p.comum || p.cidadeUf || p.bairro || "").toUpperCase(); // [Funcionamento]: Puxa a igreja ou território de origem dele.
    ...
    if (ataData?.partes) { // [Funcionamento]: Se existirem hinos divididos por blocos na ata.
      totals.hinos = ataData.partes.reduce((acc, p) => 
        acc + (p.hinos?.filter(h => h && h.trim() !== '').length || 0), 0); // [Funcionamento]: Soma a quantidade real de hinos preenchidos.
    } // [Funcionamento]: Fim da validação de hinologia.

    totals.ministerio_total = (totals.ancianosCasa + totals.ancianosVisitas) + (totals.diaconosCasa + totals.diaconosVisitas) + (totals.coopOficioCasa + totals.coopOficioVisitas) + (totals.coopJovensCasa + totals.coopJovensVisitas) + (totals.encLocalCasa + totals.encLocalVisitas) + (totals.examinadorasCasa + totals.examinadorasVisitas); // [Funcionamento]: Totaliza o corpo de liderança local e de visitas.
    ...
    totals.geral = totals.orquestra + totals.irmandade + totals.ministerio_total + totals.encRegional; // [Funcionamento]: Operação final que dita o Total Geral de almas do ensaio.
  ...
  const dataPorExtenso = useMemo(() => { // [Funcionamento]: Converte datas técnicas em formato extenso tradicional (ex: 01 de Março de 2026).
    try {
      ...
      if (dataAlvo) {
        ...
        if (!isNaN(dataObjeto.getTime())) { // [Funcionamento]: Se for uma data real e válida.
    ...
    return new Date().toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' }); // [Funcionamento]: Fallback seguro com a data de hoje.
  }, [ataData]); // [Funcionamento]: Monitora alterações baseadas na ata.

  // v6.9: EXTRAÇÃO DA PALAVRA MANTIDA PARA TRANSFERÊNCIA VIA PROPS
  const palavraLidaData = useMemo(() => { // [Funcionamento]: Organiza os dados da Palavra Lida do Firestore para as telas filhas.
    if (ataData?.palavra) {
      return {
    ...
    return { livro: '---', capitulo: '---', verso: '---', assunto: 'Palavra da Santa Escritura exortada.' }; // [Funcionamento]: Fallback caso esteja em branco.
  }, [ataData]); // [Funcionamento]: Reage se a ata sofrer updates.

  // v6.9: FILTRO DA LISTA DE HINOS DO ARRAY DO FIRESTORE
  const hinosEnsaiadosLista = useMemo(() => { // [Funcionamento]: Limpa strings vazias do vetor de hinos ensaiados lançado na portaria.
  ...
  const totalPrincipais = stats.cordas + (stats.madeiras + stats.saxofones) + stats.metais; // [Funcionamento]: Soma absoluta das três principais famílias orquestrais.
  const getPerc = (val, total) => total > 0 ? ((val / total) * 100).toFixed(1) : "0.0"; // [Funcionamento]: Função utilitária de cálculo percentual com uma casa decimal fixada.

  const handleShareAlimentacao = () => { // [Funcionamento]: Constrói e envia os totais do lanche da portaria via WhatsApp.
    const msg = whatsappService.obterTextoAlimentacao(ataData, stats); // [Funcionamento]: Compila o texto limpo da Copa.
    if (navigator.share) navigator.share({ text: msg }).catch(() => {}); // [Funcionamento]: Aciona o menu nativo de partilha do smartphone.
    else { navigator.clipboard.writeText(msg); toast.success("Copiado!"); } // [Funcionamento]: Fallback salvando na área de transferência.
  }; // [Funcionamento]: Fim da rotina de lanche.

  const handleShareEstatistico = () => { // [Funcionamento]: Envia o resumo estatístico formal do ensaio regional para o WhatsApp.
    const msg = whatsappService.obterTextoEstatistico({ ...ataData, counts }, stats); // [Funcionamento]: Compila a mensagem estruturada.
    if (navigator.share) navigator.share({ text: msg }).catch(() => {}); // [Funcionamento]: Dispara partilha nativa do celular.
    else { navigator.clipboard.writeText(msg); toast.success("Resumo Estatístico Copiado!"); } // [Funcionamento]: Fallback copiando o texto.
  }; // [Funcionamento]: Fim da rotina estatística.

  const handleGeneratePDF = async () => { // [Funcionamento]: Puxa os dados da comarca sede e dispara a compilação do relatório impresso em PDF.
    ...
    try {
      ...
      pdfEventRegionalService.generateAtaRegional(stats, ataData, userData, counts, sedeData); // [Funcionamento]: Constrói o PDF.
    ...
    } catch (error) { toast.dismiss(loadingToast); toast.error("Erro ao gerar PDF."); } // [Funcionamento]: Captura e trata falhas bloqueando travamentos de UI.
  }; // [Funcionamento]: Fim da rotina de PDF.

  // v6.9: DISPARADOR DO FOTÓGRAFO TRAZIDO PARA O CORPO DO MAESTRO E PASSADO À SCREENRESUMO
  const handleGenerateImage = async () => { // [Funcionamento]: Transforma o palco oculto invisível em imagem PNG de alta definição para envio instantâneo.
    if (!cardImagemRef.current) return; // [Funcionamento]: Trava de segurança caso a âncora visual não esteja acoplada na árvore.
    const loadingToast = toast.loading("📸 Gerando Relatório Solene em Imagem..."); // [Funcionamento]: Notifica o início da fabricação da mídia.
    ...
    try {
      ...
      const dataUrl = await htmlToImage.toPng(cardImagemRef.current, { // [Funcionamento]: Converte o HTML em string de dados PNG pura.
      ...
      const file = new File([blob], `Relatorio_Ensaio_${ataData?.cidadeNome || 'Regional'}.png`, { type: 'image/png' }); // [Funcionamento]: Carimba o nome dinâmico e o formato PNG no arquivo físico.

      toast.dismiss(loadingToast); // [Funcionamento]: Remove o aviso de carregamento da tela.

      if (navigator.canShare && navigator.canShare({ files: [file] })) { // [Funcionamento]: Se o celular possuir o motor moderno de compartilhamento de arquivos.
      ...
      } else { // [Funcionamento]: Caso o navegador seja antigo ou esteja rodando em desktop sem API de share.
    ...
    } catch (error) { // [Funcionamento]: Intercepta quebras catastróficas do html-to-image.
  ...
  const podeExportar = hasPermission(userData, 'generate_report', ataData?.scope); // [Funcionamento]: Guarda o booleano de validação de nível de acesso.

  return ( // [Funcionamento]: Palco de renderização visual definitivo do Maestro móvel.
    <div className="space-y-4 text-slate-900 pb-24 max-w-md mx-auto px-2 font-sans">
      ...
      <div className="flex gap-2 items-stretch w-full select-none">
        ...
        <div className="bg-slate-950 px-4 py-2.5 rounded-[1.8rem] shadow-2xl border border-white/5 flex items-center justify-between flex-1 min-w-0 text-left relative overflow-hidden">
          ...
          <div className="leading-tight shrink-0">
            <p className="text-[7px] font-black text-amber-500 uppercase tracking-[0.2em] mb-0.5 italic">Alimentação</p>
            <h3 className="text-lg font-[1000] text-white uppercase italic tracking-tighter">Resumo</h3>
          </div>
          ...
          <div className="flex items-center gap-3 border-l border-white/10 pl-3 h-8 shrink-0">
            <div className="text-center leading-none">
              <p className="text-[7px] font-black text-slate-500 uppercase italic mb-0.5">Orq</p>
              <p className="text-[11px] font-black text-white tracking-tight">{stats.orquestra}</p>
            </div>
            <div className="text-center leading-none">
              <p className="text-[7px] font-black text-slate-500 uppercase italic mb-0.5">Coral</p>
              <p className="text-[11px] font-black text-white tracking-tight">{stats.irmandade}</p>
            </div>
            {podeExportar && ( // [Funcionamento]: Verifica se o usuário tem crachá com acesso para emitir partilhas.
              ...
              >
                <Share2 size={16} /> {/* [Funcionamento]: Ícone verde premium vetorizado de compartilhamento. */}
              </button>
            )}
          </div>
        </div>
        ...
        {podeExportar && ( // [Funcionamento]: Exibe o botão de relatório impresso se ele possuir nível de acesso.
          ...
          >
            <FileText size={16} className="text-blue-600" /> {/* [Funcionamento]: Desenho do arquivo técnico de PDF. */}
            <span className="font-extrabold tracking-tight mt-0.5">PDF</span> {/* [Funcionamento]: Inscrição em caixa alta original. */}
          </button>
        )}
      </div>
      ...
      <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200/50 select-none">
        ...
        >
          <LayoutGrid size={12} /> Geral {/* [Funcionamento]: Ícone e texto da Aba 1. */}
        </button>
        <button 
        ...
        >
          <Briefcase size={12} /> Ministério {/* [Funcionamento]: Ícone e texto da Aba 2. */}
        </button>
        <button 
        ...
        >
          <Scale size={12} /> Equilíbrio {/* [Funcionamento]: Ícone e texto da Aba 3. */}
        </button>
        <button 
        ...
        >
          <PieChart size={12} /> Resumo {/* [Funcionamento]: Ícone e texto da Aba 4. */}
        </button>
      </div>

      {/* 🎛 cache-PROP-BRIDGE: DISTRIBUIDOR DE TELAS POR PONTE DE MEMÓRIA DE CUSTO FIRESTORE ZERO */}
      <div className="w-full">
        {currentScreen === 'geral' && <ScreenGeral stats={stats} />} {/* [Funcionamento]: Aciona a UI de cartões totais injetando a matemática. */}
        
        {currentScreen === 'ministerio' && <ScreenMinisterio stats={stats} ataData={ataData} />} {/* [Funcionamento]: Aciona a UI de acordeões nominais do ministério. */}
        
        {currentScreen === 'equilibrio' && <ScreenEquilibrio stats={stats} getPerc={getPerc} />} {/* [Funcionamento]: Aciona a UI de engenharia de som e naipes. */}
