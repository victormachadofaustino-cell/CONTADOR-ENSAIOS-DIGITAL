...
export const hasPermission = (user, action, context = null) => {
  ...
  if (action === 'gerenciar_musicos') { // Explicação: Valida se o secretário pode mexer nas fichas fixas de músicos.
    if (user.accessLevel === ROLES.COMISSAO) { // Explicação: Se for Comissão, valida se a igreja comuns alvo pertence à sua Regional.
    ...
    if (user.accessLevel === ROLES.CIDADE) { // Explicação: Se for Encarregado de Cidade, valida os limites da comarca municipal.
    ...
    if (user.accessLevel === ROLES.GEM) { // Explicação: GEM Local pode gerenciar músicos se a igreja comuns for estritamente a dele.
  ...
  if (action === 'gerenciar_comum') { // Explicação: Valida quem altera dados físicos da igreja comum na aba de Ajustes.
    if (config.canManageComum === true) return true; // Explicação: Nível master passa direto.
    if (config.canManageComum === 'regional_scope') { // Explicação: Comissão altera se a comum pertencer à sua regional.
      return context && context.regionalId === user.regionalId; // Explicação: Retorna verdadeiro se as chaves de regional baterem.
    }
    if (config.canManageComum === 'cidade_scope') { // Explicação: Encarregado de cidade altera se a comum pertencer à sua cidade.
      return context && context.cidadeId === user.cidadeId; // Explicação: Retorna verdadeiro se as comarcas municipais baterem.
    }
    if (config.canManageComum === 'local_only') { // Explicação: GEM altera se a comum for rigorosamente a dele de cadastro.
  ...
  if (action === 'gerenciar_ministerio') { // Explicação: Valida quem gerencia as fichas nominais da lista do altar.
    if (config.canManageMinisterio === true) return true; // Explicação: Permissão master universal.
    if (config.canManageMinisterio === 'regional_scope') { // Explicação: Comissão gerencia se a comum for da regional dele.
      return context && context.regionalId === user.regionalId; // Explicação: Confere o elo regional.
    }
    if (config.canManageMinisterio === 'cidade_scope') { // Explicação: Encarregado de cidade gerencia se a comum for da sua cidade.
      return context && context.cidadeId === user.cidadeId; // Explicação: Confere a jurisdição da comarca.
    }
    if (config.canManageMinisterio === 'local_only') { // Explicação: GEM gerencia se a comum for estritamente a dele.
  ...
  if (action === 'gerenciar_instrumentos_config') { // Explicação: Valida quem altera as grades orquestrais permitidas na casa.
    if (config.canManageInstruments === true) return true; // Explicação: Chave mestre liberada.
    if (config.canManageInstruments === 'regional_scope') { // Explicação: Comissão altera se a comum for da sua regional.
      return context && context.regionalId === user.regionalId; // Explicação: Confere o elo regional do banco.
    }
    if (config.canManageInstruments === 'cidade_scope') { // Explicação: Encarregado de cidade altera se a comum for da sua cidade.
      return context && context.cidadeId === user.cidadeId; // Explicação: Confere a jurisdição municipal do banco.
    }
    if (config.canManageInstruments === 'local_only') { // Explicação: GEM altera se a comum for estritamente a dele.
  ...
  if (action === 'criar_ensaio_regional') { // Explicação: Gatilho que valida se o obreiro pode acionar o formulário de ensaio regional macro.
  ...
  if (action === 'delete_event') {
    ...
    if (user.accessLevel === ROLES.CIDADE) { // Explicação: Encarregado de cidade apaga se o evento pertencer à sua cidade de direito.
      return context && context.cidadeId === user.cidadeId; // Explicação: Valida se as cidades do gestor e do ensaio casam.
    }
    if (config.canDeleteEvent === 'local_only') { // Explicação: Se a regra do crachá disser que só mexe nos locais da própria igreja.
  ...
  if (action === 'manage_team') {
    ...
    if (config.canManageTeam === 'local_only') { // Explicação: Se for restrito ao ambiente local de direito da casa.
      if (typeof context === 'object' && context !== null) {
  ...
  if (action === 'generate_report') {
    ...
    if (config.canGenerateReport === 'local_only') { // Explicação: Se for restrito ao ambiente local da respectiva casa comuns.
      if (typeof context === 'object' && context !== null) {
