...
const Header = ({ onChurchChange, onRegionalChange }) => {
  // Início do componente do cabeçalho.
  ...
  useEffect(() => {
    ...
    if (isMaster) {
      // Se for Master, começa a vigiar os chamados.
      const q = query(collection(db, "tickets_global")); // Pergunta ao banco sobre os chamados.
      unsubTickets = onSnapshot(q, (snap) => {
        // Escuta se chegar chamado novo.
        setPendingTickets(
          snap.docs.filter((d) => d.data().status === "pendente").length,
        ); // Conta os pendentes.
      });
    }
    ...
    return () => {
      unsubReg();
      unsubTickets();
    }; // Desliga tudo ao fechar para economizar bateria.
  ...
  const handleTrocaRegional = (id) => {
    // Função disparada ao escolher uma nova regional no menu.
    ...
    toast.success("Jurisdição Alterada", {
  ...
  const handleSync = () => {
    // Função para atualizar os dados na marra.
        ...
        <div className="flex flex-col items-start leading-none text-left min-w-0 flex-1">
          ...
          <div className="relative group w-full max-w-[200px]">
            ...
            >
              ...
              {listaRegionais.map(
                (
                  reg, // Cria uma opção para cada regional cadastrada.
                ) => (
                  <option
                    key={reg.id}
                    value={reg.id}
                    className="text-slate-950 font-sans not-italic font-bold text-xs uppercase"
                  >
                    {reg.nome}
                  </option>
                ),
            ...
            {isMaster && ( // Coloca a setinha indicando que é um menu de escolher.
              <ChevronDown
                size={12}
                className="absolute right-0 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none group-hover:text-blue-600 transition-colors"
              />
            )}
          </div>
        </div>
          ...
          >
            ...
            {pendingTickets > 0 && ( // Mostra a bolinha se houver chamados.
              <span className="absolute -top-1 -right-1 w-4 h-4 bg-amber-500 rounded-full border-2 border-white animate-bounce shadow-sm flex items-center justify-center">
                <span className="text-[7px] font-black text-slate-950">
                  {pendingTickets}
                </span>
              </span>
            )}
