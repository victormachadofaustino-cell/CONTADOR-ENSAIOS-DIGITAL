        ...
        {isOpen && (
          <div className="fixed inset-0 z-[200]">
            ...
            <motion.div
              ...
              animate={{ x: 0 }}
              exit={{ x: "100%" }}
              className="absolute top-0 right-0 h-full w-[88%] max-w-sm bg-[#F1F5F9] shadow-2xl flex flex-col text-left overflow-hidden"
            >
              ...
              <div className="p-8 bg-slate-950 text-white rounded-bl-[3rem] shadow-xl relative">
                <div className="flex justify-between items-start mb-6">
                  ...
                  <button
                    onClick={onClose}
                    className="p-2 bg-white/5 rounded-xl text-white/40 active:scale-90 transition-all"
                  >
                    <X size={20} />
                  </button>
                </div>
                    ...
                    {isEditingName ? (
                      <div className="flex items-center gap-2 w-full bg-white/10 rounded-lg px-2 py-1 border border-amber-500/50">
                        ...
                        <button
                          ...
                          className="text-emerald-400 active:scale-90 transition-all"
                        >
                      ...
                      <>
                        <h2 className="text-xl font-[900] uppercase italic tracking-tighter truncate max-w-[80%]">
                          {userData?.name}
                        </h2>
                        <button
                ...
                <div className="mt-6 flex flex-col gap-2">
                  <div className="inline-flex items-center gap-2 bg-white/10 px-3 py-1.5 rounded-full border border-white/5 self-start">
                    ...
                    <span className="text-[9px] font-black uppercase italic tracking-wider text-amber-500">
                      {getLevelLabel()}
                    </span>
                  </div>
                </div>
              ...
              <div className="flex-1 p-6 space-y-6 overflow-y-auto no-scrollbar">
                ...
                {isBasico && (
                  <div className="bg-amber-50 border border-amber-100 p-5 rounded-3xl space-y-2">
                    ...
                    <p className="text-[10px] font-medium text-amber-800/70 leading-relaxed">
                      Seu acesso permite Contar os instrumentos dos eventos
                      criados e visualizar Atas e Dashboards da sua comum.
                    </p>
                  </div>
                )}
                ...
                <div className="space-y-3">
                  <p className="text-[9px] font-black text-slate-400 uppercase italic px-2">
                    Colaboração
                  </p>
                  <button
                    onClick={() => setIsTicketsOpen(true)}
                    className="w-full bg-white p-5 rounded-[2rem] border border-slate-200 flex justify-between items-center active:scale-95 shadow-sm group"
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`p-2 rounded-xl ${isMaster && pendingTickets > 0 ? "bg-amber-500 text-slate-950 animate-pulse" : "bg-blue-50 text-blue-600"}`}
                      >
                        <Lightbulb size={18} />
                      </div>
                      <div className="text-left">
                        <span className="text-[10px] font-black text-slate-950 uppercase italic block leading-none">
                          {isMaster
                            ? "Zeladoria do Sistema"
                            : "Meu Mural de Sugestões"}
                        </span>
                        <span className="text-[8px] font-bold text-slate-400 uppercase mt-1 block">
                          {isMaster
                            ? `${pendingTickets} chamados ativos`
                    ...
                    <div className="bg-white p-5 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-left leading-tight">
                          <span className="text-[9px] font-black text-slate-900 uppercase block">
                            {isGemLocal ? "Sua Comum" : "Sua Cidade"}
                          </span>
                          <span className="text-[11px] font-bold text-slate-500 uppercase italic truncate block max-w-[150px]">
                            {userData?.comum || userData?.cidadeNome}
                          </span>
                        </div>
                      </div>
                ...
                {isMaster && (
                  <div className="space-y-3">
                    ...
                    <button
                      onClick={() => setIsRegionalManagerOpen(true)}
                      className="w-full bg-white p-5 rounded-[2rem] border border-slate-200 flex justify-between items-center active:scale-95 shadow-sm"
                    >
                      <div className="flex items-center gap-3">
                        <div className="p-2 bg-blue-50 text-blue-600 rounded-xl">
                          <Building size={16} />
                        </div>
                        <span className="text-[10px] font-black text-slate-950 uppercase italic leading-none">
                          Gerenciar Regionais
                        </span>
                      </div>
                      <span className="bg-blue-600 text-white text-[8px] px-2 py-1 rounded-lg font-black">
                        {listaRegionais.length}
                      </span>
                    </button>
                  </div>
                )}
              ...
              <div className="p-6 border-t border-slate-200 bg-white">
                <button
                  ...
                  className="w-full bg-red-50 text-red-600 p-4 rounded-2xl flex items-center justify-center gap-3 font-black text-[10px] uppercase italic active:scale-95 border border-red-100 transition-all hover:bg-red-100"
                >
      ...
      <AnimatePresence>
        {isTicketsOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-[300] bg-[#F1F5F9] flex flex-col p-6 overflow-hidden"
          >
            <div className="flex justify-between items-center mb-6">
              ...
              <button
                ...
                className="p-2 bg-white rounded-full text-slate-300 shadow-sm active:scale-90"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto space-y-3 no-scrollbar pb-10">
              {(isMaster ? tickets : myTickets).length === 0 ? (
                <div className="py-20 text-center text-slate-300 font-black uppercase italic text-[10px]">
                ...
                (isMaster ? tickets : myTickets).map((t) => (
                  <div
                    key={t.id}
                    className={`p-5 rounded-[2rem] border relative overflow-hidden transition-all bg-white ${t.status === "pendente" ? "border-blue-100 shadow-md" : "border-slate-100 opacity-80"}`}
                  >
                    <div
                      className={`absolute top-0 left-0 w-1 h-full ${t.status === "resolvido" || t.status === "agradecido" ? "bg-emerald-500" : t.status === "rejeitado" ? "bg-red-500" : "bg-amber-500"}`}
                    />
                    ...
                    <div className="flex justify-between mb-2">
                      <div className="flex gap-2 items-center">
                        ...
                        <div className="flex flex-col">
                          <span className="text-[10px] font-black uppercase text-slate-950 leading-none">
                            {t.userName}
                          </span>
                          <span className="text-[7px] font-bold text-slate-400 uppercase italic mt-1">
                            {t.tipo} • {t.modulo}
                          </span>
                        </div>
                      </div>
                    ...
                    <p className="text-[11px] font-bold text-slate-600 italic bg-slate-50 p-3 rounded-2xl mb-4">
                      "{t.mensagem}"
                    </p>

                    {t.respostaMaster && (
                      <div className="bg-blue-50/50 p-3 rounded-2xl border border-blue-100 mb-2">
                        <p className="text-[7px] font-black text-blue-600 uppercase mb-1 italic">
                    ...
                    {isMaster && t.status === "pendente" && (
                      <div className="mt-4 space-y-3">
                        <textarea
                          className="w-full bg-white border border-slate-200 rounded-2xl p-3 text-[10px] font-bold outline-none h-20 resize-none focus:border-blue-300 transition-all"
                          placeholder="Escreva uma nota de retorno..."
                          value={selectedTicketId === t.id ? resposta : ""}
                          onChange={(e) => {
                        ...
                        <div className="flex gap-2">
                          <button
                            onClick={() =>
                              handleUpdateStatus(t.id, "resolvido")
                            }
                            className="flex-1 bg-emerald-500 text-white py-3 rounded-xl text-[9px] font-black uppercase italic shadow-lg active:scale-95 transition-all"
                          >
                            Concluir
                          </button>
                          <button
                            onClick={() =>
                              handleUpdateStatus(t.id, "rejeitado")
                            }
                            className="bg-red-50 text-red-500 px-4 rounded-xl text-[9px] font-black uppercase italic border border-red-100"
                          >
                            Recusar
                          </button>
                        </div>
        ...
        {isRegionalManagerOpen && isMaster && (
          <div className="fixed inset-0 z-[400] flex items-center justify-center p-6 text-left">
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => setIsRegionalManagerOpen(false)}
              className="absolute inset-0 bg-slate-950/80 backdrop-blur-md"
            />
            <motion.div
              initial={{ scale: 0.9 }}
              animate={{ scale: 1 }}
              className="relative w-full max-w-sm bg-white rounded-[3rem] p-8 shadow-2xl flex flex-col max-h-[85vh]"
            >
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-[900] text-slate-950 uppercase italic tracking-tighter">
                  Regionais Ativas
              ...
              >
                <input
                  ...
                  className="flex-1 bg-slate-50 p-4 rounded-2xl font-black text-slate-950 text-[10px] outline-none border border-slate-200 focus:border-blue-400 transition-all uppercase italic"
                  value={newRegionalName}
                  onChange={(e) => setNewRegionalName(e.target.value)}
                />
                <button
                  type="submit"
                  className="bg-blue-600 text-white h-12 w-12 flex items-center justify-center rounded-2xl shadow-lg active:scale-90 transition-transform"
                >
                  <Plus size={20} />
                </button>
              </form>
              <div className="flex-1 overflow-y-auto space-y-2 no-scrollbar">
                {listaRegionais.map((reg) => (
                  <div
                    key={reg.id}
                    className="p-4 bg-slate-50 rounded-2xl flex justify-between items-center border border-slate-100 group"
                  >
                    <span className="text-[10px] font-black uppercase italic truncate max-w-[180px]">
                      {reg.nome}
                    </span>
                    <button
                      onClick={() =>
                        deleteDoc(doc(db, "config_regional", reg.id))
                      }
