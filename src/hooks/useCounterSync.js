import { useState, useEffect, useRef, useMemo } from 'react'; // Explicação: Importa as ferramentas esenciais do React para controlar estados, efeitos e cache de memória RAM.
import { db, doc, onSnapshot, collection, updateDoc, getDocs, query, where } from '../config/firebase'; // Explicação: Importa os conectores oficiais e métodos de consultas do SDK do Firebase Firestore.
...
export const useCounterSync = (currentEventId, counts) => { // Explicação: Declara o gancho Custom Hook que gerencia a reatividade de rede do contador de presenças.
  ...
  // v10.9.1: MONITOR DA SINCRONIZAÇÃO OTIMISTA E CONTROLE DE FOCO CONTRA PISCADAS
  useEffect(() => { // Explicação: Efeito que intercepta a chegada de novas contagens do Firebase e decide se mescla ou aguarda.
    const isFreshLocalUpdate = Date.now() - lastLocalUpdateRef.current < 3000; // Explicação: Confere se o celular sofreu clique nos últimos 3 segundos para proteger a RAM local.

    if (counts && !isFreshLocalUpdate) { // Explicação: Se houver dados novos na nuvem e o operador não estiver metralhando botões locais.
      if (focusedField) { // Explicação: Se o teclado físico ou virtual estiver aberto editando uma caixa de texto.
        const [instId, field] = focusedField.split('_'); // Explicação: Recorta a chave para isolar o ID do instrumento e o subcampo.
        setLocalCounts(prev => { // Explicação: Mescla de forma cirúrgica protegendo a digitação atual do usuário.
          const newCounts = { ...counts }; // Explicação: Clona integralmente a sacola atual de dados do Firebase.
          if (newCounts[instId]) { // Explicação: Se o instrumento ativo constar no novo pacote de rede.
            newCounts[instId] = { ...newCounts[instId], [field]: prev[instId]?.[field] }; // Explicação: Preserva o valor intermediário da RAM do input para não sumir o texto.
      ...
      } else { // Explicação: Caso o usuário esteja apenas assistindo a tela sem inputs ativos.
        setLocalCounts(counts); // Explicação: Atualiza e sincroniza a tela mestre imediatamente com os dados do servidor.
  ...
  useEffect(() => { // Explicação: Liga o link reativo para monitorar as mutações e o status de encerramento da ata.
    ...
    const unsubEvent = onSnapshot(eventRef, (s) => { // Explicação: Conecta o ouvinte em tempo real onSnapshot.
      if (s.exists() && isMounted) { // Explicação: Se o documento existir legitimamente no servidor e a flag de montagem estiver de pé.
        const data = s.data(); // Explicação: Despeja os dados internos do snapshot técnico.
        const ataConsolidada = { // Explicação: Consolida os metadados da ata com segurança contra nulos.
        ...
        const isFreshLocalUpdate = Date.now() - lastLocalUpdateRef.current < 3000; // Explicação: Recalcula o delay de clique de segurança.
        if (!isFreshLocalUpdate) { // Explicação: Se o operador estiver parado sem cliques recentes.
    ...
    return () => { isMounted = false; unsubEvent(); }; // Explicação: Corta e limpa a conexão com a nuvem ao sair do painel do contador.
  ...
  const updateCount = (id, field, value, section, userData) => { // Explicação: Método que processa e despacha a soma ou subtração de músicos de linha.
    ...
    lastLocalUpdateRef.current = Date.now(); // Explicação: Renova a marcação Unix do clique local para travar o escudo de 3 segundos anti-sobrescrita.
    
    setLocalCounts(prev => { // Explicação: Atualiza instantaneamente a tela de forma otimista para o app dar sensação de velocidade extrema.
      const targetId = (section?.toUpperCase() === 'IRMANDADE') ? 'coral' : (section?.toUpperCase() === 'ORGANISTAS') ? 'orgao' : id; // 🚀 ALINHAMENTO DE CHAVES SÊNIOR: Ajustado para minúsculo padrão para acoplar no 'coral' legítimo do banco.
      ...
      if (section?.toUpperCase() === 'IRMANDADE') { // Explicação: Avalia se o input pertence às linhas de gênero do Coral.
        ...
        return { // Explicação: Retorna o novo estado remontado com o total reajustado em tempo real de execução.
          ...prev, // Explicação: Clona os outros instrumentos intactos.
          'coral': { // 🚀 PACIFICAÇÃO CASE-SENSITIVE: Satura as chaves unificadas de destino em caixa baixa.
      ...
      return { ...prev, [targetId]: { ...prev[targetId], [field]: parsedValue } }; // Explicação: Lógica padrão otimista estável para os demais instrumentos comuns da orquestra.
    }); // Explicação: Encerra o setState otimista.

    eventService.updateInstrumentCount(eventComumId, currentEventId, { instId: id, field, value, userData, section }) // Explicação: Dispara o pacote enxuto estabilizado de rede para o motor central gravar no Firestore.
  ...
  const setOwnership = async (id, myUID, userName) => { // Explicação: Método encarregado de carimbar o ID e nome do secretário como dono daquela aba.
    ...
    setLocalCounts(prev => ({ // Explicação: Injeta o nome do operador de forma otimista no cabeçalho da sanfona visível.
      ...prev,
      [id]: { ...prev[id], responsibleId: myUID, responsibleName: userName || "Você" }
    })); // Explicação: Encerra o setState otimista da caneta.

    try { // Explicação: Tenta gravar a assinatura física no documento mestre do Firebase.
      await updateDoc(doc(db, 'events_global', currentEventId), { // Explicação: Envia a instrução de atualização de nós.
    ...
    } catch (e) { // Explicação: Trata erros de rede.
  ...
  return { // Explicação: Devolve a sacola contendo todas as variáveis e comandos de rede mastigados para a página visível comsumir.
    localCounts, setLocalCounts,
    ataData, eventComumId, eventDateRaw, isCountsLocked,
    ...
    updateCount, setOwnership, MAPA_TRADUTOR_EXTENSO // Explicação: Devolve os métodos operacionais e dicionário.
